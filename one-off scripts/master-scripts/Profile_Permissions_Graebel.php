...
$db = PearDatabase::getInstance();
$results = $db->pquery('SELECT name FROM vtiger_tab');
$tabs = [];
while ($row =& $results->fetchRow()) { //Permissions
    $tabs[] = $row[0];
}

foreach ($profiles as $profileName => $profileDetails) {           //Profiles
    foreach ($tabs as $tab) {
        foreach ($profiles['Administrator']['Calendar'] as $permissionName) {
            Vtiger_Utils::ExecuteQuery("UPDATE `vtiger_profile2tab` SET `permissions` = 0 WHERE `profileid` = (SELECT `profileid` FROM `vtiger_profile` WHERE `profilename` = '$profileName') AND `tabid` = (SELECT `tabid` FROM `vtiger_tab` WHERE `name` = '$tab')");
            insertPermission($profileName, $tab, $permissionName, 0);
        }
    }
}
