...
print "<h2>Begin modifications to Accounts fields  </h2>\n";
foreach ($fieldNames as $fieldName) {
    AddMandatoryAMFM($moduleName, $fieldName);
}
print "<h2>END modifications to Accounts fields </h2>\n";

function AddMandatoryAMFM($moduleName, $fieldName)
{
    $db = PearDatabase::getInstance();
    if ($module = Vtiger_Module::getInstance($moduleName)) {
        $accountField = Vtiger_Field::getInstance($fieldName, $module);
        if ($accountField) {
            $typeOfData = $accountField->typeofdata;
            ...
            if ($isMatch === false) {
                ...
                print "$stmt\n";
                print "$typeOfData, " . $accountField->id  ."<br />\n";
                $db->pquery($stmt, [$typeOfData, $accountField->id]);
