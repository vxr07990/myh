...
// OT 2669 - replacing fields in Status picklist for Vehicles

echo "<br>Started Vehicle StatusPicklistUpdate</br>";
...
$moduleName = 'Vehicles';
$picklistFieldName = 'vehicle_status';
...
if ($field1) {
    ...
    blowOutPickListVSPU($picklistFieldName);

    $picklistValues = [];
    foreach ($picklistOrder as $key=>$value) {
        $id = getPicklistIdVSPU($picklistFieldName, $value);
        print "ID: $id<br/>";
        if ($id === false) {
            ...
            $newItem = addNewPicklistItemVSPU($value, $picklistFieldName, $moduleName);
            $id = $key;
...
function addNewPicklistItemVSPU($newValue, $pickListName, $moduleName)
{
    $response = false;
...
function getPicklistIdVSPU($picklistFieldName, $picklistValue)
{
    $rv = false;
...
function blowOutPickListVSPU($picklistFieldName)
{
    $db = PearDatabase::getInstance();
...
echo "<br>Finished Vehicle StatusPicklistUpdate</br>";
