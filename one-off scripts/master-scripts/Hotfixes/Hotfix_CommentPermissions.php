...

{
    ...
    if ($module) {
        ...
        */
        ...
        $result = $db->pquery('SELECT `profilename` FROM `vtiger_profile`', []);
        ...
        if (is_array($fields) && $fields > 0) {
            ...
            print "Fields is not an array with at least 1 thing.<br />";
        }
...
function newPermissions($profileName, $tabId, $permissions, $activities, $fields, $moduleName)
{
    ...
    foreach ($fields as $fieldId => $readOnly) {
        $permission0 = getFieldPermission($profileId, $fieldId, $tabId);
        if ($permission0 === null) {
            echo "<br>field $fieldId permissions not set for $profileName, CORRECTING...<br>";
            setFieldPermission($profileId, $fieldId, $readOnly, $tabId, $permission);
            echo "<br>DONE!<br>";
        } elseif ($permission0 != $permission) {
            echo "<br>field $fieldId permissions not CORRECT for $profileName, CORRECTING...<br>";
            updateFieldPermission($profileId, $fieldId, $readOnly, $tabId, $permission);
            echo "<br>DONE!<br>";
        } else {
...
function setFieldPermission($profileId, $fieldId, $readOnly, $tabId, $visible)
{
    $adb = PearDatabase::getInstance();
    $sql = 'INSERT INTO `vtiger_profile2field` (profileid, tabid, fieldid, visible, readonly) VALUES (?, ?, ?, ?, ?)';
    $adb->pquery($sql, array($profileId, $tabId, $fieldId, $visible, $readOnly));
}

function updateFieldPermission($profileId, $fieldId, $readOnly, $tabId, $visible)
{
    $adb = PearDatabase::getInstance();
    $sql = 'UPDATE `vtiger_profile2field` SET profileid = ?, tabid = ?, fieldid = ?, visible = ?, readonly = ?
            WHERE profileid = ? AND tabid = ? AND fieldid = ?';
    $adb->pquery($sql, array($profileId, $tabId, $fieldId, $visible, $readOnly, $profileId, $tabId, $fieldId));
}
