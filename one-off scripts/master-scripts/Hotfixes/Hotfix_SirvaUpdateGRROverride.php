 ...
 *
 *Goals:
 * update grr_override_amount aka LBL_ESTIMATES_GRROVERIDEAMOUNT
 * in block: LBL_QUOTES_TPGPRICELOCK
 */
include_once('vtlib/Vtiger/Menu.php');
include_once('vtlib/Vtiger/Module.php');

foreach (['Estimates', 'Quotes'] as $moduleName) {
    print "<h2>START update GRR override for $moduleName module. </h2>\n";
    $blockName  = 'LBL_QUOTES_TPGPRICELOCK';
    ...
    if (!$module) {
        ...
        if (!$block) {
            ...
            $fields   = [
            'grr_override_amount' => [
                'label'           => 'LBL_ESTIMATES_GRROVERIDEAMOUNT',
                'name'            => 'grr_override_amount',
                'table'           => 'vtiger_quotes',
                'column'          => 'grr_override_amount',
            ...
            $rField   = addFields_SUGO($fields, $module, true);
            echo "<li>finished update to fields in $blockName in $moduleName</li><br>";
        }
    }
...
//END process update
function addFields_SUGO($fields, $module)
{
    $returnFields = [];
