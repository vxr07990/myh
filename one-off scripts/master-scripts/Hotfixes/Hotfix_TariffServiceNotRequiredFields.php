...
if ($field1) {
    ...
    if ($db->getAffectedRowCount($query)>0) {
        echo '<h4>Updated '.$field1->name.' Successfully with a typeofdata V~O</h4>';
    } else {
        echo '<h4>'.$field1->name.' already has typeofdata set to V~O</h4>';
    }
} else {
    echo '<h3>FAILED TO FIND A FIELD WITH A NAME OF bulky_chargeper - Update failed</h3>';
}

$field1 = Vtiger_Field::getInstance('service_base_charge_applies', $module);
...
if ($field1) {
    ...
    echo '<h3>FAILED TO FIND A FIELD WITH A NAME OF service_base_charge_applies - Update failed</h3>';
}
