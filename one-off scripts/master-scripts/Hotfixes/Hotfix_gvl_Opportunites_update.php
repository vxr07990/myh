 ...
 *
 *Goals:
 * Updates to Opportunities based on design mock up.
 *
 ...
 *
 * RENAME:
 *  - Sales Stage  => Opportunity Status
 *  - Billing Type => shipment type
 *
 * Create new block: HHG Information
 * MOVE from Opportunity Details
 *  - Competitive
 *  - Coordinator ******
 *
 * Update Address Details to Address Information:
 * HIDE:
 *  - Origin Phone 1
 *  - Destination Phone 1
 *  - Origin Phone 2
...
$hhgBlockName = 'LBL_OPPORTUNITIES_HHG_INFORMATION';
...
foreach (['Opportunities', 'Potentials'] as $moduleName) {
    ...
    if (!$module) {
        ...
        $hhgBlockInstance = Vtiger_Block::getInstance($hhgBlockName, $module);
        if (!$hhgBlockInstance) {
            $hhgBlockInstance        = new Vtiger_Block();
            $hhgBlockInstance->label = $hhgBlockName;
            $module->addBlock($hhgBlockInstance);
        }
        ...
        hideFields_GOU($hideFields, $module);
        ...
        moveFields_GOU($moveFields, $module, $hhgBlockInstance->id);
        ...
        reorderBlocks_GOU($blockSeq, $module);
        ...
        echo "<li>Reordering block $hhgBlockName</li><br>";
        reorderFieldsByBlock_GOU($fieldSeq, $hhgBlockName, $moduleName);
    ...
    $fieldSeq = [
        ...
        'related_to',
        'leadsource',
        //@NOTE: this isn't a field here?
        //'sales_person',
        'closingdate',
        'agentid',
        'assigned_user_id',
        //these are not shown, don't care about where they are
        'potential_no',
        'amount',
        'opportunity_type',
        'nextstep',
    ...
    reorderFieldsByBlock_GOU($fieldSeq, 'LBL_OPPORTUNITY_INFORMATION', 'Potentials');
    ...
    reorderFieldsByBlock_GOU($fieldSeq, 'LBL_POTENTIALS_INFORMATION', 'Opportunities');
}

function moveFields_GOU($fields, $module, $newBlockID)
{
    if (is_array($fields)) {
...
//END process update
function hideFields_GOU($fields, $module)
{
    if (is_array($fields)) {
        $db = PearDatabase::getInstance();
        foreach ($fields as $field_name) {
...
//END process update
function addFields_GOU($fields, $module)
{
    $returnFields = [];
...
function reorderFieldsByBlock_GOU($fieldSeq, $blockLabel, $moduleName)
{
    $db = PearDatabase::getInstance();
...
function reorderBlocks_GOU($blockSeq, $module)
{
    $db = PearDatabase::getInstance();
