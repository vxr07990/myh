 ...
 *
 * Create Block: IntraTariff Information
 *
 * Add:
 * Intra ca discount
 * Intra ca SIT discount
 * Intra tx discount
 * Intra tx SIT discount
 * Intra dereg state discount
 * Intra dereg state SIT discount
 *
...
$blockName = 'LBL_CONTRACTS_INTRA_TARIFF_INFORMATION';
foreach (['Contracts'] as $moduleName) {
    ...
    if (!$module) {
        ...
        if ($block) {
            ...
            moveFields_CCUITB($moveFields, $module, $block->id);
            */
            
            //add these fields:
            $addFields = [
                'intra_ca_discount' => [
                    'label'           => 'LBL_CONTRACTS_INTRA_CA_DISCOUNT',
                    'name'            => 'intra_ca_discount',
                    'table'           => 'vtiger_contracts',
                    'column'          => 'intra_ca_discount',
                ...
                'intra_ca_sit_discount' => [
                    'label'           => 'LBL_CONTRACTS_INTRA_CA_SIT_DISCOUNT',
                    'name'            => 'intra_ca_sit_discount',
                    'table'           => 'vtiger_contracts',
                    'column'          => 'intra_ca_sit_discount',
                    'columntype'      => 'varchar(15)',
                    'uitype'          => 9,
                    'typeofdata'      => 'N~O',
                    'displaytype'     => '1',
                ...
                'intra_tx_discount' => [
                    'label'           => 'LBL_CONTRACTS_INTRA_TX_DISCOUNT',
                    'name'            => 'intra_tx_discount',
                    'table'           => 'vtiger_contracts',
                    'column'          => 'intra_tx_discount',
                    'columntype'      => 'varchar(15)',
                    'uitype'          => 9,
                    'typeofdata'      => 'N~O',
                    'displaytype'     => '1',
                ...
                'intra_tx_sit_discount' => [
                    'label'           => 'LBL_CONTRACTS_INTRA_TX_SIT_DISCOUNT',
                    'name'            => 'intra_tx_sit_discount',
                    'table'           => 'vtiger_contracts',
                    'column'          => 'intra_tx_sit_discount',
                    'columntype'      => 'varchar(15)',
                    'uitype'          => 9,
                    'typeofdata'      => 'N~O',
                    'displaytype'     => '1',
                ...
                'intra_dereg_state_discount' => [
                    'label'           => 'LBL_CONTRACTS_INTRA_DEREG_STATE_DISCOUNT',
                    'name'            => 'intra_dereg_state_discount',
                    'table'           => 'vtiger_contracts',
                    'column'          => 'intra_dereg_state_discount',
                    'columntype'      => 'varchar(15)',
                    'uitype'          => 9,
                    'typeofdata'      => 'N~O',
                    'displaytype'     => '1',
                ...
                'intra_dereg_state_sit_discount' => [
                    'label'           => 'LBL_CONTRACTS_INTRA_DEREG_STATE_SIT_DISCOUNT',
                    'name'            => 'intra_dereg_state_sit_discount',
                    'table'           => 'vtiger_contracts',
                    'column'          => 'intra_dereg_state_sit_discount',
                    'columntype'      => 'varchar(15)',
                    'uitype'          => 9,
                    'typeofdata'      => 'N~O',
                    'displaytype'     => '1',
            ...
            addFields_CCUITB($addFields, $module);
            ...
            unhideFields_CCUITB($orderFieldSeq, $module);
            
            echo "<li>Reordering block $blockName</li><br>";
            reorderFieldsByBlock_CCUITB($orderFieldSeq, $blockName, $moduleName);

            //no fields to hide since this is being created!
            //$hideFields = [
            //];
            //hideFields_CCUITB ($hideFields, $module);
        }
        print "<h2>finished add fields to $moduleName module. </h2>\n";
    }
}

function moveFields_CCUITB($fields, $module, $newBlockID)
{
    if (is_array($fields)) {
...
//END process update
function hideFields_CCUITB($fields, $module)
{
    if (is_array($fields)) {
        $db = PearDatabase::getInstance();
        foreach ($fields as $field_name) {
...
function unhideFields_CCUITB($fields, $module)
{
    if (is_array($fields)) {
        $db = PearDatabase::getInstance();
        foreach ($fields as $field_name) {
...
//END process update
function addFields_CCUITB($fields, $module)
{
    $returnFields = [];
...
function reorderFieldsByBlock_CCUITB($fieldSeq, $blockLabel, $moduleName)
{
    $db = PearDatabase::getInstance();
...
function reorderBlocks_CCUITB($blockSeq, $module)
{
    $db = PearDatabase::getInstance();
