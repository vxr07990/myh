...
$sql = "SELECT vtiger_agentmanager.* FROM vtiger_agentmanager
        JOIN `vtiger_crmentity` ON (vtiger_crmentity.crmid = vtiger_agentmanager.agentmanagerid)
        WHERE `deleted`=0 AND `setype`='AgentManager'";
$result = $db->pquery($sql, []);
while ($row =& $result->fetchRow()) {
    try {
        $matchSql = "SELECT vtiger_agents.* FROM vtiger_agents
                JOIN `vtiger_crmentity` ON (vtiger_crmentity.crmid = vtiger_agents.agentsid)
                WHERE `deleted`=0 AND `agent_number`=? LIMIT 1";
