 ...
 *
 *Goals:
 * "create" a new uitype so we can have a uitype that makes a picklist of value = id and text = name
 * instead of the 15 type which is value = name and text = name.
 *
 * Add tariff_id to LBL_ORDERS_INFORMATION for Orders
 *
...
foreach (['Orders'] as $moduleName) {
    print "<h2>START add tariff field to $moduleName module. </h2>\n";
    ...
    if (!$module) {
        ...
        if (!$block) {
            ...
            $rField   = addFields_ATTO($fields, $module, true);
        }
    }
...
//END process update
function addFields_ATTO($fields, $module)
{
    $returnFields = [];
