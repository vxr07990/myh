...
if (function_exists("call_ms_function_ver")) {
    ...
    $rebuildPackItems = false;
} else {
    $rebuildPackItems = true;
}
...
if (!Vtiger_Utils::CheckColumnExists('vtiger_tariffbulky', 'standardItem')) {
    Vtiger_Utils::AddColumn('vtiger_tariffbulky', 'CartonBulkyId', 'INT(30) DEFAULT NULL');
    Vtiger_Utils::AddColumn('vtiger_tariffbulky', 'standardItem', 'INT(3) DEFAULT 0');
    $rebuildPackItems = true;
}
...
if ($rebuildPackItems) {
    ...
    $stmt                 = 'SELECT * FROM `vtiger_tariffbulky` WHERE `CartonBulkyId` IS NULL AND `description` = ?';
