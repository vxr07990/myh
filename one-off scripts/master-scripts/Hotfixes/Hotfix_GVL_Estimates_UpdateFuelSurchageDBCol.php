...
//accessorial_fuel_surcharge is a max of 9.999 because it's a decimal (5,3) changing it to match these others.
//a decimal (10,3) based on one-off scripts/master-scripts/Hotfixes/Hotfix_Sirva_UpdateFuelSurchageDBCol.php
//but that did varchar(11)
                ...
                if ($res = $db->pquery($stmt)) {
                    while ($value = $res->fetchRow()) {
                        if ($value['Field'] == $fieldName) {
                            if (strtolower($value['Type']) != 'decimal(10,3)') {
                                echo "Updating $fieldName to be a decimal(10,3) type.<br />\n";
                                $db   = PearDatabase::getInstance();
                                $stmt = 'ALTER TABLE `vtiger_quotes` MODIFY COLUMN `'.$fieldName.'` decimal(10,3) DEFAULT NULL';
                                $db->pquery($stmt);
                            }
