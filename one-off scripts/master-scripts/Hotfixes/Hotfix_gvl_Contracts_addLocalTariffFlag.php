...
foreach (['Contracts'] as $moduleName) {
    ...
    if (!$module) {
        ...
        $fields = [];
        $block0 = Vtiger_Block::getInstance($blockName, $module);
        if ($block0) {
            $fields = [
                'local_tariff' => [
                    'label'           => 'LBL_CONTRACT_LOCAL_TARIFF',
                    'name'            => 'local_tariff',
            ...
            addFields_GCALTF($fields, $module);
            $fieldSeq = [
                'contract_no',
                'nat_account_no',
                'account_id',
            ...
            reorderFieldsByBlock_GCALTF($fieldSeq, $blockName, $moduleName);
        } else {
            print "Failed to load block: $blockName -- $moduleName<br />\n";
        }
    }
}

//END process update
function addFields_GCALTF($fields, $module)
{
    $returnFields = [];
...
function reorderFieldsByBlock_GCALTF($fieldSeq, $blockLabel, $moduleName)
{
    $db = PearDatabase::getInstance();
