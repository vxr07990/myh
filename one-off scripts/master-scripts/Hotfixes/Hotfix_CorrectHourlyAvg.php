 ...
 *
 *Goals:
 * Update TariffServices to have Hourly Average (Lb/Man/Hr) to be
 * use for poundage and not rates.
 *
 * add label to: languages/en_us/TariffServices.php
 *
 */
include_once('vtlib/Vtiger/Menu.php');
include_once('vtlib/Vtiger/Module.php');

$blockName  = 'LBL_TARIFFSERVICES_HOURLYAVG';
foreach (['TariffServices'] as $moduleName) {
    ...
    if (!$module) {
        ...
        if (!$block) {
            ...
            $fields = [
                'hourlyavg_rate' => [
                    'label'               => 'LBL_TARIFFSERVICES_POUNDS',
                    'name'                => 'hourlyavg_rate',
                    'table'               => 'vtiger_tariffservices',
                    'column'              => 'hourlyavg_rate',
            ...
            $rField = addFields_CHA($fields, $module, true);

            $rate_type_field = Vtiger_Field::getInstance('rate_type', $module);
            if ($rate_type_field) {
                $rate_type_value = 'Hourly Avg Lb/Man/Hour';
                $db   = PearDatabase::getInstance();
                $stmt = 'SELECT * FROM `vtiger_rate_type` WHERE `rate_type` = ? LIMIT 1';
                if ($res = $db->pquery($stmt, [$rate_type_value])) {
                    $value      = $res->fetchRow();
                    if ($value['presence'] == 1) {
                        print "UPDATING picklist to hide ($rate_type_value)<br />";
                        $updateStmt = 'UPDATE `vtiger_rate_type` set `presence` = 0 WHERE `rate_typeid` = ?';
                        $db->pquery($updateStmt, [$value['rate_typeid']]);
                    }
                } else {
                    print "DID NOT Find rate_type ($rate_type_value) in (vtiger_rate_type)<br />";
                }
            }
        }
    }
...
//END process update
function addFields_CHA($fields, $module)
{
    $returnFields = [];
        ...
        if ($field0) {
            ...
            if ($data['replaceExisting'] && $data['block']->id == $field0->getBlockId()) {
                ...
                if ($data['uitype'] && $field0->uitype != $data['uitype']) {
                    echo "Updating $field_name to uitype=".$data['uitype'].".<br />\n";
