...
if ($moduleInstance) {
    ...
    $moduleInstance->name = 'TariffManager';
    $moduleInstance->save();
...
$oldNameField = Vtiger_Field::getInstance('name', $moduleInstance);
if ($oldNameField) {
    Vtiger_Utils::ExecuteQuery("UPDATE `vtiger_field` SET presence=1 WHERE fieldid=".$oldNameField->id);
}

$agencyNum = Vtiger_Field::getInstance('tariffmanager_agencynumber', $moduleInstance);
if ($agencyNum) {
    Vtiger_Utils::ExecuteQuery("UPDATE `vtiger_field` SET presence=1 WHERE fieldid=".$agencyNum->id);
}
/*
