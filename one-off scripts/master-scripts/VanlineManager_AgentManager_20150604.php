...
$vanlineIsNew = false;
$agentIsNew = false;
/*$menuInstance = Vtiger_Menu::getInstance('RELOHQ_SYSTEM_ADMIN');
if($menuInstance)
{
    echo "<br> menue RELOHQ_SYSTEM_ADMIN ALalready exists <br>";
}
else {
    $menuInstance = Vtiger_Menu();
    $menuInstance->label = 'SYSTEM_ADMIN_TAB';
...
if (!$vanlineInstance) {
    ...
    ModTracker::enableTrackingForModule($vanlineInstance->id);
}
...
if ($vanlineField13) {
    ...
    $vanlineField13->column = 'modifiedtime';
...
/*$vanlineField15 = Vtiger_Field::getInstance('is_parent', $vanlineInstance);
if($vanlineField15) {
    echo "Field is_parent already exists";
} else {
    $vanlineField15 = new Vtiger_Field();
    $vanlineField15->label = 'LBL_VANLINEMANAGER_ISPARENT';
    $vanlineField15->name = 'is_parent';
    $vanlineField15->table = 'vtiger_vanlinemanager';
    $vanlineField15->column = 'is_parent';
    $vanlineField15->columntype = 'VARCHAR(3)';
    $vanlineField15->uitype = 56;
    $vanlineField15->typeofdata = 'C~O';

    $vanlineBlock1->addField($vanlineField15);

    //createDefaultProfile("Parent Vanline User");
}*/

if ($vanlineIsNew) {
    echo "<br><h2>IN VANLINE IS NEW IF STATMENT</h2><br>";
...
if (!$agentInstance) {
    ...
    ModTracker::enableTrackingForModule($agentInstance->id);
}

echo "<h4>begin set agent manager sharing script</h4><br>";
if ($agentInstance) {
    echo "agent manager exists, setting sharing rules<br>";
    //$agentInstance->setDefaultSharing('Private');
    //$agentInstance->initWebservice();
    echo "sharing rules set<br>";
} else {
    echo "agent manager does not exist, no action taken to set sharing<br>";
}
...
if ($agentField14) {
    ...
    $agentField14->column = 'modifiedtime';
...
if ($agentIsNew) {
    echo "<br><h2>IN AGENT IS NEW IF STATMENT</h2><br>";
