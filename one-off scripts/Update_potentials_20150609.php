...
if ($field1) {
    ...
    $field1->setPicklistValues(array('Local Move', 'Interstate Move', 'Commercial Move', 'National Account'));

    $potentialsblock1->addField($field1);
}

$field2 = Vtiger_Field::getInstance('estimate_type', $potentialsInstance);
if ($field2) {
    echo "<li>The estimate_type field already exists</li><br>";
} else {
    $field2 = new Vtiger_Field();
    $field2->label = 'LBL_POTENTIALS_ESTIMATETYPE';
    $field2->name = 'estimate_type';
    $field2->table = 'vtiger_potentialscf';
    $field2->column = 'estimate_type';
    $field2->columntype='VARCHAR(200)';
    $field2->uitype = 16;
    $field2->typeofdata = 'V~O';
    $field2->displaytype = 1;
    $field2->quickcreate = 1;
    $field2->setPicklistValues(array('Binding', 'Non-Binding', 'Not to Exceed'));

    $potentialsblock1->addField($field2);
}

$field3 = Vtiger_Field::getInstance('pricing_type', $potentialsInstance);
if ($field3) {
    echo "<li>The pricing_type field already exists</li><br>";
} else {
    $field3 = new Vtiger_Field();
    $field3->label = 'LBL_POTENTIALS_PRICING';
    $field3->name = 'pricing_type';
    $field3->table = 'vtiger_potentialscf';
    $field3->column = 'pricing_type';
    $field3->columntype='VARCHAR(200)';
    $field3->uitype = 16;
    $field3->typeofdata = 'V~O';
    $field3->displaytype = 1;
    $field3->quickcreate = 2;
    $field3->setPicklistValues(array('Peak', 'Non Peak'));

    $potentialsblock1->addField($field3);
}
