...
class Vtiger_Loader
{
    ...
    protected static $existsPathCache = array();
    ...
    {
        if (!array_key_exists($path, self::$existsPathCache)) {
            self::$existsPathCache[$path] = file_exists($path);
        }
        return self::$existsPathCache[$path];
    }
    ...
    public static function getComponentClassName($componentType, $componentName, $moduleName='Vtiger')
    {
        ...
        if (self::fileExists($moduleSpecificComponentFilePath)) {
        ...
        if (!empty($firstFallBackDir) && !empty($firstFallBackClassPath)) {
            ...
            if (self::fileExists($fallBackComponentFilePath)) {
