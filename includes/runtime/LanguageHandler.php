...
class Vtiger_Language_Handler
{
    ...
    public static function getModuleStringsFromFile($language, $module='Vtiger')
    {
        ...
        if (empty(self::$languageContainer[$language][$module])) {
            ...
            if (file_exists($file)) {
                //@NOTE: if you're here looking for blank lines.  Good!  Good on you!
                //it's the language file, so print $file here and find that file has extra
                //blank lines after the php close tag which probably should have just been omitted.
                require $file;
    ...
    {
        $res = Vtiger_Cache::get('LanguageHandler', 'CurrentUser');
        if($res)
        {
            return $res;
        }
        ...
        $res = empty($language) ? vglobal('default_language') : $language;
        Vtiger_Cache::set('LanguageHandler', 'CurrentUser', $res);
        return $res;
    }
...
function getTimeZoneDisplayValue($timeZoneName, $dateTimeValue=null) {
    ...
    try {
        $timeZoneObject = new DateTimeZone($timeZoneName);
    } catch (Exception $e) {
        return '';
    }

    $timeZoneTranslation = vtranslate($timeZoneName, 'Users');
    $timeZoneAfterOffset = strstr($timeZoneTranslation, ' ');
    $offset = $timeZoneObject->getOffset($dateTime) / 3600; //offset gets returned in seconds - converting to hours
    $timeZoneDisplay = $offset < 0 ? '-' : '+';
    $offset = abs($offset);
    if($timeZoneObject->getOffset($dateTime) % 3600 == 0) {
        $timeZoneDisplay .= $offset.':00';
    } else {
        $timeZoneDisplay .= $offset.':30';
    }

    return '(UTC' . $timeZoneDisplay . ')' . $timeZoneAfterOffset;
}

function getTimeZoneDisplayValueFromFieldNames($timeZoneName, $dateField, $timeField, $recordId) {
    $dateValue = date('Y-m-d');
    $timeValue = date('H:i:s');
    if ($recordId) {
        try {
            $record    = Vtiger_Record_Model::getInstanceById($recordId);
            $dateValue = $record->get($dateField);
            $timeValue = $record->get($timeField);
        } catch (Exception $ex) {
            //OH NOEs
        }
    }
    return getTimeZoneDisplayValue($timeZoneName, $dateValue.' '.$timeValue);
}
