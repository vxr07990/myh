...
class Vtiger_Viewer extends SmartyBC
{
    ...
    public function __construct($media='')
    {
        ...
        } elseif (empty($templatesDir) || !file_exists($templatesDir)) {
...
{
    global $vtiger_current_version, $rev;
    if (stripos($url, '://') === false) {
        if (!$rev) {
            //@NOTE: the trim could make it empty...
            if (getenv('REVISION')) {
                $rev = trim(getenv('REVISION'));
            } else if (getenv('FOOTER')) {
                //@NOTE: backward compatible version that made sense at thetime
                $rev = getenv('FOOTER');
                //FOOTER = "Rev d9cccbf7"
                $rev = preg_replace('/.*rev[\s:]+/i', '', $rev);
                //@NOTE: Actually it can be: FOOTER = "Rev ccaa3555 Server: QA"
                $rev = preg_replace('/\s+.*$/', '', $rev);
            }
        }
        if ($rev) {
            $url = $url.'?v='.$rev;
