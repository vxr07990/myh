...
class Users_Edit_View extends Users_PreferenceEdit_View
{
    ...
    public function preProcessSettings(Vtiger_Request $request)
    {
        ...
        $viewer->assign('VANLINE_DEPTH', $this->getVanlineDepth());
        $viewer->view('SettingsMenuStart.tpl', $qualifiedModuleName);
    }
    public function getVanlineDepth()
    {
        $db = PearDatabase::getInstance();
        $sql = "SELECT roleid FROM `vtiger_role` WHERE depth = 2 OR depth = 3";
    ...
    {
        $db = PearDatabase::getInstance();
        $sql = "SELECT roleid FROM `vtiger_role` WHERE rolename = '$roleName'";
        $result = $db->pquery($sql, []);
        $return = '';
        while ($row =& $result->fetchRow()) {
            $return = $row['roleid'];
