...
class Users_InAgentManagerRelation_View extends Vtiger_Index_View
{
    public function process(Vtiger_Request $request)
    {
        $agentManagerId = $request->get('record');
		...
		JOIN `vtiger_user2agency` ON `vtiger_users`.id=`vtiger_user2agency`.userid WHERE `vtiger_user2agency`.agency_code=?";
        $result         = $db->pquery($query, [$agentManagerId]);
        $usersArray     = [];
        ...
        $row          = $result->fetchRow();
        //file_put_contents('logs/devLog.log', "\n depth: $row[0]", FILE_APPEND);
        if (($row[0] <= 6 && getenv('INSTANCE_NAME') != 'sirva') || ($row[0] <= 3 && getenv('INSTANCE_NAME') == 'sirva')) {
            $canAdd = true;
        }
        $addButton =
            $canAdd?'<button type="button" class="btn addButton" data-url="index.php?module=AgentManager&view=AddAgencyUser&record='.
                    $agentManagerId.
                    '&user='.
                    $userId.
