...
class Users_SaveAjax_Action extends Vtiger_SaveAjax_Action
{
    ...
    public function checkPermission(Vtiger_Request $request)
    {
        ...
        if (!$currentUserModel->isAdminUser() && !$currentUserModel->isParentVanLineUser()) {
            $mode = $request->getMode();
            if (($mode == 'savePassword' || $mode == 'saveExchangeCredentials') && (isset($userId) && $currentUserModel->getId() != $userId)) {
                throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
            } elseif (($mode != 'savePassword' && $mode != 'saveExchangeCredentials') && ($currentUserModel->getId() != $request->get('record'))) {
                throw new AppException(vtranslate('LBL_PERMISSION_DENIED', 'Vtiger'));
            }
        }
    }
    ...
    public function savePassword(Vtiger_Request $request)
    {
        ...
        if($request->get('relModule') == 'Employees') {
            if($this->checkParentVanlinePermissions($request)) {
                $user = new Users();
                $userModel = $user->retrieveCurrentUserInfoFromFile(1);
            } else {
    ...
    private function checkParentVanlinePermissions(Vtiger_Request $request) {
        global $current_user;
        $agentsCurrentUser = getMembersByUser($current_user->id);
        $agentsTargetUser  = getMembersByUser($request->get('userid'));
        $agentsIntersect   = array_intersect($agentsCurrentUser, $agentsTargetUser);
        return count($agentsIntersect) > 0;
    }
}
