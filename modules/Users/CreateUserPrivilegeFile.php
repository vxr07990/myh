...
/**
 * Get the OS specific owner's group name.
 *
 * @todo Implement a Windows specific platform check
 *
 * @internal
 *
 * @return string Returns "staff" on OS X or "www-data" otherwise
 */
function _get_os_group_name()
{
    if (function_exists('posix_uname') && posix_uname()[sysname] == 'Darwin') {
        return 'staff';
    }
...
{
    file_put_contents('logs/chmodTest.log', date('Y-m-d H:i:s - ')."Entering createUserPrivilegesfile function\n", FILE_APPEND);
    global $root_directory;
    $handle=@fopen($root_directory.'user_privileges/user_privileges_'.$userid.'.php', "w+");
    $os_group_name = _get_os_group_name();
    ...
    if ($handle) {
        ...
        $user_focus->retrieve_entity_info($userid, "Users", true);
        $userInfo=array();
        ...
        if ($user_focus->is_admin == 'on') {
            ...
            chgrp($root_directory.'user_privileges/user_privileges_'.$userid.'.php', $os_group_name);
            chmod($root_directory.'user_privileges/user_privileges_'.$userid.'.php', 0755);
            return;
        } else {
...
function createUserSharingPrivilegesfile($userid)
{
    ...
    if ($handle) {
        ...
        if ($user_focus->is_admin == 'on') {
            ...
            fputs($handle, $newbuf);
            fclose($handle);
            chgrp($root_directory.'user_privileges/sharing_privileges_'.$userid.'.php', $os_group_name);
            chmod($root_directory.'user_privileges/sharing_privileges_'.$userid.'.php', 0755);
            return;
        } else {
