        ...
        foreach ($blockModelList as $blockLabel => $blockModel) {
            ...
            if ($fieldModelList) {
                ...
                foreach ($fieldModelList as $fieldName => $fieldModel) {
                    ...
                    $fieldModel->set('editable', $fieldModel->isEditable());

                    if (
                        empty($recordId) &&
                        (
                            $fieldModel->get('uitype') == 99 ||
                            $fieldModel->get('uitype') == 106
                        )
                    ) {
                    ...
                    if (
                        in_array($fieldModel->get('uitype'), array(156)) &&
                        $currentUserModel->getId() !== $recordId
                    ) {
                        $fieldModel->set('editable', true);
                        if ($fieldModel->get('uitype') == 156) {
                            $fieldValue = false;
                            //@NOTE: looks like this isn't used because it's checked from this get?
