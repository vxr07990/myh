...
class Users_Privileges_Model extends Users_Record_Model
{
    ...
    public static function getInstanceById($userId)
    {
        ...
        //Require the files exist before loading them.
    ...
    public static function getCurrentUserPrivilegesModel()
    {
        ...
        //short if the userid is not set, this should never happen, but might as well handle
        if (!$currentUserId) {
            return null;
        }

        $cache = Vtiger_Cache::getInstance();
        //mapping cache name to include the currentUserId to encompass the case where the current user id can be changed during run
        if ($cache->get('usersInfo', 'currentUserPrivs'.$currentUserId)) {
            return $cache->get('usersInfo', 'currentUserPrivs'.$currentUserId);
        } else {
            $instance = self::getInstanceById($currentUserId);
            $cache->set('usersInfo', 'currentUserPrivs'.$currentUserId, $instance);
        }
    ...
    public static function isPermitted($moduleName, $actionName, $record=false)
    {
        ...
        if ($permission == 'yes') {
            if ($moduleName == 'VanlineManager') {
                //file_put_contents('logs/hideThingsForGlobalEdit.log', "\nmodule: $moduleName \naction: $actionName \nrecord: $record \n Permitted.", FILE_APPEND);
            }
            return true;
        }
        if ($moduleName == 'VanlineManager') {
            //file_put_contents('logs/hideThingsForGlobalEdit.log', "\nmodule: $moduleName \naction: $actionName \nrecord: $record \n Not Permitted.", FILE_APPEND);
        }
        return false;
    }
