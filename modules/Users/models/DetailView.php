...
class Users_DetailView_Model extends Vtiger_DetailView_Model
{
    ...
    public function getDetailViewLinks($linkParams)
    {
        ...
        if (($currentUserModel->isParentVanLineUser() == true || $currentUserModel->isAdminUser() == true || $currentUserModel->get('id') == $recordId || (getenv('INSTANCE_NAME') == 'uvlc' && $currentUserModel->isAgencyAdmin())) && $recordModel->get('status') == 'Active') {
            $recordModel = $this->getRecord();

            if ($currentUserModel->isParentVanLineUser() == true && $currentUserModel->get('id') != $recordId) {
            ...
            if ($currentUserModel->isParentVanLineUser() && $currentUserModel->get('id') != $recordId || $currentUserModel->isAdminUser() && $currentUserModel->get('id') != $recordId) {
        ...
        } elseif($currentUserModel->isAdminUser()){
            $detailViewLink = ['linktype' => 'DETAILVIEWBASIC',
                                 'linklabel' => 'LBL_EDIT',
                                 'linkurl' => $recordModel->getEditViewUrl(),
                                 'linkicon' => ''];
