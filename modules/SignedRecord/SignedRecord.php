...
class SignedRecord extends Vtiger_CRMEntity
{
    ...
    function vtlib_handler($moduleName, $eventType)
    {
        if ($eventType == 'module.postinstall') {
            ...
            $this->changeFieldNames($moduleName);
...
//            $this->changeFieldNames($moduleName);
     ...
     * @throws Exception
     */
    public function createCustomFields($moduleName)
    {
        $initData = array(
     ...
     * @param string $moduleName
     */
    public function createDependentsList($moduleName)
    {
     ...
     * @return bool
     */
    public function isExistRelatedList($tabid, $related_tabid, $name)
    {
        global $adb;
     ...
     * @param string $moduleName
     */
    public function updateWsEntity($moduleName)
    {
        global $adb;
        $rs = $adb->pquery("SELECT * FROM `vtiger_ws_entity` WHERE `name` = ?", array($moduleName));
        if ($adb->num_rows($rs) == 0) {
     ...
     * @param $moduleName
     */
    public function convertToRelatedFields($moduleName) {
        global $adb;
     ...
     * @param $moduleName
     */
    public function changeFieldNames($moduleName) {
        // rmeove signedrecord_status
        global $adb;
        $rs = $adb->pquery("SELECT fieldid FROM vtiger_field WHERE tablename LIKE 'vtiger_signedrecord' AND columnname LIKE 'signedrecord_status' 
                            AND fieldname LIKE 'signedrecord_status'");
        if ($adb->num_rows($rs) > 0) {
            $adb->pquery("DELETE FROM vtiger_field WHERE tablename LIKE 'vtiger_signedrecord' AND columnname LIKE 'signedrecord_status' 
                            AND fieldname LIKE 'signedrecord_status'");
        }

        // Change status field to signedrecord_status field name & status uitype
        $adb->pquery("UPDATE vtiger_field
                          SET fieldname = 'signedrecord_status', uitype = 1
                            WHERE tablename LIKE 'vtiger_signedrecord' AND columnname LIKE 'status' AND fieldname LIKE 'status'");
    }
