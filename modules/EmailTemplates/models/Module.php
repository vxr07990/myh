...
class EmailTemplates_Module_Model extends Vtiger_Module_Model
{
    ...
    public function saveRecord($recordModel)
    {
        ...
        if (empty($templateid)) {
            ...
            $sql = "INSERT INTO vtiger_emailtemplates(templatename, subject, description, body, deleted, owner_id, templateid) VALUES (?,?,?,?,?,?,?)";
        } else {
            $sql = "UPDATE vtiger_emailtemplates SET templatename=?, subject=?, description=?, body=?, deleted=?/*, owner_id = ? */ WHERE templateid = ?";
        }
        $params = array(decode_html($recordModel->get('templatename')), decode_html($recordModel->get('subject')),
                decode_html($recordModel->get('description')), $recordModel->get('body'), 0, $recordModel->get('owner_id'), $templateid);
    ...
    public function getAllModuleEmailTemplateFields()
    {
        ...
        if(!in_array('Opportunities',$allModuleList))
        {
            $allModuleList[] = 'Opportunities';
        }
        $allRelFields = array();
