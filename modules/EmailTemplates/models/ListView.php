...
class EmailTemplates_ListView_Model extends Vtiger_ListView_Model
{
    ...
    public function getListViewEntries($pagingModel)
    {
        ...
        $agents = $current_user->getBothAccessibleOwnersIdsForUser();
        ...
        $searchKey   = $db->sql_escape_string($this->get('search_key'));
        $searchValue = $db->sql_escape_string($this->get('search_value'));

        $listQuery .= " JOIN `vtiger_users` ON owner_id=`vtiger_users`.id WHERE `vtiger_emailtemplates`.deleted=0 AND (owner_id=1";
        if(count($agents)) {
            $listQuery .= " OR agent_ids REGEXP '[[:<:]](";
        ...
        for ($i = 0; $i < $num_rows; $i++) {
            ...
            //file_put_contents('logs/devLog.log', "\n SUBJECT: ".$row['subject'], FILE_APPEND);
            ...
            $row['subject'] = str_replace('$software_name$', $softwareName, $row['subject']);
    ...
    public function getListViewCount()
    {
        ...
        $agents = $user->getBothAccessibleOwnersIdsForUser();
