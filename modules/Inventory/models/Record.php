...
class Inventory_Record_Model extends Vtiger_Record_Model
{
    ...
    public function getProducts()
    {
        ...
        for ($i=1;$i<=$productsCount; $i++) {
            ...
            //file_put_contents('logs/devLog.log', "\n pre number_format netPrice : ".print_r($relatedProducts[$i]['netPrice'.$i], true), FILE_APPEND);
            $relatedProducts[$i]['netPrice'.$i] = number_format($relatedProducts[$i]['netPrice'.$i], $numOfCurrencyDecimalPlaces, '.', '');
            //file_put_contents('logs/devLog.log', "\n post number_format netPrice : ".print_r($relatedProducts[$i]['netPrice'.$i], true), FILE_APPEND);
        }
    ...
    {
        return parent::setParentRecordData($parentRecordModel);
        ...
        if ($data['account_id'] != '0') {
            ...
            //file_put_contents('logs/BillingMapTest.log', date('Y-m-d H:i:s - ').$sql."\n".print_r($params, true)."\n", FILE_APPEND);
            //file_put_contents('logs/BillingMapTest.log', date('Y-m-d H:i:s - ').print_r($data, true)."\n", FILE_APPEND);
        } /*else {
            $db = PearDatabase::getInstance();
