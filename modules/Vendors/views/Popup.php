...
class Vendors_Popup_View extends Vtiger_Popup_View
{
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer)
    {
        ...
        if (!empty($relatedParentModule) && !empty($relatedParentId)) {
            ...
            $listViewModel = Vendors_ListView_Model::getInstanceForPopup($moduleName, $cvId);
        ...
        // End
        if ($sourceModule == 'Claims' || $sourceModule == 'ClaimItems') {
            ...
            $this->listViewEntries = $this->getApplicableEmployees($orderId);
        }
    ...
    public function getApplicableEmployees($sourceRecord)
    {
        ...
        if ($result) {
            while ($row = & $result->fetchRow()) {
                $employees[] = Vtiger_Record_Model::getInstanceById($row[0]);
            }
        }
