...
class Emails extends CRMEntity
{
    ...
    {
        global $adb;
        $linked = $_REQUEST['linked_ids'];
        if($linked && count($linked) > 0) {
            $recordModel = Vtiger_Record_Model::getInstanceById($this->id, 'Emails');
            foreach($linked as $relatedID) {
                $relatedRecordModel = Vtiger_Record_Model::getInstanceById($relatedID);
                $this->UpdateRelation($this->id, 'Emails', $relatedID, $relatedRecordModel->getModuleName());
            }
        }
