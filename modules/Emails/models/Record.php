...
class Emails_Record_Model extends Vtiger_Record_Model
{
    ...
    public function save()
    {
        ...
        if ($this->get('email_flag')!="MailManager") {
            $v = DateTimeField::convertToUserTimeZone(date('Y-m-d H:i'));
            $this->set('date_start', $v->format('Y-m-d'));
            $this->set('time_start', $v->format('H:i'));
        }
    ...
    public function send()
    {
       ...
       //$mailer->Host = $this->getUserSMTPHost($currentUserModel->getId());
        //$mailer->Username = $this->getUserSMTPUsername($currentUserModel->getId());
        //$mailer->Password = $this->getUserSMTPPassword($currentUserModel->getId());
        //$fromEmail = $this->getFromEmailAddress();
       list ($fromEmail, $mailerHost, $mailerUsername, $mailerPassword) = $this->getUserSMTPInfo($currentUserModel->getId());
        if ($mailerHost) {
        ...
        foreach ($toEmailInfo as $id => $emails) {
            ...
            //file_put_contents('logs/devLog.log', "\n this: ".print_r($this, true), FILE_APPEND);
            ...
            if (strpos($description, '<img src="test/logo/MoveCRM.png" title="MoveCRM.png" alt="MoveCRM.png" />')) {
                $description = str_replace('<img src="test/logo/MoveCRM.png" title="MoveCRM.png" alt="MoveCRM.png" />', "<img src='cid:MoveCRM' />", $description);
                $moveCRMlogo = true;
            }

            if (strpos($description, '<img src="test/logo/MoveHQ.png" title="MoveHQ.png" alt="MoveHQ.png" />')) {
                $description = str_replace('<img src="test/logo/MoveHQ.png" title="MoveHQ.png" alt="MoveHQ.png" />', "<img src='cid:MoveHQ' />", $description);
                $moveHQlogo = true;
            }
            ...
            foreach ($emails as $email) {
                ...
                //file_put_contents('logs/devLog.log', "\n attachments: ".print_r($attachments, true), FILE_APPEND);

                //file_put_contents('logs/devLog.log', "\n desc: ".$description, FILE_APPEND);
                ...
                if ($moveCRMlogo) {
                    //file_put_contents('logs/devLog.log', "\n MOVECRM LOGO", FILE_APPEND);
                    //While sending email template and which has '$logo$' then it should replace with company logo
                    $mailer->AddEmbeddedImage(dirname(__FILE__).'/../../../test/logo/MoveCRM.png', 'MoveCRM', 'MoveCRM.png', 'base64', 'image/png');
                }

                if ($moveHQlogo) {
                    //file_put_contents('logs/devLog.log', "\n MOVEHQ LOGO", FILE_APPEND);
                    //While sending email template and which has '$logo$' then it should replace with company logo
                    $mailer->AddEmbeddedImage(dirname(__FILE__).'/../../../test/logo/MoveHQ.png', 'MoveHQ', 'MoveHQ.png', 'base64', 'image/png');
                }
            ...
            if (!$status) {
                ...
                if (!empty($folderName) && !empty($mailString)) {
                   ...
                   // file_put_contents('logs/devLog.log', "\n MAIL STRING: $mailString", FILE_APPEND);
    ...
    public function getFromEmailAddress()
    {
        ...
        $fromEmail = false;
        $result = $db->pquery('SELECT user_smtp_fromemail FROM vtiger_users WHERE id=?', array($current_user->id));
        if ($db->num_rows($result)) {
            $fromEmail = decode_html($db->query_result($result, 0, 'user_smtp_fromemail'));
        } else {
    ...
    public function getUserSMTPInfo($id) {
        $db = PearDatabase::getInstance();
        $stmt   = 'SELECT
                    user_server,
                    user_smtp_username,
        ...
        $result = $db->pquery($stmt, [$id]);
        if ($db->num_rows($result)) {
            $fromEmail = decode_html($db->query_result($result, 0, 'user_smtp_fromemail'));
            $host      = decode_html($db->query_result($result, 0, 'user_server'));
            $username  = decode_html($db->query_result($result, 0, 'user_smtp_username'));
            $password  = decode_html($db->query_result($result, 0, 'user_smtp_password'));
            //decode_html($db->query_result($result, 0, 'user_smtp_authentication'));
        }

        return [$fromEmail, $host, $username, $password];
    }
    /**
