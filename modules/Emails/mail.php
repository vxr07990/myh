...
function send_mail($module, $to_email, $from_name, $from_email, $subject, $contents, $cc='', $bcc='', $attachment='', $emailid='', $logo='', $useGivenFromEmailAddress=false)
{
    ...
    $mail = new PHPMailer();
    if (getenv('LOG_EMAIL_SENDS')) {
        //set indepth logging for PHPMailer
        $mail->SMTPDebug = 2;
        if ($tLogFile = getenv('LOG_EMAIL_FAILS_FILE')) {
            $mail->Debugoutput = 'file';
            $mail->DebugoutputFile = $tLogFile;
        }
    }
    ...
    if (empty($mail->Host)) {
        if (getenv('LOG_EMAIL_SENDS')) {
            if ($tLogFile = getenv('LOG_EMAIL_FAILS_FILE')) {
                file_put_contents($tLogFile, date('c')." : (".__FILE__.":".__LINE__.") MAIL FAIL : MISSING HOST: ".print_r($mail->Host, true)."\n", FILE_APPEND);
            }
        }
        return 0;
    }
    ...
    if ($mail_status != 1) {
        ...
        if (getenv('LOG_EMAIL_SENDS')) {
            if ($tLogFile = getenv('LOG_EMAIL_FAILS_FILE')) {
                file_put_contents($tLogFile, date('c')." : (".__FILE__.":".__LINE__.") MAIL FAIL1 : ".print_r($mail->ErrorInfo, true)."\n", FILE_APPEND);
                file_put_contents($tLogFile, date('c')." : (".__FILE__.":".__LINE__.") MAIL FAIL2 : ".print_r($mail_error, true)."\n", FILE_APPEND);
            }
        }
    } else {
        $mail_error = $mail_status;
        if (getenv('LOG_EMAIL_SENDS')) {
            if ($tLogFile = getenv('LOG_EMAIL_SENDS_FILE')) {
                file_put_contents($tLogFile,
                                  date('c')." : (".__FILE__.":".__LINE__.") MAIL SUCCESS : " . $mail_error . "\n#########################\n"
                                  .print_r($mail->GetSentMIMEMessage(), true)
                                  ."\n#########################\n",
                                  FILE_APPEND);
            }
        }
