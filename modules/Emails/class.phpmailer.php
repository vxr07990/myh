...
class PHPMailer
{
   ...
   * Right now we only honor "echo" or "error_log" or "file"
   * @var string
   */
  public $Debugoutput     = "echo";

  /**
   * Sets the write file for when Debugoutput is set to file
   * @var string
   */
  public $DebugoutputFile = '';
      ...
      switch ($this->Debugoutput) {
          ...
          case 'file':
              if ($this->DebugoutputFile) {
                  file_put_contents($this->DebugoutputFile, date('c')." : ".$str, FILE_APPEND);
                  break;
              }
          case 'html':
  ...
  public function Send()
  {
      try {
          if (!$this->PreSend()) {
              if ($this->SMTPDebug) {
                  $this->edebug("class.phpmailer.php:" . __LINE__ . ") Failed PreSend function\n");
              }
              return false;
          }
          $rv = $this->PostSend();
          if ($this->SMTPDebug) {
              $this->edebug("class.phpmailer.php:" . __LINE__ . ") PostSend function response: (".print_r($rv, true).")\n");
          }
          return $rv;
          ...
          if ($this->SMTPDebug) {
              $this->edebug("class.phpmailer.php:" . __LINE__ . ") Send function catch: (".print_r($e->getMessage(), true).")\n");
          }
          if ($this->exceptions) {
              throw $e;
          }
  ...
  protected function SmtpSend($header, $body)
  {
      ...
      if (!$this->SmtpConnect()) {
          if ($this->SMTPDebug) {
              $this->edebug("class.phpmailer.php:".__LINE__.") SmtpSend: (".$this->Lang('smtp_connect_failed')."\n");
          }
      ...
      if (!$this->smtp->Mail($smtp_from)) {
          if ($this->SMTPDebug) {
              $this->edebug("class.phpmailer.php:".__LINE__.") SmtpSend: (".$this->Lang('from_failed').$smtp_from.' : '.implode(',', $this->smtp->getError())."\n");
          }
    ...
    foreach ($this->to as $to) {
      ...
      if (!$this->smtp->Data($header . $body)) {
          if ($this->SMTPDebug) {
              $this->edebug("class.phpmailer.php:".__LINE__.") SmtpSend: (".$this->Lang('data_not_accepted')."\n");
          }
      ...
      if ($this->SMTPDebug) {
          $this->edebug("class.phpmailer.php:".__LINE__.") SmtpSend: RETURN TRUE\n");
      }
      return true;
  }
  ...
  public function SmtpConnect($options = array())
  {
    ...
    if ($this->smtp->Connected()) {
      ...
      $this->smtp->DebugoutputFile = $this->DebugoutputFile;
      ...
      if ($this->SMTPDebug) {
          $this->edebug("class.phpmailer.php:".__LINE__.") SmtpConnect this: (".print_r($this, true).")\n");
      }
  ...
  */
  ...
  public function CreateHeader()
  {
    ...
    if ($this->Mailer != 'mail') {
      ...
      if(!$currentUser->hasCustomSMTP())
      {
          if(getenv('SUPPORT_EMAIL_ADDRESS') != '')
          {
              $sender = getenv('SUPPORT_EMAIL_ADDRESS');
              if(getenv('SUPPORT_EMAIL_NAME') != '')
              {
                  $this->SetFrom($sender, getenv('SUPPORT_EMAIL_NAME'), FALSE);
              }
              else
              {
                  $this->SetFrom($sender, 'Support', FALSE);
              }
          }
          else
          {
              $sender = "support@igcsoftware.com";
              $this->SetFrom($sender, 'Support', FALSE);
          }
      }
  ...
  */
  ...
  protected function SetError($msg)
  {
      ...
      if ($this->SMTPDebug) {
          $this->edebug("class.phpmailer.php:".__LINE__.") SetError msg: (".print_r($msg, true).")\n");
      }
      $this->ErrorInfo = $msg;
  }
