...
class AgentManager_Popup_View extends Vtiger_Popup_View {
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
        ...
        if (!empty($relatedParentModule) && !empty($relatedParentId)) {
            ...
            $this->listViewEntries = $this->removeSelfAndChildOfSrcRecord($listViewEntries,$sourceRecord);
            // đây là chỗ cần sửa $this->listViewEntries
...
//            $this->listViewEntries = $this->removeSelfAndChildOfSrcRecord($listViewEntries,$sourceRecord);
    ...
    function removeSelfAndChildOfSrcRecord($listViewEntries,$sourceRecord){

        foreach ($listViewEntries as $key => $objViewEntry){
            $recordModel = Agents_Record_Model::getInstanceById($objViewEntry->getId());
            $recordId = $recordModel->getId();
            $recordParentId = $recordModel->get('cf_agent_manager_parent_id');

            if ($recordId == $sourceRecord){
                unset($listViewEntries[$key]);
            }

            if (!empty($recordParentId) && $recordParentId == $sourceRecord){
                $listViewEntries = $this->removeSelfAndChildOfSrcRecord($listViewEntries,$recordId);
            }
        }
