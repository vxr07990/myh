...
class Opportunities_ComposeEmail_View extends Vtiger_Footer_View
{
    ...
    public function composeMailData($request)
    {
        ...
        $selectedFields  = $request->get('selectedFields');//print_r($request);
                ...
                if ($selectedFields) {
                    foreach ($selectedFields as $field) {
                        ...
                        if (!empty($value) && (!$emailOptOutValue)) {
                            $to[]                   = $value;
                            $toMailInfo[$id][]      = $value;
                            $toMailNamesList[$id][] = ['label' => $recordModel->getName(), 'value' => $value];
                        }
                    }
                }
            }
        ...
        $oppRecordModel = Vtiger_Record_Model::getInstanceById($selectedIds, 'Opportunities');
        $oppContactId = $oppRecordModel->get('contact_id');
        $contactRecordModel = Vtiger_Record_Model::getInstanceById($oppContactId, 'Contacts');
        $oppContactEmail = $contactRecordModel->get('email');
        $oppContactName = getEntityName('Contacts', $oppContactId);
        $oppContactName = $oppContactName[$oppContactId];

        if (!empty($oppContactEmail)) {
            $to[]                             = $oppContactEmail;
            $toMailInfo[$oppContactId][]      = $oppContactEmail;
            $toMailNamesList[$oppContactId][] = ['label' => $oppContactName, 'value' => $oppContactEmail];
        }
        ...
        try {
            ...
            if (isset($editEmail)&&!empty($editEmail)) {
                foreach ($editEmail as $email) {
                    $emailRecordModel = Vtiger_Record_Model::getInstanceById($email, 'Emails');
                    $emailResult = $db->pquery("SELECT description, label FROM vtiger_crmentity WHERE crmid = ?", [$email]);
                    if (isset($emailResult)) {
                        $row = $emailResult->fetchRow();
                        if (!empty($row)) {
                            $viewer->assign('SUBJECT', $row[1]);
                            $viewer->assign('DESCRIPTION', $row[0]);
                        }
                    }
                }
            }
        } catch (Exception $e) {
            file_put_contents('logs/devLog.log', "\n IO HERE Code failed to run \n" . $e->getMessage(), FILE_APPEND);
        }
    ...
    public function emailPreview($request)
    {
        ...
        $basicActionLink = array(
            ...
            'linkurl' => Vtiger_Util_Helper::toSafeHTML("javascript:Vtiger_Detail_Js.triggerSendEmail('".$object_param."','Emails');"),
            'linkicon' => ''
        );

        $viewer->assign('FORWARD_EMAIL', $basicActionLink['linkurl']);
        ...
        foreach ($parentIds as $index => $parentFieldId) {
            ...
            foreach ($emailFields as $emailField) {
                ...
                if (in_array($emailValue, $TO)) {
                    ...
                    $toMailNamesList[$parentId][] = ['label' => $parentRecordModel->getName(), 'value' => $emailValue];
                }
            }
        }
        $viewer->assign('TO', $TO);
