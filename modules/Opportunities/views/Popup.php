...
class Opportunities_Popup_View extends Vtiger_Popup_View
{
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer)
    {
        ...
        /**
         * For sirva need to filter out the opportunities that can be related togheter
         */

        if (getenv('INSTANCE_NAME') == 'sirva' && $request->get('src_module') == 'Opportunities' && !$searchValue) {
            $srcRecordModel = Vtiger_Record_Model::getInstanceById($sourceRecord, 'Opportunities');
            $viewer->assign('POPUP_MODE', 'LeadMatching');

            $contactId = $srcRecordModel->get('contact_id');
            $contactModel = Vtiger_Record_Model::getInstanceById($contactId, 'Contacts');

            // TODO: Figure out why this got commented out.
	        $_REQUEST['search_value'] =  $contactModel->get('phone');

            $viewer->assign('CONTACT_PHONE', $contactModel->get('phone'));
        }
        ...
        if(method_exists($listViewModel, 'getModule')) {
            ...
            $transformedSearchParams = Vtiger_Util_Helper::transferListSearchParamsToFilterCondition($searchParmams, $listViewModel->getRelatedModuleModel());
        }
        $listViewModel->set('search_params', $transformedSearchParams);
        ...
        //Prevent the same record to be related to itself
        unset($this->listViewEntries[$sourceRecord]);
