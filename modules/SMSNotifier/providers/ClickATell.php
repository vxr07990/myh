...
class SMSNotifier_ClickATell_Provider implements SMSNotifier_ISMSProvider_Model
{
    ...
    public function send($message, $toNumbers)
    {
        ...
        //update each number to have a preceeding 1
        foreach ($toNumbers as $key => $toNumber) {
            $match = preg_match('/^1/', $toNumber);
            //if there is no preceeding one add a 1.
            //False means the regexp errored, don't do anything because something is probably broke and needs found.
            if (!$match && $match !== false) {
                $toNumbers[$key] = 1 . $toNumber;
            }
        }
