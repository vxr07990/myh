...
class HelpDesk_Module_Model extends Vtiger_Module_Model
{
    ...
    public function getTicketsByStatus($owner, $dateFilter)
    {
		...
		$sql = 'SELECT COUNT(*) as count, CASE WHEN vtiger_troubletickets.status IS NULL OR vtiger_troubletickets.status = "" THEN "" ELSE vtiger_troubletickets.status END AS statusvalue 
			FROM vtiger_troubletickets INNER JOIN vtiger_crmentity ON vtiger_troubletickets.ticketid = vtiger_crmentity.crmid ' .
			' INNER JOIN vtiger_ticketstatus ON vtiger_troubletickets.status = vtiger_ticketstatus.ticketstatus '.
			' WHERE vtiger_crmentity.deleted = 0 '.Users_Privileges_Model::getNonAdminAccessControlQuery($this->getName()). $ownerSql .' '.$dateFilterSql;
		if(!$currentUser->isAdminUser()){
			$sql .= ' AND vtiger_crmentity.agentid IN ( '. generateQuestionMarks($accesibleAgents).' )';
			$params = array_merge($params, $accesibleAgents);
		}
		$sql .= 'GROUP BY statusvalue ORDER BY vtiger_ticketstatus.sortorderid';
