...
class ModComments_Record_Model extends Vtiger_Record_Model
{
    ...
    public static function getRecentComments($parentRecordId, $pagingModel)
    {
        ...
        $queryGenerator->setFields(array('parent_comments', 'createdtime', 'modifiedtime', 'related_to',
                                    'assigned_user_id', 'commentcontent', 'creator', 'id', 'customer', 'reasontoedit', 'userid', 'from_mailconverter', 'note_source', 'provider'));
    ...
    /**
     * Function returns all the comments attached to a record
     * @param <Integer> $record
     * @return ModComments_Record_Model(s)
     */
    public static function getAllComments($record)
    {
        $db = PearDatabase::getInstance();

        $commentsModels = [];

        $sql = "SELECT modcommentsid FROM `vtiger_modcomments` WHERE related_to=?";
        $result = $db->pquery($sql, [$record]);
        while ($row =& $result->fetchRow()) {
            $commentsModels[] = self::getInstanceById($row['modcommentsid']);
        }
    ...
    public static function getAllParentComments($parentId)
    {
        ...
        $queryGenerator->setFields(array('parent_comments', 'createdtime', 'modifiedtime', 'related_to', 'id',
            'assigned_user_id', 'commentcontent', 'creator', 'customer', 'reasontoedit', 'userid', 'note_source', 'provider', 'cf_comment_type'));
        $query = $queryGenerator->getQuery();
        ...
        if ($parentModuleName == 'Opportunities') {
            $arrRecordIds=array($parentId);
            $rsSurveys=$db->pquery("SELECT cubesheetsid FROM vtiger_cubesheets
INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_cubesheets.cubesheetsid
WHERE deleted=0 AND is_primary = 1 AND potential_id=?", array($parentId));
            if ($db->num_rows($rsSurveys)>0) {
                while ($rowSurvey = $db->fetch_array($rsSurveys)) {
                    $arrRecordIds[] = $rowSurvey['cubesheetsid'];
                }
            }
        
            // Get linked Estimates
            $rsEstimates=$db->pquery("SELECT quoteid FROM vtiger_quotes
INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_quotes.quoteid
WHERE deleted=0 AND is_primary = 1 AND potentialid=?", array($parentId));
            if ($db->num_rows($rsEstimates)>0) {
                while ($rowEstimate = $db->fetch_array($rsEstimates)) {
                    $arrRecordIds[] = $rowEstimate['quoteid'];
                }
            }

            $query = $query ." AND related_to IN (".generateQuestionMarks($arrRecordIds).") AND parent_comments = '' ORDER BY vtiger_crmentity.createdtime DESC";
            $result = $db->pquery($query, array($arrRecordIds));
        } else {
    ...
    public function getChildComments()
    {
        ...
        $queryGenerator = $listView->get('query_generator');
        $queryGenerator->setFields(array('parent_comments', 'createdtime', 'modifiedtime', 'related_to', 'id',
                                            'assigned_user_id', 'commentcontent', 'creator', 'reasontoedit', 'userid', 'note_source', 'provider'));
        $query = $queryGenerator->getQuery();
