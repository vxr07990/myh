...
require_once 'libraries/MoveCrm/Traits/DateTime.php';
    ...
    public function handleEvent($eventName, $data)
    {
        ...
        if ($eventName == 'vtiger.entity.aftersave') {
            ...
            if ($relatedToId) {
                ...
                /**
                 * Update `HelpDesk` (Tickets) `modifiedtime`
                 */
                if ($moduleName == 'HelpDesk') {
                    require_once 'libraries/MoveCrm/Models/Tickets.php';

                    try {
                        $ticket = new \MoveCrm\Models\Tickets($relatedToId);
                        $ticket->updateModifiedTime();
                    } catch (\Exception $e) {
                ...
                if (!method_exists($focus, 'retrieve_entity_info')) {
                    //@TODO: Determine if we should throw an error or just return.
                    //throw new Exception('moduleName: '. $moduleName . ' does not have the method: retrieve_entity_info');
                    return;
                }
