...
class ProjectTask_LocalDispatchDayBook_View extends Vtiger_List_View
{
    protected $default_local_dispatch_view = 130; //Harcode the filter that show the local dispatch tasks
    ...
    public function getHeaderCss(Vtiger_Request $request)
    {
        ...
        $cssFileNames       = [
            '~/layouts/vlayout/modules/ProjectTask/resources/style.css',
            '~/layouts/vlayout/modules/ProjectTask/resources/dhtmlx/dhtmlxgantt.css',
        ];
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer)
    {
        ...
        if (empty($selectedDate) || $selectedDate == '') {
            $selectedDate = DateTimeField::convertToUserFormat(date('Y-m-d'));
        } else {
            $selectedDate = DateTimeField::convertToUserFormat($selectedDate);
        }
        $searchParmams[][] = [
            'startdate',
            'bw',
            $selectedDate.','.$selectedDate //user format!
        ];
