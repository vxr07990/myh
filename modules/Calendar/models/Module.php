...
class Calendar_Module_Model extends Vtiger_Module_Model
{
    ...
    public function getSideBarLinks($linkParams)
    {
        ...
        $listLinkTypes = array('LISTVIEWSIDEBARWIDGET');
        $listLinks = Vtiger_Link_Model::getAllByType($this->getId(), $listLinkTypes);
        foreach ($listLinks['LISTVIEWSIDEBARWIDGET'] as $linkObject) {
            if ($linkObject->linklabel == 'Google Calendar'
                || $linkObject->linklabel == 'LBL_CALENDAR_EXCHANGESYNC') {
                $links['SIDEBARWIDGET'][] = $linkObject;
            }
        }
        return $links;
    }
    ...
    */
    public static function getSharedUsersOfCurrentUser($id)
    {
        ...
        //file_put_contents('logs/devLog.log', "\n USER ID: ".$id, FILE_APPEND);
        
        // $currentRole = $currentUser->getRole();

        // $sql = "SELECT depth, rolename FROM vtiger_role WHERE roleid = ?";
        // $result = $db->pquery($sql, array($currentRole));
        // $row = $result->fetchRow();

        // $currentDepth = $row['depth'];
        // $roleName = $row['rolename'];

        //file_put_contents('logs/devLog.log', "\n ROLE: ".print_r($roleDepth, true), FILE_APPEND);
...
//			WHERE sharedid=? OR (vtiger_users.status='Active' AND vtiger_users.calendarsharedtype='public' AND vtiger_users.id != ?);";
        //} else if($currentUser->isVanlineUser()){
            //filterUserAccessibleUsers();
        //} else{
            //$agentCondition = getListviewAgentCondition('Calendar');
            //file_put_contents('logs/devLog.log', "\n ACON: $agentCondition", FILE_APPEND);
            /*$userVanlines = array();
            $userAgents = array();

            $sql = "SELECT DISTINCT vanlineid FROM vtiger_users2vanline WHERE userid = ?";
            $result = $db->pquery($sql, array($id));
            $row = $result->fetchRow();
            while($row != null){
                $userVanlines[] = $row['vanlineid'];
                $row = $result->fetchRow();
            }

            //file_put_contents('logs/devLog.log', "\n USER VANLINES: ".print_r($userVanlines, true), FILE_APPEND);

            $sql = "SELECT DISTINCT vtiger_user2agency.agency_code, vtiger_agentmanager.agency_name FROM vtiger_user2agency RIGHT JOIN vtiger_agentmanager ON vtiger_agentmanager.agentmanagerid = vtiger_user2agency.agency_code RIGHT JOIN vtiger_crmentity ON vtiger_user2agency.agency_code = vtiger_crmentity.crmid WHERE vtiger_crmentity.deleted = 0 AND userid = ?";
            foreach($userVanlines as $userVanline){
                $sql .= ' OR vtiger_agentmanager.vanline_id = '.$userVanline;
            }
            ...
            while($row != null){
                ...
                $userAgents[] = $row['agency_code'];
                $row = $result->fetchRow();
            }

            //file_put_contents('logs/devLog.log', "\n USER AGENTS: ".print_r($userAgents, true), FILE_APPEND);

            $inequality = '>=';

            //if sales person don't show other sales person roles
            if($currentDepth == 9 && strpos($roleName, ' Sales Person')){
                $inequality = '>';
                //file_put_contents('logs/devLog.log', "\n INEQUALITY CHANGED TO: >", FILE_APPEND);
            }

            $query = "SELECT vtiger_users.first_name,vtiger_users.last_name, vtiger_users.id as userid FROM vtiger_sharedcalendar RIGHT JOIN vtiger_users ON vtiger_sharedcalendar.userid=vtiger_users.id AND status= 'Active'
            LEFT JOIN vtiger_user2agency ON vtiger_user2agency.userid=vtiger_users.id
            LEFT JOIN vtiger_user2role ON vtiger_user2role.userid=vtiger_users.id
            LEFT JOIN vtiger_users2vanline ON vtiger_users.id=vtiger_users2vanline.userid LEFT JOIN vtiger_vanlinemanager ON vtiger_users2vanline.vanlineid=vtiger_vanlinemanager.vanlinemanagerid
            LEFT JOIN vtiger_agentmanager ON vtiger_agentmanager.agentmanagerid=vtiger_user2agency.agency_code OR vtiger_vanlinemanager.vanlinemanagerid=vtiger_agentmanager.vanline_id
            LEFT JOIN vtiger_role ON vtiger_user2role.roleid=vtiger_role.roleid
            WHERE sharedid=? OR (vtiger_users.status='Active' AND vtiger_users.calendarsharedtype='public' AND vtiger_users.id != ? AND vtiger_role.depth ".$inequality." ?";
            ...
            foreach($userAgents as $index => $userAgent){
                ...
                $query .= 'vtiger_agentmanager.agentmanagerid = '.$userAgent;
                if($index == count($userAgents)-1){
                    $query .= ')';
                }
            }
            $query .= ');';*/
        ...
        if (!$currentUser->isAdminUser()) {
            $userIds = filterUserAccessibleUsers($userIds);
        }
        
        //file_put_contents('logs/devLog.log', "\n USER IDS: ".print_r($userIds, true), FILE_APPEND);
    ...
    */
    public static function getCalendarViewTypes($id)
    {
        ...
        for ($i=0; $i<$rows; $i++) {
            ...
            if ($activityTypes['module'] == 'OASurveyRequests') {
                $currentUser = Users_Record_Model::getCurrentUserModel();
                $depth = getRoleDepth($currentUser->get('roleid'));
                if ($depth != 6) {
                    continue;
                }
            }
    ...
    {
        parent::deleteRecord($recordModel);
        vimport('~~/modules/Calendar/RepeatEvents.php');
        Calendar_RepeatEvents::removeOccurrences($recordModel->getId());
    }
    
    //$statusType accepted -> 'event' or 'task'
    public static function getStatusValues($statusType = false)
    {
        if(!$statusType){
            return [];
        }
        ...
        $result = $adb->pquery("SELECT * FROM vtiger_".$statusType."status", array());
        
        while ($row = $adb->fetch_array($result)) {
            $values[$row[$statusType.'statusid']] = $row[$statusType.'status'];
        }
