...
/**
     ...
     * @return bool
     */
    public function isOccurrence()
    {
        $db = PearDatabase::getInstance();
        $sql = "SELECT * FROM `vtiger_recurrencerel` WHERE `activityid`=?";
    ...
    public function getRecurrenceRoot()
    {
        ...
        $db = PearDatabase::getInstance();
        $sql = "SELECT parentid FROM `vtiger_recurrencerel` WHERE activityid=?";
        $result = $db->pquery($sql, [$this->getId()]);
        $row = $result->fetchRow();
        return Vtiger_Record_Model::getInstanceById($row['parentid']);
    }
}
