...
class Calendar_CalendarUserActions_Action extends Vtiger_Action_Controller
{
    public function __construct()
    {
        ...
        $this->exposeMethod('saveStatusForCalendar');
    }
    ...
    public function saveStatusForCalendar(Vtiger_Request $request)
    {
        ...
        $activityArray = [
            'id' => vtws_getWebserviceEntityId($module, $recordId),
            $fieldName => $newStatus
        ];
        try {
            $update = vtws_revise($activityArray, $user);
        } catch (Exception $exc) {
            global $log;
            $log->debug('Error updating Account:' . $exc->getMessage());
        }
        $result = (isset($update) ? array('success' => true) : array('success' => false));
        $response = new Vtiger_Response();
        $response->setResult($result);
        $response->emit();
    }
