...
class ListviewColors_Uninstall_View extends Settings_Vtiger_Index_View {

    function process (Vtiger_Request $request) {
        global $adb;
        echo '<div class="container-fluid">
                <div class="widget_header row-fluid">
                    <h3>VTE List View Colors</h3>
                </div>
                <hr>';
        // Uninstall module
        $module = Vtiger_Module::getInstance('ListviewColors');
    ...
    function removeData(){
        ...
        $adb->pquery("DELETE FROM vtiger_settings_field WHERE `name` = ?",array('VTE List View Colors'));
        // drop tables
        $sql = "DROP TABLE `vte_listview_colors`;";
        $result = $adb->pquery($sql,array());

        $message.= "&nbsp;&nbsp;- Delete VTE List View Colors tables";
