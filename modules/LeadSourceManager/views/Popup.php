...
class LeadSourceManager_Popup_View extends Vtiger_Popup_View
{
    protected $listViewEntries = false;
    protected $listViewHeaders = false;
    //the regexp to limit the source_types for role >= 4
    protected $AgentLimitRegexp = '/^agent/i';
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer)
    {
        ...
        //because the default operator behavior is 'c' (LIKE) and I want it to be 'e' =
        $defaultOperator = 'e';
        ...
        if ((!empty($searchKey)) && (!empty($searchValue))) {
            ...
            $listViewModel->set('operator', $defaultOperator);
        }
        if (!empty($relatedParentModule) && !empty($relatedParentId)) {
        ...
        //dump the inactives.
        $this->pareInactives();
        //parse the list by the rule that sales manager or lower only gets "Agent.*" source_types
        if($moduleName != 'LeadSourceManager') {
          $this->pareByRoleDepth();
        }
    ...
    private function pareInactives()
    {
        $temp = [];

        foreach ($this->listViewEntries as $entry) {
            if ($entry->get('active') == 'yes') {
                $temp[] = $entry;
            }
        }
        $this->listViewEntries = $temp;
    }

    private function pareByRoleDepth()
    {
        $temp = [];

        $current_user = Users_Record_Model::getCurrentUserModel();
        $roleDepth = $current_user->getUserRoleDepth();
        foreach ($this->listViewEntries as $entry) {
            if ($roleDepth >= 4) {
                $testType = preg_match($this->AgentLimitRegexp, $entry->get('source_type'));
                if ($testType || $testType === false) {
                    $temp[] = $entry;
                }
            } else {
                $temp[] = $entry;
