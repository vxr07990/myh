...
class Documents_List_View extends Vtiger_List_View
{
    ...
    public function preProcess(Vtiger_Request $request)
    {
        ...
        $fieldModel = Vtiger_Field_Model::getInstance('foldername', $documentModuleModel);
        if($fieldModel){
            $folderList =  $fieldModel->getPicklistValues();
        }
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer)
    {
        ...
        if (!$this->listViewEntries) {
            ...
            foreach ($entries as $entry) {
                $id = $entry->get('id');
                $value = [];
                $sql = "SELECT label,setype FROM `vtiger_senotesrel` JOIN `vtiger_crmentity` ON `vtiger_senotesrel`.crmid=`vtiger_crmentity`.crmid WHERE notesid=?";
                $result = $db->pquery($sql, [$id]);
                while ($row = $result->fetchRow()) {
                    $value[$row['setype']][] = $row['label'];
                }
                $rVal = '';
                foreach($value as $type => $labels)
                {
                    if($rVal != '')
                    {
                        $rVal .= ' -- ';
                    }
                    $rVal .= $type . ': ' . implode(',', $labels);
                }
                $entry->set('Related To', $rVal);
            }
