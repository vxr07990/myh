...
class Documents extends CRMEntity
{
    ...
    // OT 16414 - Removed 'notes_title' from mandatory fields
    public $mandatory_fields = array('createdtime' ,'modifiedtime','filename','filesize','filetype','filedownloadcount','assigned_user_id' );
    ...
    //we want ot override the file step so we can send to the invoice API
    public function saveentity($module, $fileid = '')
    {
        ...
        if (strtolower(getenv('INSTANCE_NAME')) == 'graebel') {
            //@TODO: fix to set filestatus=1 when the document is created, because filestatus does in fact matter.
            $recordID = $fieldList['record'];
            if ($recordID) {
                if (!$this->table_name) {
                    $this->table_name = 'vtiger_notes';
                }
                if (!$this->table_index) {
                    $this->table_index = 'notesid';
                }
                $stmt = 'UPDATE `'.$this->table_name.'` SET `filestatus` = 1 WHERE `'.$this->table_index.'` = ? LIMIT 1';
                $db   = PearDatabase::getInstance();
                $db->pquery($stmt, [$recordID]);
            }
        }

        //This is turned off until we finish Invoice API.
        if (false && strtolower(getenv('INSTANCE_NAME')) == 'graebel' && getenv('GVL_API_ON')) {
            //Check to see if any of the related records are an Order
            $this->checkForOrderParent($fieldList);
            try {
                if (isset($this->orderParentId) && $this->orderParentId != '') {
                    $orderNumber = $this->orderParentId;
                } elseif ($fieldList['sourceRecord'] && $fieldList['sourceModule'] == 'Orders') {
                    $orderNumber = $fieldList['sourceRecord'];
                }
                if ($fieldList['invoice_pkg_format'] && $orderNumber) {
                    /*
                    //This is the safest way to check all uploaded documents whether they need to be sent to API.
                    //might be excessive.
                    $invoiceAPI      = new MoveCrm\GraebelAPI\invoiceHandler(['orderNumber' => $this->parentid]);
                    $setPkgResp      = $invoiceAPI->setPackage($fieldList['invoice_pkg_format']);
                    /* actually the createInvoice should be able to pull this document from all the others.
                    //add this current document to the list to send.
                    //invoice api will be responsible for pull doc/url etc.
                    //@TODO addDocument requires a document record model
                    $addDocument = $invoiceAPI->addDocument($thisRecordModel);
                    */
                    $invoiceResponse = MoveCrm\GraebelAPI\invoiceHandler::triggerInvoiceAPI(MoveCrm\GraebelAPI\invoiceHandler::$TRIGGER_NEW_DOC,
                                                                                            [
                                                                                                'orderNumber'        => $orderNumber,
                                                                                                'invoice_pkg_format' => $fieldList['invoice_pkg_format'],
                                                                                                'document'           => $fieldList['invoice_document_type'],
                                                                                                'transactionMode'    => 'insert'
                                                                                            ]
                    );
                } else {
                    //Either we don't have an order number or we don't have a format,
                    //so it makes no sense to send to the API handler.
                }
            } catch (Exception $ex) {
...
//	Function to get fieldname for uitype 28 assuming that doc has only one file upload type
    ...
    public function checkForOrderParent($fieldList)
    {
        $db = PearDatabase::getInstance();
        $sql = "SELECT crmid FROM `vtiger_senotesrel` JOIN `vtiger_crmentity` USING (crmid) WHERE notesid=? AND setype='Orders'";

        $result = $db->pquery($sql, $fieldList['record']);
        $row = $result->fetchRow();
        if ($row) {
            $this->orderParentId = $row['crmid'];
        }
    }

    public function retrieve($id)
    {
        $outArray                 = parent::retrieve($id);
        $recordModel              = Vtiger_Record_Model::getInstanceById($id);

        if (getenv('INCLUDE_DOCUMENT_DATA')) {
            $outArray['download_uri'] = $recordModel->getDownloadFileURL();
            list ($outArray['base64_file_data']) = $recordModel->retrieveDocumentContents();
            $outArray['base64_file_data'] = base64_encode($outArray['base64_file_data']);
        }
