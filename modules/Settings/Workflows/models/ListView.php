...
class Settings_Workflows_ListView_Model extends Settings_Vtiger_ListView_Model
{
    ...
    public function getListViewEntries($pagingModel)
    {
        ...
        //Only get records has match with agency of current user
        $params = [];
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $arrAgentids = array_keys($currentUser->getAccessibleAgentsForUser());
        if(!empty($arrAgentids)){
            $agentConditions = array();
            foreach ($arrAgentids as $agentid){
                $agentConditions[] = "CONCAT(' |##| ',agents,' |##| ') like '% |##| ".$agentid." |##| %'";
            }
            $listQuery .= "AND (".implode(' OR ',$agentConditions).")";
        }

        //Hidding system workflows from agency admin user
//        if ($currentUser->isAgencyAdmin()) {
//            $accesibleAgents = '(' . implode(',', array_keys($currentUser->getAccessibleAgentsForUser())) . ')';
//            $listQuery .= " AND agents IN $accesibleAgents";
//        }

        $sourceModule = $this->get('sourceModule');
        if (!empty($sourceModule) && !$currentUser->isAgencyAdmin()) {
            $listQuery .= ' AND module_name = ?';
            $params[] = $sourceModule;
        } elseif (!empty($sourceModule) && $currentUser->isAgencyAdmin()) {
            $listQuery .= ' AND module_name = ?';
            $params[] = $sourceModule;
        }
