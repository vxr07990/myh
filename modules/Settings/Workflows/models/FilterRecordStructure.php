...
//                if(array_key_exists('business_line2', $fieldModelList))
//                {
//                    unset($fieldModelList['business_line']);
//                }
//                if(array_key_exists('business_line_est2', $fieldModelList))
//                {
//                    unset($fieldModelList['business_line_est']);
//                }
        ...
        // Add Opportunities fields to Documents workflow
        if ($moduleModel->getName() == 'Documents') {
            $opportunitiesModuleModel=Vtiger_Module_Model::getInstance('Opportunities');
            $opportunitiesFields=$opportunitiesModuleModel->getFields();
            foreach ($opportunitiesFields as $opportunitiesFieldName => $opportunitiesFieldModel) {
                if ($opportunitiesFieldModel->isViewable()) {
                    $name = "(potentialid : (Opportunities) $opportunitiesFieldName)";
                    $label = vtranslate($opportunitiesFieldModel->get('label'), $baseModuleModel->getName()).' : ('.vtranslate('Opportunities', 'Opportunities').') '.vtranslate($opportunitiesFieldModel->get('label'), 'Opportunities');
                    $opportunitiesFieldModel->set('workflow_columnname', $name)->set('workflow_columnlabel', $label);
                    if (!empty($recordId)) {
                        $fieldValueType = $recordModel->getFieldFilterValueType($name);
                        $fieldInfo = $fieldModel->getFieldInfo();
                        $fieldInfo['workflow_valuetype'] = $fieldValueType;
                        $opportunitiesFieldModel->setFieldInfo($fieldInfo);
                    }
                    $values[vtranslate('Opportunities', 'Opportunities')][$name] = clone $opportunitiesFieldModel;
                }
            }
        }
