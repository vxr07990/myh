...
//		}
    ...
    public function save()
    {
        ...
        if($this->get('agents')) {
            $this->set('agents', $this->removeDuplicateAgents($this->get('agents')));
        }
    ...
    public function getDependentModules()
    {
        ...
        $result = $db->pquery("SELECT fieldname, tabid, typeofdata, vtiger_ws_referencetype.type as reference_module FROM vtiger_field
								...
								INNER JOIN vtiger_ws_referencetype ON vtiger_ws_fieldtype.fieldtypeid = vtiger_ws_referencetype.fieldtypeid WHERE vtiger_ws_referencetype.type=?
							UNION
							SELECT fieldname, tabid, typeofdata, relmodule as reference_module FROM vtiger_field
								INNER JOIN vtiger_fieldmodulerel ON vtiger_field.fieldid = vtiger_fieldmodulerel.fieldid WHERE relmodule=?", array($moduleName, $moduleName));

        $noOfFields = $db->num_rows($result);

        $dependentFields = array();
        // List of modules which will not be supported by 'Create Entity' workflow task
        $filterModules = array('Invoice', 'Quotes', 'SalesOrder', 'PurchaseOrder', 'Emails', 'Calendar', 'Events', 'Accounts', 'MoveRoles', 'OASurveyRequests', 'ExtraStops', 'Project');
        ...
        for ($i = 0; $i < $noOfFields; ++$i) {
            ...
            if (in_array($tabModuleName, $filterModules) || $tabModuleName == $moduleName) {
                continue;
            }
...
//                if ($dataTypeInfo[1] == 'M' && $moduleName == $referenceModule) { // If the current reference field is mandatory
...
//        }
    ...
    public function getWfAgents()
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT agents FROM com_vtiger_workflows WHERE workflow_id=?', array($this->get('workflow_id')));

        if ($result && $db->num_rows($result) > 0) {
            return $this->removeDuplicateAgents($db->query_result($result, 0, 'agents'));
        }
    }

    public function removeDuplicateAgents($agents) {
            // Adding extra logic here to ensure that duplicate agents aren't present, because some have dups for whatever reason.
