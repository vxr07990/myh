...
class Settings_Workflows_EditTask_View extends Settings_Vtiger_Index_View
{
    public function process(Vtiger_Request $request)
    {
        ...
        //vtranslate the values so they display with what the user is used to
        if ($moduleModel && $taskObject->field_value_mapping) {
            //make sure there is a field_value_mapping because setting it to null amazingly breaks jquery.min.js
            $fieldMapping = Zend_Json::decode($taskObject->field_value_mapping);
            foreach ($fieldMapping as $key => $mappingInfo) {
                //Because it may be undesired to translate EVERYTHING for now I'm conditionalizing.
                //Lets translate everything.. conditionalize what you DON'T want translated
                //if (
                    //$mappingInfo['fieldname'] == 'business_line' ||
                    //$mappingInfo['fieldname'] == 'business_line_est'
                //) {
                    $fieldMapping[$key]['value'] = vtranslate($mappingInfo['value'], $moduleModel->getName());
                //}
            }
            $taskObject->field_value_mapping = Zend_Json::encode($fieldMapping);
        }
        ...
        // Add email field of Opportunities linked Contacts
        if ($moduleModel->getName() == 'Documents') {
            $emailFieldoptions .='<option value="$(contact_id : (Contacts) email)">Contact Name : (Contacts) Primary Email</option>
<option value="$(contact_id : (Contacts) secondaryemail)">Contact Name : (Contacts) Secondary Email</option>';
        }

        $emailFields = $recordStructureInstance->getAllEmailFields();
        foreach ($emailFields as $metaKey => $emailField) {
            //removed the comma so now the Edit3.js does the work of adding a comma.
        ...
        if (getenv('IGC_MOVEHQ') && $workflowModel->getModule()->name == 'Orders') {
            $assignedMoveRoles = [];
            $db = PearDatabase::getInstance();
            $result = $db->pquery('SELECT moveroles_role FROM `vtiger_moveroles_role`', []);
            while ($row =& $result->fetchRow()) {
                $assignedMoveRoles[] = $row['moveroles_role'];
            }
            $viewer->assign('ASSIGNED_MOVE_ROLES_LIST', $assignedMoveRoles);
        }
