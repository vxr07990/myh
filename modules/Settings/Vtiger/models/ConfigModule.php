...
class Settings_Vtiger_ConfigModule_Model extends Settings_Vtiger_Module_Model
{
    ...
    public function getEditViewUrl()
    {
        ...
        return '?module=System&parent=Settings&view=ConfigEditorEdit&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
    }
    ...
    {
        $menuItem = $this->getMenuItem();
        return '?module=System&parent=Settings&view=ConfigEditorDetail&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
    }
    ...
    public function getPicklistValues($fieldName)
    {
        if ($fieldName === 'default_module') {
            ...
            return $moduleData;
        } elseif ($fieldName === 'default_layout') {
            $db = PearDatabase::getInstance();
...
//			'history_max_viewed'			=> array('label' => 'LBL_MAX_HISTORY_VIEWED',				'fieldType' => 'input'),
    ...
    public function validateFieldValues($updatedFields)
    {
        ...
        } elseif (preg_match('/[\'";?><]/', $updatedFields['HELPDESK_SUPPORT_NAME'])) {
            return "LBL_INVALID_SUPPORT_NAME";
        } elseif (!preg_match('/[a-zA-z0-9]/', $updatedFields['default_module'])) {
