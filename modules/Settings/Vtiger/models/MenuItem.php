...
class Settings_Vtiger_MenuItem_Model extends Vtiger_Base_Model
{
    ...
    protected static $pinnedTable = 'vtiger_settings_userpins';
    ...
    public static $transformedUrlMapping = array(
        ...
        'index.php?module=Settings&action=listnotificationschedulers&parenttab=Settings' => 'index.php?module=Settings&submodule=System&view=Schedulers',
        ...
        'index.php?module=Settings&action=OrganizationConfig&parenttab=Settings' => 'index.php?parent=Settings&module=System&view=CompanyDetails',
        'index.php?module=Settings&action=EmailConfig&parenttab=Settings' => 'index.php?parent=Settings&module=System&view=OutgoingServerDetail',
        'index.php?module=Settings&action=CurrencyListView&parenttab=Settings' => 'index.php?parent=Settings&module=Currency&view=List',
        'index.php?module=Settings&action=TaxConfig&parenttab=Settings' => 'index.php?module=System&parent=Settings&view=TaxIndex',
        ...
        'index.php?module=Settings&action=OrganizationTermsandConditions&parenttab=Settings' => 'index.php?parent=Settings&module=System&view=TermsAndConditionsEdit',
        'index.php?module=Settings&action=CustomModEntityNo&parenttab=Settings' => 'index.php?module=System&parent=Settings&view=CustomRecordNumbering',
        ...
        'index.php?module=ConfigEditor&action=index' => 'index.php?module=System&parent=Settings&view=ConfigEditorDetail',
        ...
        'index.php?module=Settings&action=Announcements&parenttab=Settings' => 'index.php?parent=Settings&module=System&view=AnnouncementEdit',
    ...
    {
        return 'index.php?module=System&parent=Settings&action=Basic&mode=updateFieldPinnedStatus&fieldid='.$this->getId();
    }
    ...
    private function updatePinStatus($pinned=false)
    {
        ...
        if($currentUser->isAdminUser()) {
            //If admin, update the global value and return
            $query = 'UPDATE `'.self::$itemsTable.'` SET pinned=? WHERE '.self::$itemId.'=?';
            $db->pquery($query, array($pinnedStatus, $this->getId()));
            return;
        }

        $query = 'SELECT * FROM `'.self::$pinnedTable.'` WHERE '.self::$itemId.'=? AND '.self::$userId.'=?';
        $res = $db->pquery($query, [$this->getId(), $currentUser->getId()]);
        if($res && $db->num_rows($res) > 0) {
            //Update pinned status
            $query = 'UPDATE `'.self::$pinnedTable.'` SET pinned=? WHERE '.self::$itemId.'=? AND '.self::$userId.'=?';
            $db->pquery($query, [$pinnedStatus, $this->getId(), $currentUser->getId()]);
        } else {
            //Insert new row for pinned status for this user
            $query = 'INSERT INTO `'.self::$pinnedTable.'` VALUES (?,?,?)';
            $db->pquery($query, [$this->getId(), $currentUser->getId(), $pinnedStatus]);
        }
    }
    ...
    public static function getAll($menuModel=false, $onlyActive=true)
    {
        ...
        if (!$currentUserModel->isAdminUser() && $currentUserModel->isLowLevelAdminUser()) {
            $skipMenuItemList = $currentUserModel->getLowLevelAdminSkippedMenus();
        }
    ...
    public static function getPinnedItems($fieldList = array())
    {
        ...
        $currentUserModel = Users_Record_Model::getCurrentUserModel();
        if ($currentUserModel->isLowLevelAdminUser()) {
            $skipMenuItemList = $currentUserModel->getLowLevelAdminSkippedMenus();

            //Get user-pinned items
            $sql = 'SELECT * FROM `'.self::$pinnedTable.'` WHERE pinned=1 AND '.self::$userId.'=?';
            $res = $db->pquery($sql, [$currentUser->getId()]);
            if($res && $db->num_rows($res) > 0) {
                //User-pinned items exist. Build query to pull those only
                $userPins = [];
                while($row =& $res->fetchRow()) {
                    $userPins[] = $row[self::$itemId];
                }
                $query = 'SELECT * FROM '.self::$itemsTable.' WHERE active=0 AND '.self::$itemId.' IN ('.generateQuestionMarks($userPins).')';
                $params = $userPins;
                $updateUserPins = false;
            } else {
                $query = 'SELECT * FROM '.self::$itemsTable.' WHERE pinned=1 AND active = 0';
                $params = [];
                $updateUserPins = true;
            }
            if(count($skipMenuItemList) > 0) {
                $query .= ' AND name NOT IN ('.generateQuestionMarks($skipMenuItemList).')';
                $params = array_merge($params, $skipMenuItemList);
            }
        ...
        if($updateUserPins && !$currentUserModel->isAdminUser()) {
            $sql = 'INSERT INTO `'.self::$pinnedTable.'` VALUES (?,?,?)';
            foreach($menuItemModels as $id=>$model) {
                $db->pquery($sql, [$id, $currentUser->getId(), 1]);
            }
        }
        return $menuItemModels;
    }
