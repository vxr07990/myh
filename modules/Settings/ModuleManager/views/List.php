...
use MoveCrm\Modules\Collection as ModuleCollection;
...
class Settings_ModuleManager_List_View extends Settings_Vtiger_Index_View
{
    public function process(Vtiger_Request $request)
    {
        ...
        $modules = new ModuleCollection(Settings_ModuleManager_Module_Model::getAll());
        $modules = array_values($modules->getArrayCopySortedByLabel());
        $qualifiedModuleName = $request->getModule(false);
        $viewer = $this->getViewer($request);

        $viewer->assign('ALL_MODULES', $modules);
