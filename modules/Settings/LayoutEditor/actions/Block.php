...
class Settings_LayoutEditor_Block_Action extends Settings_Vtiger_Index_Action
{
    ...
    {
        file_put_contents('logs/layoutEditor.log', date('Y-m-d H:i:s - ').print_r($request, true)."\n", FILE_APPEND);
        ...
        $newLabel = strtoupper("LBL_".$sourceModule."_BLOCK_".str_replace(' ', '', $request->get('label')));
        $blockInstance->set('label', $newLabel);
        ...
        if (!$isDuplicate) {
            try {
                ...
                $this->pushBlockToDB($id, $request->get('label'), $sourceModule);
    ...
    public function pushBlockToDB($blockId, $label, $moduleName)
    {
        ...
        $db->pquery($sql, []);
        $sql = "INSERT INTO `$blockTable` VALUES(?,?,?)";
        $db->pquery($sql, [$blockId, $label, $moduleName]);
    }
}
