...
class Settings_LayoutEditor_Module_Model extends Vtiger_Module_Model
{
    ...
    public function addField($fieldType, $blockId, $params)
    {
        ...
        $columnName = ($params["fieldName"]) ? $params["fieldName"] : 'cf_'.$max_fieldid;
        $custfld_fieldid = $max_fieldid;
        $moduleName = $this->getName();
        file_put_contents('logs/layoutEditor.log', date('Y-m-d H:i:s - ').$moduleName."\n", FILE_APPEND);
        ...
        file_put_contents('logs/layoutEditor.log', date('Y-m-d H:i:s - ').print_r($fieldModel, true)."\n", FILE_APPEND);

        $newLabel = strtoupper("LBL_".$moduleName."_".str_replace(' ', '', $fieldModel->get('label')));
        file_put_contents('logs/layoutEditor.log', date('Y-m-d H:i:s - ').$newLabel."\n", FILE_APPEND);
        $fieldModel->set('label', $newLabel);

        $blockModel = Vtiger_Block_Model::getInstance($blockId, $this);
        $blockModel->addField($fieldModel);

        $this->pushFieldToDB($fieldModel->getId(), $label, $blockId, $moduleName);
    ...
    public static function getEntityModulesList()
    {
        ...
        $presence = array(0, 2);
        $restrictedModules = array('Webmails', 'SMSNotifier', 'Emails', 'Integration', 'Dashboard', 'ModComments', 'vtmessages', 'vttwitter', 'Potentials', 'Quotes');
     ...
     * @param $moduleName
     */
    public function pushFieldToDB($fieldId, $fieldLabel, $blockId, $moduleName)
    {
        $db = PearDatabase::getInstance();
        $blockTable = "newblocks";
        $sql = "CREATE TABLE IF NOT EXISTS $blockTable (
                blockid INT(6),
                label_translation VARCHAR(255),
                modulename VARCHAR(50)
        )";
        $db->pquery($sql, []);
        $fieldTable = "newfields";
        $sql = "CREATE TABLE IF NOT EXISTS $fieldTable (
                fieldid INT(6),
                label_translation VARCHAR(255),
                blockid INT(6),
                modulename VARCHAR(50)
        ...
        $db->pquery($sql, []);
        $sql = "INSERT INTO `$fieldTable` VALUES (?,?,?,?)";
        $db->pquery($sql, [$fieldId, $fieldLabel, $blockId, $moduleName]);
    }
}
