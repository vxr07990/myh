...
class VanlineManager_Popup_View extends Vtiger_Popup_View
{
    ...
    public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer)
    {
        ...
        //@TODO: consider finding a less hack solution.
        if ($sourceModule != 'LocalCarrier') {
            parent::initializeListViewContents($request, $viewer);
        } else {
            //in order to allow SalesManager to select a brand for local carrier
            //it just does the SELECT and builds the listEntries from that directly.
            //everything else is standard from Vtiger_Popup_View
            ...
            if (!empty($relatedParentModule) && !empty($relatedParentId)) {
                ...
                $listViewModel     =
                    Vtiger_RelationListView_Model::getInstance($parentRecordModel, $moduleName, $label);
            ...
            //since our sales manager won't be able to get the entries the normal way rebuild listViewEntries from
            // the table directly.
            $db     = PearDatabase::getInstance();
            $stmt   = 'SELECT `vanline_name`,`phone1`,`vanlinemanagerid` FROM `vtiger_vanlinemanager`';
            $result = $db->pquery($stmt, []);
            while ($row =& $result->fetchRow()) {
                $this->listViewEntries[$row['vanlinemanagerid']] =
                    Vtiger_Record_Model::getInstanceById($row['vanlinemanagerid'], 'VanlineManager');
            }
