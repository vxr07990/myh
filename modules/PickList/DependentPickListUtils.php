...
class Vtiger_DependencyPicklist
{
    ...
    public static function getPickListDependency($module, $sourceField, $targetField, $accesibleAgents = false)
    {
        global $adb;
        ...
        if( ! $accesibleAgents ){
            ...
            if(method_exists($currentUser, 'getBothAccessibleOwnersIdsForUser')){
                 $accesibleAgents = $currentUser->getBothAccessibleOwnersIdsForUser(); //I'm not 100% percent sure the OT 4482 will me merge in before this. Just in case it's not
            }else{
                ...
                if ( count($accesibleVanlines) > 0 ){
                    $accesibleAgents = array_merge($accesibleAgents,$accesibleVanlines);
                }
            }
        }

        $result = $adb->pquery('SELECT * FROM vtiger_picklist_dependency '
                ...
                . ' WHERE tabid=? AND sourcefield=? AND targetfield=? AND agentid IN ('. generateQuestionMarks($accesibleAgents).')',
                array($tabId, $sourceField, $targetField,$accesibleAgents));
        ...
        $valueMapping = array();
        $mappedSourceValues = array();//@TODO: unused?
    ...
    public static function getPicklistDependencyDatasource($module)
    {
        ...
        //check if there is a picklist for the user
        //if not, check if there is a dependency for the vanline of the user
        //if not, get all dependencies for the module
        $result = $adb->pquery('SELECT * FROM vtiger_picklist_dependency '
                . ' INNER JOIN vtiger_custom_picklist_dependency ON vtiger_custom_picklist_dependency.picklistdependencyid = vtiger_picklist_dependency.id '
                . ' WHERE tabid=? AND agentid IN ('. generateQuestionMarks($accesibleAgents).')',
                array($tabId,$accesibleAgents));
        $noofrows = $adb->num_rows($result);
        if($noofrows == 0){
            $result = $adb->pquery('SELECT * FROM vtiger_picklist_dependency '
                . ' INNER JOIN vtiger_custom_picklist_dependency ON vtiger_custom_picklist_dependency.picklistdependencyid = vtiger_picklist_dependency.id '
                . ' WHERE tabid=? AND agentid IN ('. generateQuestionMarks($accesibleVanlines).')',
                array($tabId,$accesibleVanlines));
            $noofrows = $adb->num_rows($result);
            if($noofrows == 0){
