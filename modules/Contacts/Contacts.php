...
// vtiger_account query goes for ecery single vtiger_account row
    ...
    */
    public function get_opportunities($id, $cur_tab_id, $rel_tab_id, $actions=false)
    {
      ...
      /**
      * Function to get Contact related Surveys
     ...
     public function get_surveys($id, $cur_tab_id, $rel_tab_id, $actions=false)
     {
         ...
         $userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>
                            'vtiger_users.first_name', 'last_name' => 'vtiger_users.last_name'), 'Users');
         $query = "select case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,vtiger_crmentity.*, vtiger_surveys.*,vtiger_potential.potentialname,vtiger_contactdetails.lastname,vtiger_account.accountname from vtiger_surveys inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_surveys.surveysid left outer join vtiger_contactdetails on vtiger_contactdetails.contactid=vtiger_surveys.contact_id left outer join vtiger_potential on vtiger_potential.potentialid=vtiger_surveys.potential_id  left join vtiger_account on vtiger_account.accountid = vtiger_surveys.account_id LEFT JOIN vtiger_surveyscf ON vtiger_surveyscf.surveysid = vtiger_surveys.surveysid left join vtiger_users on vtiger_users.id=vtiger_crmentity.smownerid left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid where vtiger_crmentity.deleted=0 and vtiger_contactdetails.contactid=".$id;
    ...
    */
    public function get_emails($id, $cur_tab_id, $rel_tab_id, $actions=false)
    {
        ...
        $query = "select case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name," .
                ...
                " from vtiger_activity, vtiger_seactivityrel, vtiger_contactdetails, vtiger_users, vtiger_emaildetails, vtiger_email_track, vtiger_crmentity" .
                " left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid" .
                " where vtiger_seactivityrel.activityid = vtiger_activity.activityid" .
                " and vtiger_contactdetails.contactid = vtiger_seactivityrel.crmid" .
