...
class Leads_Module_Model extends Vtiger_Module_Model
{
    ...
    */
     ...
     * @return array e.g. `Array ( [0] => Array ( [0] => 1 [count] => 1 [1] => Attempted to Contact [leadstatusvalue] => Attempted to Contact ) )`
     */
    private function _getLeadCountsByStatus($groupId, $dateFilter = array())
    {
        $db = PearDatabase::getInstance();
        $ownerSql = $this->getOwnerWhereConditionForDashBoards($groupId);
        $params = array();
        ...
        $accessControlQuery = Users_Privileges_Model::getNonAdminAccessControlQuery($this->getName());
        $sql = "SELECT COUNT(*) as count,
			    CASE WHEN vtiger_leadstatus.leadstatus
		   	    IS NULL OR vtiger_leadstatus.leadstatus = ''
			    THEN ''
			    ELSE vtiger_leadstatus.leadstatus
			    END AS leadstatusvalue
			    ...
			    GROUP BY leadstatusvalue
			    ORDER BY vtiger_leadstatus.sortorderid";
        file_put_contents('logs/devLog.log', "\n S : ".print_r($sql, true), FILE_APPEND);
    ...
    /**
     * Returns all leads with a "status" grouped by the aforementioned status.
     * @param int $groupId
     * @param string[] $dateFilter
     * @return array e.g. `Array ( [0] => Array ( [0] => 1 [1] => Attempted to Contact [2] => Attempted to Contact ) )`
     */
    public function getLeadsByStatus($owner, $dateFilter = array())
    {
        /*if (!is_numeric($groupId) && !empty($groupId)) {
            throw new Exception(
                sprintf(
                    'Numeric value or empty string required for the `$groupId` parameter, a value of `%s` supplied to `%s`.',
                    $groupId,
                    __METHOD__
                )
            );
        ...
        $result = $this->_getLeadCountsByStatus($groupId, $dateFilter);
        $response = array();

        foreach ($result as $row) {
            $status = array();
            $status[] = $row['count'];
            $status[] = vtranslate($row['leadstatusvalue'], $this->getName());
            $status[] = (!empty($row['leadstatusvalue'])) ? $row['leadstatusvalue']
                                                          : 'LBL_BLANK';
            $response[] = $status;
        }
    ...
    public function getLeadsBySource($owner, $dateFilter)
    {
        ...
        $sql = 'SELECT COUNT(*) as count,leadsourcevalue, sortorderid FROM (SELECT sortorderid, leadid, CASE WHEN vtiger_leaddetails.leadsource IS NULL OR vtiger_leaddetails.leadsource = "" THEN ""
        ...
        GROUP BY leadid) as LeadData GROUP BY leadsourcevalue ORDER BY sortorderid';
    ...
    public static function getConvertedInfo($recordIdsList = array())
    {
        ...
        if ($recordIdsList) {
            ...
            for ($i=0; $i<$numOfRows; $i++) {
                $convertedInfo[$db->query_result($result, $i, 'leadid')] = $db->query_result($result, $i, 'converted');
            }
        }
    ...
    /**
     * Generates a `smownerid` SQL statement parameter.
     * @param  int $groupId
     * @return string|null e.g. " smownerid = 26"
     */
    public function getOwnerWhereConditionForDashBoards($owner)
    {
        if (!is_numeric($owner) && !empty($owner)) {
            throw new Exception(
                sprintf(
                    'A numeric or empty string parameter is required for: `%s`, provided parameter: `%s`.',
                    __METHOD__,
                    $owner
                )
            );
        ...
        $currentUserModel = Users_Record_Model::getCurrentUserModel();
        if (empty($owner)) {
            $accessibleUsers = filterUserAccessibleUsers($currentUserModel->getAccessibleUsers());
            //file_put_contents('logs/devLog.log', "\n AccessibleUsers : ".print_r($accessibleUsers, true), FILE_APPEND);
            $userSql = "(";
            $userSql .= implode(',', array_keys($accessibleUsers));
            $userSql .= ")";
        } else {
            $userSql = "($owner)";
        }

        return (!$currentUserModel->isAdminUser()) ? " smownerid IN $userSql"
                                                   : null;
    }
}
