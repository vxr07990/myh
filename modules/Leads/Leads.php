    ...
    */
    public function save_module($module)
    {
        if (getenv('INSTANCE_NAME') != 'graebel') {
            ...
            $MoveRolesrModel = Vtiger_Module_Model::getInstance('MoveRoles');
            if ($MoveRolesrModel && $MoveRolesrModel->isActive()) {
                //one issue:  [module] => Leads
                $MoveRolesrModel->saveMoveRoles($_REQUEST, $this->id);
            }
        }
        //Address List save
    ...
    public function saveentity($module, $fileid = '')
    {
        ...
        if ($record) {
            $recordModel = Leads_Record_Model::getInstanceById($record);
            if ($recordModel &&
               ($recordModel->get('leadstatus') == 'Cancelled'
                   || $recordModel->get('leadstatus') == 'Duplicate'
               )
            ) {
    ...
    */
    ...
    public function get_emails($id, $cur_tab_id, $rel_tab_id, $actions=false)
    {
        ...
        $query ="select case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name," .
                ...
                " join vtiger_emaildetails on vtiger_crmentity.crmid=vtiger_emaildetails.emailid" .
                " join vtiger_email_track on vtiger_crmentity.crmid=vtiger_email_track.mailid" .
                " left join vtiger_groups on vtiger_groups.groupid=vtiger_crmentity.smownerid" .
                " left join vtiger_users on  vtiger_users.id=vtiger_crmentity.smownerid" .
