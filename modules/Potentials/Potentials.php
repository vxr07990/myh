...
class Potentials extends CRMEntity
{
    ...
    */
    ...
    public function saveParticipantAgents()
    {
        /* $db = PearDatabase::getInstance();

        $inbox_id = NULL; //for Amin's stuff this probably needs to get set ot something, I can't tell what though

        $modifiedOn = (new DateTime())->format('Y-m-d H:i:s');
        $currentUserModel = Users_Record_Model::getCurrentUserModel();
        $modifiedBy = $currentUserModel->id; // get the current user's ID don't know why this needs to be here either

        $agentPermission    = vtlib_purify($_REQUEST['agent_permission']);
        $agentTypeArray     = vtlib_purify($_REQUEST['agent_type']); //Which picklist item the Agent Type dropdown is
        $agentIdArray       = vtlib_purify($_REQUEST['opp_participants']);
        $agentStatusArray   = vtlib_purify($_REQUEST['agent_status']);
        $linesId            = vtlib_purify($_REQUEST['table_id']);

        $oppRecord =  $this->id;

        if ($agentPermission && is_array($agentPermission)) {
            foreach ($agentPermission as $rowNo => $permission) {

                //stuff that's different for each participating agent
                $agentId    = $agentIdArray[$rowNo];
                $agentType  = $agentTypeArray[$rowNo];
                $status     = $agentStatusArray[$rowNo];
                $lineId     = $linesId[$rowNo];

                if ($status == '2') {
                    $sql = "DELETE FROM `vtiger_participating_agents` WHERE id=? AND crmentity_id=?";
                    $db->pquery($sql, [$lineId, $oppRecord]);
                    continue;
                }

                $result = $db->pquery("SELECT id FROM `vtiger_participating_agents` WHERE agent_type = ? AND agent_id=? AND crmentity_id = ?", array($agentType, $agentId, $oppRecord));
                if($db->num_rows($result) > 0){

                    $sql = "UPDATE `vtiger_participating_agents` SET agent_id=?, agent_type=?, permission=?, modified_on=?, modified_by=?
                         WHERE id=? AND crmentity_id=?";

                    $db->pquery($sql, [$agentId, $agentType, $permission, $modifiedOn, $modifiedBy, $lineId, $oppRecord]);
                } else {

                    $sql = "INSERT INTO `vtiger_participating_agents`
                              (crmentity_id, inbox_id, agent_id, agent_type, permission, modified_on, modified_by, status)
                               VALUES (?,?,?,?,?,?,?,?)";

                    $db->pquery($sql, [$oppRecord, $inbox_id, $agentId, $agentType, $permission, $modifiedOn,
                        $modifiedBy, $status]);
                }
            }
        }*/
        file_put_contents('logs/devLog.log', "\n Old Securities Called in Potentials.php", FILE_APPEND);
    }
    ...
    public function getNonAdminAccessQuery($module, $user, $parentRole, $userGroups)
    {
        ...
        $query .= getParticipantAgentShareRecords('Potentials');
