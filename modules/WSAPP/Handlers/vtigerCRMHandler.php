...
class vtigerCRMHandler extends SyncHandler
{
    ...
    public function get($module, $token, $user)
    {
        ...
        if ($syncModule == 'Events') {
            $surveyResult = vtws_sync($token, 'Surveys', $syncType, $this->user);
            $surveyResult['updated'] = $this->translateTheReferenceFieldIdsToName($surveyResult['updated'], 'Surveys', $user);
            
            $result['updated'] = array_merge($result['updated'], $surveyResult['updated']);
            $result['deleted'] = array_merge($result['deleted'], $surveyResult['deleted']);
            $result['more'] = $result['more'] || $surveyResult['more'];
            $result['lastModifiedTime'] = ($result['lastModifiedTime'] <= $surveyResult['lastModifiedTime']) ? $surveyResult['lastModifiedTime'] : $result['lastModifiedTime'];
        }
        ...
        foreach ($referenceFieldDetails as $referenceFieldName => $referenceModuleDetails) {
            ...
            for ($i = 0; $i < $recordCount; $i++) {
                ...
                if (!empty($record[$referenceFieldName])) {
                    ...
                    $record[$referenceFieldName.'_referenceID'] = $wsId[1];
                }
                $records[$i] = $record;
            }
        }
