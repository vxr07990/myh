    ...
    public function generateData()
    {
        ...
        for ($i = 0; $i < $rows; $i++) {
            ...
            if ($legendField) {
                ...
                } elseif ($fieldDataType == 'multiagent') {
                    $multiAgentValue = $row[strtolower($legend)];
                    $multiAgentValues = explode(' |##| ', $multiAgentValue);
                    foreach ($multiAgentValues as $multiAgentValue) {
                        $labelList[] = vtranslate($multiAgentValue, $legendField->getModuleName());
                    }
                    $label =  implode(',', $labelList);
