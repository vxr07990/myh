...
class Reports_Record_Model extends Vtiger_Record_Model
{
    ...
    {
        if($this->getId() <= 0 && getenv('INSTANCE_NAME') == 'graebel')
        {
            return [
                'Distribution Report',
                'I.Code Report',
    ...
    public function getSelectedFields()
    {
        ...
        for ($i=0; $i<$db->num_rows($result); $i++) {
            ...
            if(array_key_exists($tableName,ReportRun::$customTableNames))
            {
                $selectedColumns[$moduleFieldLabel] = $column;
            } else if (CheckFieldPermission($fieldName, $module) == 'true' && $columnName != 'crmid') {
    ...
    public function save()
    {
        ...
        if (empty($reportId)) {
            ...
            $reportParams = array($reportId, $this->get('folderid'), $this->get('reportname'), $this->get('description'),
                    $this->get('reporttype', 'tabular'), $reportId, 'CUSTOM', $currentUser->id, 'Public',$this->get('agentid'));
            $db->pquery('INSERT INTO vtiger_report(reportid, folderid, reportname, description,
								reporttype, queryid, state, owner, sharingtype,agentid) VALUES(?,?,?,?,?,?,?,?,?,?)', $reportParams);
            ...
            $db->pquery('UPDATE vtiger_report SET reportname = ?, description = ?, reporttype = ?, folderid = ?,agentid = ? WHERE
				reportid = ?', array($this->get('reportname'), $this->get('description'), $this->get('reporttype'), $this->get('folderid'),$this->get('agentid'), $reportId));
    ...
    public function generateCountQuery($query)
    {
        ...
        $fromAndWhereQuery = explode(' order by ', array_values(array_slice($from, -1))[0]);
    ...
    public function getReportXLS()
    {
        ...
        $reportRun->writeReportToExcelFile($fileName, $advanceFilterSql);
        /*
