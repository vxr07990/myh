...
class Orders_SaveParticipants_Action extends Vtiger_BasicAjax_Action
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($field == 'participantPermission') {
            ...
            $sql = 'UPDATE `vtiger_orders_participatingagents` SET permissions = ? WHERE ordersid = ? AND participantid = ?';
            $db->pquery($sql, array($fieldValue, $record, $id));
        } elseif ($field == 'agentType') {
            $db = PearDatabase::getInstance();
            $sql = 'UPDATE `vtiger_orders_participatingagents` SET agenttype = ? WHERE ordersid = ? AND participantid = ?';
            $db->pquery($sql, array($fieldValue, $record, $id));
        }
