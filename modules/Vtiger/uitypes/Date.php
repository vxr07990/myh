...
class Vtiger_Date_UIType extends Vtiger_Base_UIType
{
    ...
    public function getEditViewDisplayValue($value)
    {
        if (empty($value) || $value === ' ') {
            ...
            } elseif ($fieldName == 'weight_date' && $moduleName == 'WFOrders'){
                $value = DateTimeField::convertToUserFormat(date('Y-m-d'));
            }
        } else {
            if ($this->verifyValidDate($value)) {
    ...
    public function verifyValidDate($value){
        //Note: assumes yyyy-mm-dd format, like convertToUserFormat()
        $dateComponents = explode('-', $value);
        return checkdate($dateComponents[1], $dateComponents[2], $dateComponents[0]);
    }
