...
class Vtiger_Util_Helper
{
    ...
    public static function formatDateIntoStrings($date, $time = false)
    {
        ...
        $dateTimeInUserTimezone = DateTimeField::convertToUserTimeZone($date.' '.$time)->format('Y-m-d H:i:s');
        $dateTimeInUserFormat = Vtiger_Datetime_UIType::getDisplayDateTimeValue($dateTimeInUserTimezone);

        list($dateInUserFormat, $timeInUserFormat) = explode(' ', $dateTimeInUserFormat, 2);
        $timeValueArr = explode(' ', $timeInUserFormat);
        list($hours, $minutes, $seconds) = explode(':', $timeValueArr[0]);

        $displayTime = $hours .':'. $minutes;
        if ($currentUser->get('hour_format') === '12') {
            $displayTime .= ' '.$timeValueArr[1];
        }
    ...
    public static function getAllPicklistValuesAsString($fieldName) {
        $db = PearDatabase::getInstance();
        $tableName = 'vtiger_'.$fieldName;
        $query1 = "SHOW TABLES LIKE '$tableName'";
        ...
        $presenceExists = columnExists("presence", "vtiger_{$fieldName}");
        if($presenceExists) {
    ...
    public static function getCustomPicklistValues($fieldName, $fieldId, $agentid = false) {
        $db = PearDatabase::getInstance();
        $primaryKey = Vtiger_Util_Helper::getPickListId($fieldName);
        //Verifying that a table exists for the picklist values
        $tableName = 'vtiger_'.$fieldName;
        $query1 = "SHOW TABLES LIKE '$tableName'";
        $result1 = $db->pquery($query1, array());
        $values   = [];
        if ($db->num_rows($result1) > 0) {
            $values = Vtiger_Util_Helper::getAllowedFieldsValuesForUser($fieldName,[$agentid], $fieldId);
        }
     ...
     */
    public static function getPickListValues($fieldName,$ownerBaseField = false,$agentid = false, $getFromCache = true)
    {
        ...
        $values   = [];
        if ($db->num_rows($result1) > 0) {
            $query    = 'SELECT '.$primaryKey.', '.$fieldName.' FROM vtiger_'.$fieldName;

            // No idea if all tables have this, not about to risk it.
            $presenceExists = columnExists("presence", "vtiger_{$fieldName}");
            if($presenceExists) {
                $query .= ' WHERE presence=1 ';
            }
            ...
            if($ownerBaseField){
                $params = [];
                $allowedPicklist = Vtiger_Util_Helper::getAllowedFieldsValuesForUser($fieldName,$agentid);

                //If the agent hasnt set any picklist values fall-back to default values. Otherwise show ONLY their values.

                if(is_array($allowedPicklist) && count($allowedPicklist) > 0 ){
                    $query    .= ($presenceExists ? ' AND ' : ' WHERE ') . $primaryKey . ' IN (' . generateQuestionMarks($allowedPicklist) . ')';
                    $params[] = $allowedPicklist;
                }else{
                    $filterOutValues = Vtiger_Util_Helper::getNotAllowedFieldsValuesForUser($fieldName,$agentid); //We filter out to include the values that are assigned to ALL agents. Those will not be on vtiger_custompicklist table
                    if(is_array($filterOutValues) && count($filterOutValues) >0){
                        $query    .=  ($presenceExists ? ' AND ' : ' WHERE ') . $primaryKey . ' NOT IN (' . generateQuestionMarks($filterOutValues) . ')';
                        $params[] = $filterOutValues;
                    }
                }
            }
    ...
    public static function isPicklistValueSpecial($fieldName, $picklistValueId){
        $tableName = 'vtiger_'.$fieldName;
        $primaryKey = Vtiger_Util_Helper::getPickListId($fieldName);
        $db = PearDatabase::getInstance();
        if( ! Vtiger_Utils::CheckColumnExists($tableName, 'special')){
            $special = 0;
        }else{
             $sql    = "SELECT special FROM $tableName WHERE $fieldName = '$picklistValueId' AND $primaryKey NOT IN (select valueid from vtiger_custompicklist where fieldid='$fieldName')";
             $result = $db->pquery($sql);
             if($db->num_rows($result) > 0){
                 $special = $db->query_result($result, 0, 'special');
             }else{
                 $special = 0;
             }
        }
    ...
    public static function getNotAllowedFieldsValuesForUser($fieldName, $accesibleAgents = false){
        if( ! $accesibleAgents ){
            $currentUser = Users_Record_Model::getCurrentUserModel();
        ...
        $db = PearDatabase::getInstance();
        $fields = [];
        $sql = "SELECT valueid FROM vtiger_custompicklist WHERE fieldid=? AND agentid NOT IN (". generateQuestionMarks($accesibleAgents).")";
        $result = $db->pquery($sql, array( $fieldName, $accesibleAgents ));
        if ( $db->num_rows($result) > 0 ) {
    ...
    public static function getAllowedFieldsValuesForUser($fieldName, $accesibleAgents = false, $fieldId = false){
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $accesibleVanlines = array_keys($currentUser->getAccessibleVanlinesForUser());

        if( ! $accesibleAgents || (is_array($accesibleAgents) && count($accesibleAgents) == 0) || $accesibleAgents[0] == NULL){
            $accesibleAgents = array_keys($currentUser->getAccessibleAgentsForUser());
        }

        if(is_array($accesibleAgents)) {
            $ownerAgent = $accesibleAgents[0];
        } else {
            $ownerAgent = $accesibleAgents;
        }
        ...
        if ($db->num_rows($result1) > 0) {
            ...
            $defaultValuesResult = $db->query($sql);
            $primaryKey = Vtiger_Util_Helper::getPickListId($fieldName);
            while($row =& $defaultValuesResult->fetchRow()) {
                $fields[$row[$primaryKey]] = $row[$fieldName];
            }

            $sql = "SELECT setype FROM `vtiger_crmentity` WHERE crmid=?";
            $ownerRes = $db->pquery($sql, [$ownerAgent]);
            if(!$ownerRes) {
                return $fields;
            }
            $isVanline = $ownerRes->fields['setype'] == 'VanlineManager';
            if(!$isVanline) {
                $sql = "SELECT vanline_id FROM `vtiger_agentmanager` WHERE agentmanagerid=?";
                $ownerRes = $db->pquery($sql, [$ownerAgent]);
                $ownerVanline = $ownerRes->fields['vanline_id'];
            } else {
                $ownerVanline = $ownerAgent;
            }

            $customFields = [];

            //Apply vanline-level customizations
            $sql = "SELECT * FROM `vtiger_picklistexceptions` WHERE agentid=? AND fieldid=? ORDER BY `id` ASC";
            $result = $db->pquery($sql, [$ownerVanline, $fieldId]);
            while($row =& $result->fetchRow()) {
                $type = $row['type'];
                switch($type) {
                    case 'ADDED': //add logic
                        $fields[$row['id']] = $row['value'];
                        break;
                    case 'RENAMED': //rename logic
                        unset($fields[$row['old_val_id']]);
                        $fields[$row['id']] = $row['value'];
                        break;
                    case 'DELETED': //delete logic
                        unset($fields[$row['old_val_id']]);
                        break;
                    case 'CUSTOM_DELETED': //delete custom item logic
                        unset($fields[$row['old_val_id']]);
                        break;
                }
            }
            ...
            if(!$isVanline) {
                //Apply agent-level customizations
                $sql    = "SELECT * FROM `vtiger_picklistexceptions` WHERE agentid=? AND fieldid=? ORDER BY `id` ASC";
                $result = $db->pquery($sql, [$ownerAgent, $fieldId]);
                while($row =& $result->fetchRow()) {
                    $type = $row['type'];
                    switch($type) {
                        case 'ADDED': //add logic
    ...
    public static function getDefaultPickListValues($fieldName,$agentid)
    {
        $picklistValues = Vtiger_Util_Helper::getPickListValues($fieldName,false);
        $filterOutValues = Vtiger_Util_Helper::getNotAllowedFieldsValuesForUser($fieldName,$agentid);
        if ( !empty($filterOutValues)){
            foreach ($picklistValues as $key => $value) {
                if( in_array($key, $filterOutValues)){
...
/**
    ...
    public static function convertDateTimeIntoUsersDisplayFormatInTimeZone($dateTime, $timeZone, $userObject = null) {
        require_once 'includes/runtime/LanguageHandler.php';
        require_once 'includes/runtime/Globals.php';
        if ($userObject) {
            $userModel = Users_Privileges_Model::getInstanceFromUserObject($userObject);
        ...
        $date = $dateTimeField->getDisplayDateInTimeZone($timeZone, $userModel);
        $time = $dateTimeField->getDisplayTimeInTimeZone($timeZone);

        return $date . ' ' .$time . ' ' . '(UTC' . $timeZoneDisplay . ')';
    }
    ...
    /**
     * Function to get the time value in user-set time zone
     * @param <Time> $time
     * @param <Vtiger_Users_Model> $userObject
     * @return <String> time in user-set time zone along with time zone string
     */
    public static function convertTimeIntoUsersTimeZone($time, $userObject = null)
    {
        if($userObject) {
            $userModel = Users_Privileges_Model::getInstanceFromUserObject($userObject);
        } else {
        ...
        if($userModel->get('hour_format') == '12') {
            ...
            $carbonTime = Carbon::createFromFormat('G:i:s', $time);
        }

        $carbonTime->setTimezone($userModel->get('time_zone'));

        $timeZoneString = '['.vtranslate($userModel->get('time_zone'), 'Users').']';

        return ($ampm ? $carbonTime->format('g:i A') : $carbonTime->format('G:i:s')).' '.$timeZoneString;
    }
    ...
    */
    ...
    public static function transferListSearchParamsToFilterCondition($listSearchParams, $moduleModel, $headers)
    {
        ...
        foreach ($listSearchParams as $groupInfo) {
            ...
            foreach ($groupInfo as $fieldSearchInfo) {
                ...
                //Phone numbers are stored as numbers with no spaces. Remove parentheses, spaces, dashes, and anything not a number
                if($fieldInfo->get('uitype') == 11){
    ...
    */
    ...
    {
        return array(/*'alphagrey' => '#666666',*/    'tightview'    => '#1560BD',    'wideview'=> '#204E81',
