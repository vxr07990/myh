...
class Vtiger_RelationListView_Model extends Vtiger_Base_Model
{
    ...
    public function getLinks()
    {
        ...
        if (isPermitted($parentModuleName, 'EditView', $parentRecordId) == 'yes') {
        ...
        //adding related records restricted by edit view permissions
        if (isPermitted($parentModuleName, 'EditView', $parentRecordId) == 'yes') {
    ...
    public function getEntries($pagingModel)
    {
        ...
        if(getenv('INSTANCE_NAME') != 'sirva' && $this->get('query_generator')) {
            ...
            foreach ($fields as $fieldName) {
                if($fieldName == 'parent_id' && $relationModuleName == 'Calendar') {
                    $relatedColumnFields['parent_id'] = $fieldName;
                } elseif (array_key_exists($fieldName, $moduleFields)) {
                    $relatedColumnFields[$moduleFields[$fieldName]->getColumnName()] = $fieldName;
                }
            }

            $query = $this->getRelationQuery();
            $queryGenerator->setFields($relatedColumnFields);
            // So this is like the worst thing ever, but this needs to work without breaking everything.
            $selectColumnSql = $queryGenerator->getSelectClauseColumnSQL();

            // to update relatedColumnFields from additional fields needed for composite fields
            $fields = $queryGenerator->getFields();
            foreach ($fields as $fieldName) {
            ...
            $fieldsToInsert = array_map('trim', explode(',', $selectColumnSql));
            $matches = [];
            // Note this won't work if there are subqueries in the select clause
            if(preg_match('/select.*\sfrom\s/i', $query, $matches, PREG_OFFSET_CAPTURE)) {
                $newSelectColumnSql = substr($matches[0][0],0, -6);
                $newSelectColumnSql = preg_replace('/select distinct/i','SELECT',$newSelectColumnSql);
                foreach($fieldsToInsert as $fSql)
                {
                    if(!preg_match("/$fSql/i", $newSelectColumnSql))
                    {
                        $newSelectColumnSql .= ", $fSql";
                    }
                }
                $query = $newSelectColumnSql . ' FROM ' . substr($query, $matches[0][1] + strlen($matches[0][0]));
            }
        }else{
        ...
        if ($relationModuleName == 'Estimates') {
            $query .= " AND setype='Estimates'";
        } elseif ($relationModuleName == 'Actuals') {
        ...
        for ($i=0; $i< $db->num_rows($result); $i++) {
            ...
            foreach ($row as $col=>$val) {
                if (array_key_exists($col, $relatedColumnFields)) {
                    ...
                    } elseif (strtolower($col) == 'modifiedtime' || strtolower($col) == 'createdtime') {
                        $val = DateTimeField::convertToUserTimeZone($val)->format('Y-m-d H:i:s');
                    }
            ...
            if($relationModuleName == 'Surveys' && array_key_exists('survey_date', $newRow)) {
                $timeZone=getFieldTimeZoneValue('survey_time', $recordId);
                $newRow['survey_date'] = DateTimeField::convertTimeZone($newRow['survey_date'].' '.$row['survey_time'], DateTimeField::getDBTimeZone(), $timeZone)->format('Y-m-d');
            }
    ...
    public function getHeaders()
    {
        ...
        if(getenv('INSTANCE_NAME') != 'sirva' && $this->get('query_generator')) {
            ...
            foreach ($fields as $fieldName) {
                if (array_key_exists($fieldName, $moduleFields)) {
                    $webserviceField = $moduleFields[$fieldName];
    ...
    public function getRelatedEntriesCount()
    {
        ...
        if ($position) {
            ...
            if (!$split[1]) {
                //@NOTE: if there is nothing set for value 1 then there is no table to "FROM" from.
                return 0;
            }
            // Back to core!
    ...
    public function updateQueryWithWhereCondition($relationQuery)
    {
        ...
        if(is_array($whereCondition)) {
            ...
            $whereCondition = strtolower($whereCondition);
            $temp1= explode("group by" , $whereCondition);
            // sigh... this needs to be fixed properly
            $temp2=explode('where', $temp1[0], 2);
            $condition = str_replace("vtiger_crmentity.deleted=0 and",'',$temp2[1]);
        }

        $updatedQuery = preg_replace ('/\s*WHERE\s*(?!.*WHERE\s*)/is', ' WHERE ' . $condition . ' AND ', $relationQuery);
...
//        }
    ...
    public function getListViewMassActions($linkParams)
    {
        ...
        $massActionLinks[] = array(
            ...
            'linkurl' => 'javascript:Vtiger_RelatedList_Js.triggerEditFilter()',
            'linkicon' => ''
        );
        ...
        $massActionLinks[] = array(
            ...
            'linkurl' => 'javascript:Vtiger_RelatedList_Js.triggerDeleteFilter()',
            'linkicon' => ''
        );
