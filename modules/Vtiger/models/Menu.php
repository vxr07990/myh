...
class Vtiger_Menu_Model extends Vtiger_Module_Model
{
    ...
    public static function getAll($sequenced = false)
    {
        ...
        foreach ($modules as $module) {
            //@TODO JG HERE something broken here: find eventually?
            //PHP Fatal error:  Call to a member function isAdminUser() on a non-object in /var/www/public/moveHQ/modules/Vtiger/models/Menu.php on line 45
            if (($userPrivModel->isAdminUser() ||
                    $userPrivModel->hasGlobalReadPermission() ||
                    ($userPrivModel->hasModulePermission($module->getId()) && $module->getName() != 'TariffManager')) && !in_array($module->getName(), $restrictedModulesList) && $module->get('parent') != '') {
