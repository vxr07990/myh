...
class Vtiger_BasicAjax_Action extends Vtiger_Action_Controller
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        $searchModuleModel->set('searchField', substr($request->get('field_name'), 0, strpos($request->get('field_name'), '_display')));
        ...
        if (is_array($records)) {
            foreach ($records as $moduleName=>$recordModels) {
                foreach ($recordModels as $recordModel) {
                    if(($relatedModule == 'Leads' || $relatedModule == 'Opportunities' || $relatedModule == 'Orders') && $searchModule == 'EmployeeRoles'){
                        if($recordModel->getData()['emprole_class_type'] == 'Office'){
                            $result[] = array(
