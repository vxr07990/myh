...
class Vtiger_MassSave_Action extends Vtiger_Mass_Action
{
    ...
    {
        if(\MoveCrm\InputUtils::CheckboxToBool($request->get('assigned_user_id_mass_edit_check'))) {
            return $this->updateAssignedTo($request);
        }
        $moduleName = $request->getModule();
        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);
    ...
    private function updateAssignedTo(Vtiger_Request $request) {
        $moduleName = $request->getModule();
        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);
        $recordModels = $this->getRecordModelsFromRequest($request);
        $allRecordSave= true;
        ...
        foreach ($recordModels as $recordId => $recordModel) {
            if (Users_Privileges_Model::isPermitted($moduleName, 'Save', $recordId)) {
                ...
                $db->pquery($sql, [$request->get('assigned_user_id'), $recordId]);
            } else {
                $allRecordSave= false;
            }
        }
