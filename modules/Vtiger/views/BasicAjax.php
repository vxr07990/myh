...
class Vtiger_BasicAjax_View extends Vtiger_Basic_View
{
    ...
    public function showSearchResults(Vtiger_Request $request)
    {
        ...
        // @NOTE Global search limiting, to disable modules without agent permissions from being searched.
        $matchingRecordsList = Vtiger_Module_Model::limitSearchableModules($matchingRecords);

        if(getenv('INSTANCE_NAME') == 'sirva') {
            foreach(['Contacts','Opportunities'] as $sortedKey) {
                if(isset($matchingRecordsList[$sortedKey])) {
                    $temp = $matchingRecordsList[$sortedKey];
                    unset($matchingRecordsList[$sortedKey]);
                    $matchingRecordsList = array_merge([$sortedKey => $temp],$matchingRecordsList);
                    unset($temp);
                }
            }
        }
