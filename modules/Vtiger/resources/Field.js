...
jQuery.Class("Vtiger_Field_Js",{
	...
	getUiTypeModel : function() {
		...
		if(type == 'referencemultipicklistall' || type == 'personnelpicklist'){
			type = 'referencemultipicklist';
		}
...
Vtiger_Field_Js('Vtiger_Referencemultipicklist_Field_Js',{},{
    /**
     * Function to get the pick list values
     * @return <object> key value pair of options
     */
     ...
     */
    getUi : function() {
        var html = '<input class="row-fluid autoComplete select2"  name="'+ this.getName() +'" value="'+this.getValue()+'"/>';
...
Vtiger_Field_Js('Vtiger_Multiagent_Field_Js',{},{
	/**
	 * Function to get the pick list values
	 * @return <object> key value pair of options
	 */
...
//custom agentpicklist base js, copied/modified from owner base js
Vtiger_Field_Js('Vtiger_Agentpicklist_Field_Js',{},{
	...
	getUi : function() {
		...
		for(var optGroup in pickListValues){
			...
			for(var option in optionGroupValues) {
				html += '<option value="'+option+'" ';
				//comparing with the value instead of key , because saved value is giving username instead of id.
				// WRONG: the saved value must be the key, if it is not, then the responsible tpl is wrong
				// this is because the value (which is the key here) is used to populate queries
				if(option == selectedOption) {
					html += ' selected ';
				}
				html += '>'+optionGroupValues[option]+'</option>';
