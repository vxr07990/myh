 ...
 * All Rights Reserved.
 *************************************************************************************/
$(document).on('keydown', 'input[int-only]', function (e) {
...
$(document).on('keydown', 'input[limit]', function (e) {
	// Allow: backspace, delete, tab, escape, enter and .
	if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
			// Allow: Ctrl+A
		(e.keyCode == 65 && e.ctrlKey === true) ||
	...
	if ($(this).val().length+1>$(this).attr('limit')) {
		e.preventDefault();
	}
});


Vtiger_Base_Validator_Js("Vtiger_PhoneValidator_Js",{
	...
	invokeValidation: function(field, rules, i, options){
		var phoneInstance = new Vtiger_PhoneValidator_Js();
	...
	validate: function(){
		...
		var strippedValue = fieldValue.replace(/\D/g, '');

		if (fieldValue == "") {
			var errorInfo = app.vtranslate("BLANK_PHONE_NUMBER");
			this.setError(errorInfo);
			return false;

		} else if (isNaN(parseInt(strippedValue))) {
			var errorInfo = app.vtranslate("INVALID_PHONE_NUMBER");
			this.setError(errorInfo);
			return false;

		} else if (strippedValue.length != 10 && strippedValue.length !== 7) {
			var errorInfo = app.vtranslate("INVALID_PHONE_NUMBER");
			this.setError(errorInfo);
			return false;
		}
	...
	getEmptyPhoneNumberError: function(){
		//let the required field handle this
		this.error = null;
		return null;
	},
...
Vtiger_Base_Validator_Js("Vtiger_greaterThanDependentField_Validator_Js",{
	...
	invokeValidation: function(field, rules, i, options){
		...
		} else if (jQuery(fieldForValidation).attr('name') == 'end_date'){
			var dependentFieldList = new Array('begin_date');
		}
	...
	validate: function(dependentFieldList){
		...
		for(var i=0; i<dependentFieldList.length; i++){
			...
			if(dependentFieldInContext.length > 0){
				...
				var dependentFieldDateInstance;
				try{
					dependentFieldDateInstance = Vtiger_Helper_Js.getDateInstance(fieldValue,dateFormat);
				}
				catch(err){
					this.setError(err);
					return false;
...
/**
 * jgrifin is adding this because calendar calls the date field a fieldtype datetime
 * I think this is better than doing an if else in BaseValidator.js
 * And now it no longer does, but if something else does i'm leaving it!
 */
Vtiger_Base_Validator_Js("Vtiger_Datetime_Validator_Js",{
	 ...
	 */
	invokeValidation: function(field, rules, i, options){
		var dateValidatorInstance = new Vtiger_Datetime_Validator_Js();
		dateValidatorInstance.setElement(field);
		var response = dateValidatorInstance.validate();
		if(response != true){
			return dateValidatorInstance.getError();
...
Vtiger_greaterThanDependentField_Validator_Js("Calendar_greaterThanDependentField_Validator_Js",{},{
	...
	getDateTimeInstance : function(field) {
		...
		var returnValue;
		try{
			returnValue = Vtiger_Helper_Js.getDateInstance(dateFieldValue,dateFormat);
		}
		catch(err){
			this.setError(err);
			return false;
...
//This needs to be below Vtiger_Email_Validator_Js to be it's child.
