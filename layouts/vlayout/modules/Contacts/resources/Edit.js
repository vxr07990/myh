...
Vtiger_Edit_Js("Contacts_Edit_Js",{},{
    ...
    registerContactTypeChange : function() {
	    var thisInstance = this;
	    var fieldMap = {"Transferee" : "customer_number", "Account" : "account_id", "Vanline" : "vanlines", "Agent" : "agents"};
	    jQuery('[name="contact_type"]').on('change', function() {
            thisInstance.toggleFieldVisibility(jQuery(this).val(), fieldMap);
        });
    },

    toggleFieldVisibility : function(typeValue, fieldMap) {
	    var fieldName = fieldMap[typeValue];
	    var field = jQuery("[name='"+fieldName+"']");
	    var fieldTd = field.closest('td');
	    if(fieldTd.hasClass('hide')) {
	        fieldTd.removeClass('hide');
	        fieldTd.prev('td').removeClass('hide');
        } else {
	        fieldTd.addClass('hide');
	        fieldTd.prev('td').addClass('hide');
        }
        ...
        for (var key in fieldMap) {
	        if(fieldMap.hasOwnProperty(key)) {
	            if(key != typeValue) {
                    ...
                    field = jQuery("[name='" + fieldName + "']");
                    fieldTd = field.closest('td');
                    fieldTd.addClass('hide');
                    fieldTd.prev('td').addClass('hide');
                }
            }
	...
	registerBasicEvents : function(container){
		...
		//testing this
		this.initializeAddressAutofill('Contacts');
		this.initializeReverseZipAutoFill('Contacts');
		this.registerContactTypeChange();
		//end testing
		loadBlocksByBusinesLine('Contacts', 'contact_type');
	}
