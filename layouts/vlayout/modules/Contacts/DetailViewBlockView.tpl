	...
	<table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if}">
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			...
			{elseif $FIELD_MODEL->get('uitype') eq "69" || $FIELD_MODEL->get('uitype') eq "105"}
				...
				{assign var=COUNTER value=$COUNTER+1}
			{else}
				{if $FIELD_NAME neq 'primary_phone_ext'}
                    ...
                    {if $BLOCK_LABEL_KEY neq 'LBL_CUSTOM_INFORMATION'}
                        ...
                        {else}
                         ...
                         {/if}
                    {/if}
					 <td class="fieldLabel {$WIDTHTYPE} {if $BLOCK_LABEL_KEY eq 'LBL_CUSTOM_INFORMATION' AND $FIELD_NAME neq 'contact_type' AND $TYPE_FIELD_MAP[$FIELD_NAME] neq $CONTACT_TYPE}hide{/if}" id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
					 ...
					 <td class="fieldValue {$WIDTHTYPE} {if $BLOCK_LABEL_KEY eq 'LBL_CUSTOM_INFORMATION' AND $FIELD_NAME neq 'contact_type' AND $TYPE_FIELD_MAP[$FIELD_NAME] neq $CONTACT_TYPE}hide{/if}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
