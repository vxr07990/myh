...
Vtiger_Edit_Js("Reports_Edit_Js",{
	...
	changeReportFolderByOwner : function (form) {
	var ownerid = form.find('#agentid').val();
	var folderid = form.find('#folderid').val();
		if(ownerid){
			...
			AppConnector.request(params).then(function (data) {
				if(data.result){
					var newOptions = '';
					var listOptions = data.result;
					var folderid = form.find('#folderid').val();
					jQuery.each(listOptions, function (id, value) {
						if(value['folderid'] == folderid){
							newOptions +='<option selected value="'+value['folderid']+'">'+value['foldername']+'</option>';
						}else{
							newOptions +='<option value="'+value['folderid']+'">'+value['foldername']+'</option>';
						}
					});
				}else{
				...
				var element = form.find('select[name="folderid"]');
				element.html(newOptions);
				element.trigger("liszt:updated");

			});
		}
		form.find('[name="agentid"]').on('change',function(e){
	...
	registerEvents : function(){
		...
		this.changeReportFolderByOwner(form);
	}
});
