...
{strip}
	<form class="form-horizontal recordEditView" id="report_step2" method="post" action="index.php">
		...
		<input type="hidden" name="agentid" value="{$REPORT_MODEL->get('agentid')}" />
				...
				<div class="row-fluid padding1per">
					<select data-placeholder="{vtranslate('LBL_ADD_MORE_COLUMNS',$MODULE)}" id="reportsColumnsList" class="select2-container span11 columns" multiple="">
						...
						{foreach key=SECONDARY_MODULE_NAME item=SECONDARY_MODULE from=$SECONDARY_MODULE_FIELDS}
							...
							{/foreach}
						{/foreach}
						{if getenv('INSTANCE_NAME')=='graebel' && ($PRIMARY_MODULE_NAME == 'Actuals' || $SECONDARY_MODULE_NAME == 'Actuals')}
							<optgroup label="(Line Items)">
								<option {if array_key_exists('Ready to Invoice', $SELECTED_FIELDS)}selected{/if} value="vtiger_detailed_lineitems:dli_ready_to_invoice:Ready to Invoice:dli_ready_to_invoice:V">Ready to Invoice</option>
								<option {if array_key_exists('Invoice Number', $SELECTED_FIELDS)}selected{/if} value="vtiger_detailed_lineitems:dli_invoice_number:Invoice Number:dli_invoice_number:V">Invoice Number</option>
							</optgroup>
						{/if}
					</select>
				</div>
