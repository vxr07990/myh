		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $COUNTER eq 2}
                            ...
                            <tr{if ($FIELD_NAME eq 'agency_related' || $FIELD_NAME eq 'vanline_related')} class="hide"{/if}>
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
								{if (
								$FIELD_NAME eq 'agency_code' ||
								$FIELD_NAME eq 'brand' ||
								$FIELD_NAME eq 'source_type' ||
								$FIELD_NAME eq 'agentid'
								)}
									{*
									 * these are readonly fields but we want to have a format like the other forms.
									 * Otherwise we could use the stringDetailView. commented out below
									 * I added _ignored tothe hidden input names, to indicate they are unused.
									 *}
									{assign var="FIELD_INFO" value=Vtiger_Util_Helper::toSafeHTML(Zend_Json::encode($FIELD_MODEL->getFieldInfo()))}
									{assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
									{assign var="FIELD_NAME" value=$FIELD_MODEL->get('name')}
									<input type="hidden" id="hiddenFields" name="{$FIELD_NAME}" value="{$FIELD_MODEL->get('fieldvalue')}" />
									<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" class="input-large {if $FIELD_MODEL->isNameField()}nameField{/if}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}_ignored" value="{$FIELD_MODEL->getDisplayValue($FIELD_MODEL->get('fieldvalue'), $RECORD_ID, $RECORD_MODEL)}" readonly data-fieldinfo='{$FIELD_INFO}' {if !empty($SPECIAL_VALIDATOR)}data-validator={Zend_Json::encode($SPECIAL_VALIDATOR)}{/if} />
									{*
