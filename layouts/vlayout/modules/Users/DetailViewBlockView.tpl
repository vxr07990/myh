        ...
        <table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if}">
            ...
            <tr>
                <th class="blockHeader" colspan="4">
                    ...
                    {if $BLOCK_LABEL_KEY eq 'LBL_USERS_EXCHANGE'}
            ...
            <tr>
                ...
                {if ($FIELD_NAME == 'vehicles_edit_permission' || $FIELD_NAME == 'drivers_edit_permission') && !$CURRENT_USER_MODEL->isAdminUser()}
                    {continue}
                {/if}
                ...
                {/if}
                {/if}
                {if $FIELD_NAME eq 'cf_oa_da_coordinator'}
                    {assign var=ROLE_FIELD_MODEL value=$FIELD_MODEL_LIST['roleid']}
                    {if $ROLE_FIELD_MODEL->get('fieldvalue') neq 'H9' && $ROLE_FIELD_MODEL->get('fieldvalue') neq 'H10' && $ROLE_FIELD_MODEL->get('fieldvalue') neq 'H11'}
                        {continue}
                    {/if}
                {/if}
