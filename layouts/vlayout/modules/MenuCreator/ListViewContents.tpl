		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}" {if isParticipantForRecord($LISTVIEW_ENTRY->getId())}style="background-color: #ffff99"{/if}>
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
				...
				<div class="actions pull-right">
					{if ($LISTVIEW_ENTRY->get("description") eq 'DEFAULT_MENU' && $CURRENT_USER_MODEL->isAdminUser()) || $LISTVIEW_ENTRY->get("description") neq 'DEFAULT_MENU'}
						<span class="actionImages">
							...
							{if $IS_MODULE_EDITABLE}{* && $CURRENT_USER_MODEL->getExtraPermission($LISTVIEW_KEY)*}
								...
								{/if}
							{/if}
							{if $IS_MODULE_DELETABLE && $LISTVIEW_ENTRY->get("description") neq 'DEFAULT_MENU'}{* && $CURRENT_USER_MODEL->getExtraPermission($LISTVIEW_KEY)*}
