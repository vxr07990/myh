...
{strip}
	...
	<table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if}">
		...
		{if not $BLOCK->get('hideblock')}
			<br>
			{include file=vtemplate_path('MenuEditorDetail.tpl', 'MenuCreator')}
			<br>
		{/if}
		
		{if $MENUGROUPS_MODULE_MODEL && $MENUGROUPS_MODULE_MODEL->isActive()}
			{include file=vtemplate_path('MenuGroupsDetail.tpl', 'MenuGroups')}
		{/if}
	{/foreach}
