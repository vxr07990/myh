...
{strip}
	...
	{if $BLOCK_LABEL_KEY neq 'LBL_COMMISSION_PLAN_ITEMS'}
	...
	<table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if}">
		...
		{/if}
         {if isset($SINGLE_FIELDS) && in_array($FIELD_NAME, $SINGLE_FIELDS)}
             ...
             <tr>
            {*This will start the next row because COUNTER=1*}
            {assign var=COUNTER value=1}
         {/if}
		{/foreach}
	...
	{else}
		{include file=vtemplate_path('CommissionPlansItemDetail.tpl', 'CommissionPlansItem') GUEST_MODULE = 'CommissionPlansItem'}
	{/if}
	{/foreach}
{/strip}
