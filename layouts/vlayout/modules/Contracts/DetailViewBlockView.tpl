...
{strip}
	...
	{if $BLOCK_LABEL_KEY eq 'LBL_CONTRACTS_ADMINISTRATIVE'}{include file="VanlineAgentTablesDetail.tpl"|@vtemplate_path:$MODULE_NAME}{/if}
	{if $BLOCK_LABEL_KEY eq 'LBL_CONTRACTS_ANNUALRATE' && $ANNUAL_RATES|@count gt 0}{include file="AnnualRateTableDetail.tpl"|@vtemplate_path:$MODULE_NAME}{/if}
    {* @NOTE: Enjoy! *}
    {if getenv('INSTANCE_NAME') neq 'sirva'}
        {if $BLOCK_LABEL_KEY eq 'LBL_CONTRACTS_FUEL_SURCHARGE_TABLE'}
			{include file=vtemplate_path('FuelSurchargeDetail.tpl',$MODULE) COUNTER=$COUNTER}
            {continue}
        {/if}
        {if $BLOCK_LABEL_KEY eq 'LBL_CONTRACTS_MISC_ITEMS'}
			{include file=vtemplate_path('MiscItemsDetail.tpl',$MODULE) COUNTER=$COUNTER}
            {continue}
        {/if}
        {if $BLOCK_LABEL_KEY eq 'LBL_CONTRACTS_FLAT_RATE_AUTO'}
            {include file=vtemplate_path('AutoRateTableDetail.tpl',$MODULE) COUNTER=$COUNTER}
            {continue}
        {/if}
    {/if}
	...
	<table name="{$BLOCK_LABEL_KEY}" class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if}">
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			{if $FIELD_NAME eq 'valuation_deductible'}
				{assign var=VALUATION_DEDUCTIBLE value=$FIELD_MODEL->get('fieldvalue')}
			{/if}
			{if $VALUATION_DEDUCTIBLE eq ''}
				{if (
				$FIELD_NAME eq 'maximum_rvp' ||
				$FIELD_NAME eq 'rvp_flat_fee' ||
				$FIELD_NAME eq 'rvp_per_1000' ||
				$FIELD_NAME eq 'rvp_per_1000_sit'
				)}
					{continue}
				{/if}
			{elseif $VALUATION_DEDUCTIBLE && $VALUATION_DEDUCTIBLE eq 'Replacement Value Protection'}
				{if (
				$FIELD_NAME eq 'free_frv' ||
				$FIELD_NAME eq 'free_frv_amount'
				)}
					{continue}
				{/if}
			{else}
				{if (
				$FIELD_NAME eq 'min_val_per_lb' ||
				$FIELD_NAME eq 'free_frv' ||
				$FIELD_NAME eq 'free_frv_amount' ||
				$FIELD_NAME eq 'maximum_rvp' ||
				$FIELD_NAME eq 'rvp_flat_fee' ||
				...
				{/if}
			{/if}
			{assign var=HIDE_PARENT value=0}
			{if ($IS_PARENT)}
				{if
				($FIELD_NAME eq 'parent_contract') ||
				($FIELD_NAME eq 'related_tariff')
				}
					{assign var=HIDE_PARENT value=1}
                {/if}
			{/if}
		<!-- {$FIELD_NAME} -->
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldLabel {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
					 <label class="muted pull-right marginRight10px{if $HIDE_PARENT} hide{/if}">
						 {if ($IS_PARENT && $FIELD_NAME eq 'contract_no')}
							 {vtranslate('LBL_CONTRACTS_PARENT',{$MODULE_NAME})}
                         {else}
							 {vtranslate({$FIELD_MODEL->get('label')},{$MODULE_NAME})}
						 {/if}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					 <span class="value{if $HIDE_PARENT} hide{/if}" data-field-type="{$FIELD_MODEL->getFieldDataType()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $FIELD_MODEL->get('uitype') eq '21'} style="white-space:normal;" {/if}>
