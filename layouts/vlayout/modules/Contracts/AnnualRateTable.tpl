{strip}
	...
	<table class="table table-bordered blockContainer showInlineTable equalSplit">
		<thead>
			<tr>
				<th class="blockHeader" colspan="8">{vtranslate("LBL_CONTRACTS_ANNUALRATE", $MODULE)}</th>
			</tr>
		</thead>
		<tbody id='annualRateIncreaseTable'>
			...
			<tr>
				...
				<td class="blockHeader" style='width:48%;text-align:center;'>{vtranslate("LBL_CONTRACTS_FROMDATE", $MODULE)}</td>
				<td class="blockHeader" style='width:48%;text-align:center;'>{vtranslate("LBL_CONTRACTS_PERCINCREASE", $MODULE)}</td>
			</tr>
			<tr class="defaultAnnualRate hide" id="annualRateRow">
				...
				<td class="fieldValue {$WIDTHTYPE}" style='width:48%;text-align:center'>
				   {assign var="dateFormat" value=$USER_MODEL->get('date_format')}
					...
					{$LOCALFIELDINFO.label = 'LBL_CONTRACTS_ANNUALRATEDATE'}
				...
				<td class="fieldValue {$WIDTHTYPE}" style='width:48%;text-align:center'>
					...
					{$LOCALFIELDINFO.label = 'LBL_CONTRACTS_ANNUALRATEINCREASE'}
		...
		{foreach key=ROW_NUM item=ANNUAL_RATE from=$ANNUAL_RATES}
			<tr class="annualRate" id="annualRateRow{$ROW_NUM+1}">
				...
				<td class="fieldValue {$WIDTHTYPE}" style='width:48%;text-align:center'>
					{$LOCALFIELDINFO.mandatory = false}
					{$LOCALFIELDINFO.name = 'annual_rate_increase'}
					{$LOCALFIELDINFO.label = 'annual_rate_increase'}
					{$LOCALFIELDINFO.type = 'percentage'}
					{$INFO = Vtiger_Util_Helper::toSafeHTML(Zend_Json::encode($LOCALFIELDINFO))}
