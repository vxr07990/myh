                ...
                <tbody>
                    {foreach item=RESOURCE from=$RESOURCE_LIST}
                        ...
                        <tr>
                            ...
                            <td>{$RESOURCE.type}</td>
                            <td><input autocomplete="off" style="width:45px" type="text" name="qty" id="qty_{$RESOURCE.id}" value="{$RESOURCE.allocatedqty}"></input></td>
                        </tr>
                    {/foreach}
                </tbody>
            </table>
        ...
        $(document).ready(function() {
            ...
            $('#button').click(function() {
                ...
                if (Object.keys(selected_ids).length > 0) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "index.php?module=ResourceDashboard&action=UpdateAssignResourcesToTask",
                        dataType: "json",
                        data: {
                        ...
                        success: function(msg) {
                            if (msg.success && msg.result['result'] == 'works') {
                                ...
                                var qty = msg.result['qty'];
                                window.location.href = url[0] + "&allocatedqty=" + qty + "&result=ok";
