...
<div class="listViewEntriesDiv contents-bottomscroll">
	...
	<table class="table table-bordered listViewEntriesTable" style="width:100%;">
		<thead>
			<tr class="listViewHeaders">
				...
				<th nowrap {if $LISTVIEW_HEADER@last} colspan="2" {/if}>
					<a href="javascript:void(0);" class="listViewHeaderValues" data-nextsortorderval="{if $COLUMN_NAME eq $LISTVIEW_HEADER->get('column')}{$NEXT_SORT_ORDER}{else}ASC{/if}" data-columnname="{$LISTVIEW_HEADER->get('column')}">{vtranslate($LISTVIEW_HEADER->get('label'), $LISTVIEW_HEADER->getModuleName())}
		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
                    <td style="min-width: 40px;" class="{$WIDTHTYPE}">
                        <input type="checkbox" value="{$LISTVIEW_ENTRY->getId()}" class="listViewEntriesCheckBox"/>
                        <img style="margin-left:10px;cursor:pointer;" class="imageElement" data-rightimage="layouts/vlayout/skins/images/rightArrowWhite.png" data-downimage="layouts/vlayout/skins/images/downArrowWhite.png" src="layouts/vlayout/skins/images/rightArrowWhite.png">
                    </td>
                    {foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
...
{/strip}
<div id="tabla_auxiliar" style="display:none;">
    <table class="table table-bordered listViewEntriesTable">
        <thead>
            <tr class="listViewHeaders">
                <th nowrap="">{vtranslate('PU Date',$MODULE)}</th>
                <th nowrap="">{vtranslate('Account Name',$MODULE)}</th>
                <th nowrap="">{vtranslate('Shipper Last Name',$MODULE)}</th>
                <th nowrap="">{vtranslate('Shipper First Name',$MODULE)}</th>
                <th nowrap="">{vtranslate('Order No',$MODULE)}</th>
                <th nowrap="">{vtranslate('Est. Weight',$MODULE)}</th>
                <th nowrap="">{vtranslate('Actual Weight',$MODULE)}</th>
                <th nowrap="">{vtranslate('Total Linehaul',$MODULE)}</th>
                <th nowrap="">{vtranslate('RD Date',$MODULE)}</th>
                <th nowrap="">{vtranslate('Origin Code',$MODULE)}</th>
                <th nowrap="">{vtranslate('Hauler Code',$MODULE)}</th>
                <th nowrap="">{vtranslate('Origin City',$MODULE)}</th>
                <th nowrap="">{vtranslate('Origin State',$MODULE)}</th>
                <th nowrap="">{vtranslate('Dest Agent',$MODULE)}</th>
                <th nowrap="">{vtranslate('Dest City',$MODULE)}</th>
                <th nowrap="">{vtranslate('Dest State',$MODULE)}</th>
                <th nowrap="">{vtranslate('Delivery Type',$MODULE)}</th>
                <th nowrap="">{vtranslate('Delivery',$MODULE)}</th>
                <th nowrap="">{vtranslate('Delivery Status',$MODULE)}</th>
                <th nowrap="">{vtranslate('Actual Weight Gross',$MODULE)}</th>
                <th nowrap="">{vtranslate('Actual Weight Tare',$MODULE)}</th>
                <th nowrap="">{vtranslate('Pro Gear',$MODULE)}</th>
                <th nowrap="">{vtranslate('Pro Gear Weight',$MODULE)}</th>
            </tr>
        </thead>
        <tbody>
