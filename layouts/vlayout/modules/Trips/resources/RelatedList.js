...
Vtiger_RelatedList_Js("Trips_RelatedList_Js",{},{
	...
	showSelectRelationPopup : function(){      
	    ...
	    var driverID = (jQuery("#Trips_detailView_fieldLabel_driver_id").next("td").text().trim() !== "") ? jQuery("#Trips_detailView_fieldValue_driver_id a").attr("href").split("record=")[1] : "";
	    var urlParams = {
		module: 'Trips',
		action: 'TripsActions',
		mode: 'checkDriverOOS',
		...
		if(data && data.result == "OK"){
		    popupInstance.show(thisInstance.getPopupParams(), function(responseString){
			var responseData = JSON.parse(responseString);
			var relatedIdList = Object.keys(responseData);
			thisInstance.addRelations(relatedIdList).then(
			    function(data){
				window.location.href = "index.php?module=Trips&view=Detail&record=" + jQuery('#recordId').val();;

			    }
			);
		    });
		}else{
		    Vtiger_Helper_Js.showPnotify(app.vtranslate('Driver is Out of Service!'));
		}
	    });
            return aDeferred.promise();
	},
	...
	getPopupParams : function(){
		...
		var parameters = {
			...
			'multi_select' : true,
                        'popup_type': 'trip_orders_related'
		}
		return parameters;
	},
