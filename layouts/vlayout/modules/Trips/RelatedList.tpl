                ...
                <div class="span6">
                    ...
                    {foreach item=RELATED_LINK from=$RELATED_LIST_LINKS['LISTVIEWBASIC']}
                        <div class="btn-group">
                            ...
                            {if $IS_SELECT_BUTTON eq true} selectRelation {/if} {if $RELATED_MODULE_NAME neq "TripsDriverCheckin"} hide {/if}"
                            ...
                            {if $HEADER_FIELD@last}
                                ...
                                <div class="pull-right actions">
                                    <span class="actionImages">
                                        {if $RELATED_MODULE_NAME eq 'Calendar'}
                                            {if $IS_EDITABLE && $RELATED_RECORD->get('taskstatus') neq 'Held' && $RELATED_RECORD->get('taskstatus') neq 'Completed'}{* && $USER_MODEL->getExtraPermission($RELATED_KEY)*}
                                                <a class="markAsHeld"><i title="{vtranslate('LBL_MARK_AS_HELD', $MODULE)}" class="icon-ok alignMiddle"></i></a>&nbsp;
                                            {/if}
                                            {if $IS_EDITABLE && $RELATED_RECORD->get('taskstatus') eq 'Held'}{* && $USER_MODEL->getExtraPermission($RELATED_KEY)*}
                                        ...
                                        {if $IS_EDITABLE}{* && $USER_MODEL->getExtraPermission($RELATED_KEY)*}
                                            ...
                                            {else}
												{if isPermitted($RELATED_MODULE->get('name'), 'EditView', $RELATED_RECORD->getId()) eq 'yes'}
													<a href='{$RELATED_RECORD->getEditViewUrl()}'><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="icon-pencil alignMiddle"></i></a>&nbsp;
                                                    {if $RELATED_MODULE_NAME neq 'Orders'}
													    <span class='duplicateSpan' data-url="{$RELATED_RECORD->getDuplicateUrl()}"><img src="layouts/vlayout/skins/images/duplicate.png" class="alignMiddle"/></span>&nbsp;
                                                    {/if}
                                                {/if}
											{/if}
                                        {/if}
                                        {if $IS_DELETABLE}{* && $USER_MODEL->getExtraPermission($RELATED_KEY)*}
                                            {if $RELATED_MODULE_NAME eq 'Calendar'}
                                                ...
                                                {/if}
                                            {else if $RELATED_MODULE_NAME eq 'Orders'}
                                                {if isPermitted($RELATED_MODULE->get('name'), 'Delete', $RELATED_RECORD->getId()) eq 'yes'}
													<a class="relationDelete" data-relatedmodule="Orders"><i title="{vtranslate('LBL_REMOVE', $MODULE)}" class="icon-trash alignMiddle"></i></a>
												{/if}
                                            {else}
												{if isPermitted($RELATED_MODULE->get('name'), 'Delete', $RELATED_RECORD->getId()) eq 'yes'}
													<a class="relationDelete"><i title="{vtranslate('LBL_DELETE', $MODULE)}" class="icon-trash alignMiddle"></i></a>
