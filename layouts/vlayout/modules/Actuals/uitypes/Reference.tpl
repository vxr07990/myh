...
{if {$REFERENCE_LIST_COUNT} gt 1}
	...
	{/if}
{/if}
{if $FIELD_NAME eq 'potential_id' and $SOURCE_MODULE eq 'Opportunities'}
	<input name="{$FIELD_MODEL->getFieldName()}" type="hidden" value="{$SOURCE_RECORD}" class="sourceField" data-displayvalue='{$OPP_NAME}' data-fieldinfo='{$FIELD_INFO}' />
{else}
...
{if $FIELD_NAME eq 'potential_id' and $SOURCE_MODULE eq 'Opportunities'}
	<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" class="{if (($smarty.request.view eq 'Edit') or ($smarty.request.module eq 'Webforms'))} span7 {else} span8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
	 value="{$OPP_NAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"
	 data-fieldinfo='{$FIELD_INFO}' placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}"
	 {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} {if $OPP_NAME}readonly="readonly"{/if}/>
{else}
	<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" class="{if (($smarty.request.view eq 'Edit') or ($smarty.request.module eq 'Webforms'))} span7 {else} span8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
...
{assign var=QUICKCREATE_RESTRICTED_MODULES value=['SalesOrder','Quotes','Invoice','PurchaseOrder', 'Contacts','Accounts', 'Orders']}
