...
{if $RELATED_MODULE_NAME eq 'WFImages'}
    {strip}
                    ...
                    {foreach item=RELATED_LINK from=$RELATED_LIST_LINKS['LISTVIEWBASIC']}
                        <div class="btn-group">
                            ...
                            {if $IS_SELECT_BUTTON eq true} selectRelation {/if} "
                                ...
                                {/if}
                                {if $IS_SELECT_BUTTON neq true}name="addButton"{/if}>{if $IS_SELECT_BUTTON eq false}<i
                                class="icon-plus icon-white"></i>{/if}&nbsp;<strong>{$RELATED_LINK->getLabel()}</strong>
                            </button>
                        </div>
                    {/foreach}
                    &nbsp;
                    ...
                    {foreach item=RELATED_RECORD from=$RELATED_RECORDS key=RELATED_KEY}
                        ...
                        >
                            ...
                            <td class="{$WIDTHTYPE}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
                                {if $RELATED_HEADERNAME eq 'imagename'}
                                <div class='row-fluid'>
                                        <div class='span6'>
                                            {assign var=IMAGE_DETAILS value=$RELATED_RECORD->getImageDetails()}
