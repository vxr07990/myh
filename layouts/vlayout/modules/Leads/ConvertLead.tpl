...
{strip}
	...
	{else}
		...
		<form class="form-horizontal" id="convertLeadForm" method="post" action="index.php">
			...
			<div id="potentialName" data-name="{$POTIENTIAL_NAME}"></div>
			<div class="modal-body accordion" id="leadAccordion">
					...
					<div class="accordion-group convertLeadModules">
						<div class="header accordion-heading">
							<div data-parent="#leadAccordion" data-toggle="collapse" class="accordion-toggle table-bordered moduleSelection" href="#{$MODULE_NAME}_FieldInfo">
								...
								<span class="pull-right"><i class="iconArrow{if $CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Accounts" && $COMPANY_NAME neq ''} icon-inverted icon-chevron-up {elseif (!$CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Contacts") || ($COMPANY_NAME eq '' && $MODULE_NAME eq "Opportunities")} icon-inverted icon-chevron-up {else} icon-inverted icon-chevron-down {/if}alignBottom"></i></span>
							</div>
						</div>
						<div id="{$MODULE_NAME}_FieldInfo" class="{$MODULE_NAME}_FieldInfo accordion-body collapse fieldInfo {if $CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Accounts" && $COMPANY_NAME neq ''} in {elseif $MODULE_NAME == "Contacts" || $MODULE_NAME eq "Opportunities"} in {/if}">
								...
								{foreach item=FIELD_MODEL from=$MODULE_FIELD_MODEL}
									<tr>
										...
										<td class="fieldValue">
											...
											{*changed to this version so it pulls opportunities version of the template*}
				...
				<div class="overflowVisible">
					<table class="table table-bordered">
							...
							<tr>
								<td class="fieldLabel">
									<label class='muted pull-right'>
										<span class="redColor">*</span> {vtranslate($FIELD_MODEL->get('label'), $MODULE_NAME)} <!-- {$MODULE_NAME} -->
						...
						{else}
							<input type="hidden" value="{$ASSIGN_TO}" name="assigned_user_id">
						{/if}
						<tr>
							<td class="fieldLabel">
								<label class='muted pull-right'>
