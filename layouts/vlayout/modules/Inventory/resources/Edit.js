...
/**
	...
	setGrandTotal : function(grandTotalValue) {
        ...
        //set the total to the total variable so that it will save the displayed grandtotal when the page is saved.
        var totalElement = jQuery('[name="total"]');
        if (totalElement != 'undefined') {
            totalElement.val(grandTotalValue);
        }
		return this;
	},
	...
	registerLineItemActionSaveEvent : function(){
		...
		editForm.on('click','button[name="lineItemActionSave"]',function(){
			...
			if (jQuery('button[name="lineItemActionSave"]').closest('.validCheck').hasClass("hide")) {
				//if the whole block is hidden just return, so we skip it from triggering a click.
				//The reasoning is that it takes 150-600ms to do the operation and since this runs
				//at submit and there's like dozens of them, it's adding up.
				return false;
			} else {
				...
				if (!match) {
					...
					//this trigger's validation to occur
...
function getRateQuote(module, block) {
	if(module === 'Quotes' && block === 'Interstate Move Details') {
		var fieldNames = ['weight', 'pickup_date', 'pickup_time', 'cf_905', 'cf_907', 'fuel_price', 'full_pack', 'full_unpack', 'bottom_line_discount', 'valuation_deductible', 'valuation_amount'];
		...
		var fuelPrice = jQuery('input[name="'+fieldNames[5]+'"]').val();
		...
		if (Estimates_Edit_Js.I().moduleName == 'Actuals') {
			...
			if (deliveryDate.length != 10) {
				errorString += errorNum + ") A valid delivery date must be set\n";
				errorExists = true;
				errorNum++;
			}
		}
		...
		var currentTd = jQuery('input[name="rate_estimate"]').closest('td');
		currentTd.progressIndicator();
		jQuery('input[name="rate_estimate"]').closest('div').addClass('hide');
		...
		var dataURL = "index.php?module=Quotes&action=GetRateEstimate&record="+getQueryVariable("record")+"&weight="+weight+"&pickupDateTime="+pickupDateTime+"&originZip="+originZip+"&destinationZip="+destinationZip+"&fuelPrice="+fuelPrice+"&fullPackApplied="+fullPackApplied+"&fullUnpackApplied="+fullUnpackApplied+"&bottomLineDiscount="+bottomLineDiscount+"&valDeductible="+valDeductibleValue+"&valuationAmount="+valuationAmount;
		AppConnector.request(dataURL).then(
			function(data) {
				if(data.success) {
					jQuery('input[name="rate_estimate"]').val(parseFloat(data.result.rateEstimate).toFixed(2));
					currentTd.progressIndicator({'mode':'hide'});
					jQuery('input[name="rate_estimate"]').closest('div').removeClass('hide');
				}
			},
			function(error, err) {
				alert(error + ": " + err);
