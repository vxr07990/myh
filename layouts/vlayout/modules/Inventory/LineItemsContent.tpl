...
{strip}
	...
	<td>
		...
		&nbsp;<a {if $MODULE eq "Quotes"}class='hide'{/if}><img src="{vimage_path('drag.png')}" border="0" title="{vtranslate('LBL_DRAG',$MODULE)}"/></a>
	...
	<td>
		...
		{else}
			<div><br><textarea id="{$comment}" name="{$comment}" class="lineItemCommentBox {if $MODULE eq "Quotes"}hide{/if}">{$data.$comment}</textarea>
		{/if}
	</td>
	<td {if $MODULE eq "Quotes"}class='hide'{/if}>
