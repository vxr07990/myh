...
<div class='editViewContainer container-fluid'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			{if $MODULE_NAME eq "Quotes" and ($BLOCK_LABEL eq "LBL_QUOTES_VALUATIONDETAILS" or $BLOCK_LABEL eq "LBL_QUOTES_SITDETAILS" or $BLOCK_LABEL eq "LBL_QUOTES_ACCESSORIALDETAILS")}{continue}{/if}
			{if $BLOCK_FIELDS|@count lte 0}{continue}{/if}
			<table name='{$BLOCK_LABEL}' class="table table-bordered blockContainer showInlineTable{if in_array($BLOCK_LABEL, $HIDDEN_BLOCKS)} hide{/if}">
			<tr>
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				<td class="fieldLabel {$WIDTHTYPE}">
					{if $FIELD_NAME neq 'interstate_mileage'}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if} {if $FIELD_MODEL->get('uitype') eq '20'} colspan="3"{/if}>
						{if $FIELD_NAME eq 'interstate_mileage'}<span class='hide'>{/if}
						{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
						{if $FIELD_NAME eq 'interstate_mileage'}</span>{/if}
					</td>
				{/if}
		...
		{if $MODULE_NAME eq "Quotes"}
			<!-- BEGIN RateEstimateEdit -->
			{include file='layouts/vlayout/modules/Quotes/RateEstimateEdit.tpl'}
		{/if}
{/strip}