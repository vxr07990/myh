		...
		{if $RELATED_MODULE_MODEL}
			...
			{foreach from=$MANDATORY_FIELD_MODELS item=MANDATORY_FIELD_MODEL}
				...
				<div class="row-fluid conditionRow padding-bottom1per">
					...
					<span class="span3">
						<select name="modulename" class="select2"  style="width: 184px" {if ($FIELD_TYPE eq 'picklist' || $FIELD_TYPE eq 'multipicklist' || $FIELD_TYPE eq 'agentpicklist' )} disabled="" {/if}>
							<option value="{$SOURCE_MODULE}">{vtranslate($SOURCE_MODULE, $SOURCE_MODULE)}</option>
							<option {if ($FIELD_TYPE eq 'picklist' || $FIELD_TYPE eq 'multipicklist' || $FIELD_TYPE eq 'agentpicklist')} selected="" {/if} value="{$RELATED_MODULE_MODEL->get('name')}">{vtranslate($RELATED_MODULE_MODEL->get('name'),$RELATED_MODULE_MODEL->get('name'))}</option>
						</select>
					</span>
					<span class="fieldUiHolder span4">
		...
		<span class="span4">
			<select name="fieldname" style="min-width: 250px">
				...
				{foreach from=$RELATED_MODULE_MODEL->getFields() item=FIELD_MODEL}
					...
					<option value="{$FIELD_MODEL->get('name')}" data-fieldtype="{$FIELD_MODEL->getFieldType()}"  data-field-name="{$FIELD_MODEL->get('name')}" data-fieldinfo="{Vtiger_Util_Helper::toSafeHTML(json_encode($FIELD_INFO))}" >
