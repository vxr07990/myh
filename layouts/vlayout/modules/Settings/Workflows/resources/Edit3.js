...
Settings_Workflows_Edit_Js("Settings_Workflows_Edit3_Js",{},{
	...
	getValues : function(tasktype) {
		...
		conditions.each(function(i, conditionDomElement){
			...
			} else if (fieldType == 'picklist' || fieldType == 'multipicklist' || fieldType == 'agentpicklist') {
				for(var key in fieldList) {
					var field = fieldList[key];
					...
					} else if(field == 'value' && valueSelectElement.is('select') && (fieldType == 'picklist' || fieldType == 'agentpicklist')){
	...
	registerFillTaskFieldsEvent: function() {
		jQuery('#saveTask').on('change','.task-fields',function(e) {
			...
			if (currentElement.hasClass('toEmailEventOptions') && oldValue.length > 0) {
				//Added this so that there aren't extraneous commas
				newValue = oldValue+','+currentElement.val();
			}
	...
	registerInsertFieldEvent : function () {
		var thisInstance = this;
		jQuery('[name="insertfieldname"]').off('change').on('change', function() {
			var options = jQuery('[name="insertfieldname"]').find('option');
			var fn = '$' + jQuery('[name="insertfieldname"]').find('option:selected').val();
			jQuery('select[name="insertfieldname"]').val('Select Field');
			jQuery('select[name="insertfieldname"]').parent().find('span:not([class])').text('Select Field');
			jQuery('select[name="insertfieldname"]').parent().find('[class="active-result result-selected"]').removeClass('result-selected');
			jQuery('select[name="insertfieldname"]').data('selectedValue', '');
			options.prop('selected', false);
			jQuery('textarea[name="description"]').insertAtCaret(fn);
		});
	},
		...
		jQuery('#addFieldBtn').on('click',function(e) {
			...
			app.showSelect2ElementView(newAddFieldContainer.find('.select2'));
                        jQuery('#scrollContainer').animate({
                            scrollTop: newAddFieldContainer.offset().top},
                            'slow');
                        newAddFieldContainer.effect("highlight", {}, 3000);
		});
	},
		...
		jQuery('#saveTask').on('change','select[name="fieldname"]',function(e){
			...
			if(selectedElement.val() != 'none'){
				...
				if(moduleNameElement.length > 0){
					...
					if(type == 'picklist' || type == 'multipicklist' || type == 'agentpicklist'){
