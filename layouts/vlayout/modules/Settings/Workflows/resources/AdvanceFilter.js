	...
	getValues : function() {
		...
		conditionGroups.each(function(index,domElement){
			...
			conditions.each(function(i, conditionDomElement){
				...
				var rowValues = {};
				if (fieldType == 'picklist' || fieldType == 'multipicklist' || fieldType == 'custompicklist') {
...
Vtiger_Field_Js('Vtiger_Reference_Field_Js',{},{
    getUi : function() {
        // var thisInstance = this;
        // var blockReturn = true;
        // var returnValue = '';
        // var nonCsrfUrl = 'index.php?module=Workflows&parent=Settings&action=GetReferenceField&fieldModule='+jQuery('input[name="module_name"]').val()+'&fieldName='+this.getName();
        var dataURL = 'index.php';
        var workflowId = getQueryVariable('record');
        var data = 'module=Workflows&parent=Settings&action=GetReferenceField&fieldModule='+this.data.modulename+'&fieldName='+this.getName();
        if(workflowId != false) {
            data += '&workflow='+getQueryVariable('record');
        }
        // AppConnector.request(nonCsrfUrl).then(
        //     function(data) {
        //         var html = data.result;
        //         var element = jQuery(html);
        //         returnValue = thisInstance.addValidationToElement(element);
        //         blockReturn = false;
        //     }
        // );
...
Vtiger_Picklist_Field_Js('Workflows_Picklist_Field_Js',{},{
        // Commenting this time because THIS SHOULD NOT BE READDED.
        //getUi : function(){
...
Vtiger_Picklist_Field_Js('Workflows_CustomPicklist_Field_Js',{},{
