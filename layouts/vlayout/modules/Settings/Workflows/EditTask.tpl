		...
		<form class="form-horizontal" id="saveTask" method="post" action="index.php">
			...
			<div id="scrollContainer">
				<div class="modal-body tabbable">
					...
					{if $TASK_TYPE_MODEL->get('tasktypename') eq 'VTEmailTask' && $SOURCE_MODULE eq 'Documents'}
						<div class="row-fluid padding-bottom1per">
							<div class="span6"><input type="checkbox" class="alignTop" value="1" name="attach_document" {if $TASK_OBJECT->attach_document neq 0}checked{/if}/>&nbsp;{vtranslate('Attach Document as Email Attachment',$QUALIFIED_MODULE)}</div>
						</div>
					{/if}
					...
					<div class="row-fluid padding-bottom1per">
						...
						<div class="span10 row-fluid {if $trigger neq null}show {else} hide {/if}" id="checkSelectDateContainer">
							<span class="span2 row-fluid">
								<input class="span6" type="number" min="0" step="0.001" name="select_date_days" value="{$days}">&nbsp;
							...
							<span class="span6 marginLeftZero">
								<select class="chzn-select" name="select_date_field">
									{foreach from=$DATETIME_FIELDS item=DATETIME_FIELD}
										<option {if $trigger['field'] eq $DATETIME_FIELD->get('name')} selected="" {/if} value="{$DATETIME_FIELD->get('name')}">{vtranslate(vtranslate($DATETIME_FIELD->get('label'),$QUALIFIED_MODULE), $MODULE_MODEL->get('name'))}</option>
									{/foreach}
								</select>
							</span>
						</div>
