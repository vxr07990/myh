			...
			<span class="span6">
				<select class="chzn-select" name="startDatefield">
					{foreach from=$DATETIME_FIELDS item=DATETIME_FIELD}
						<option {if $TASK_OBJECT->startDatefield eq $DATETIME_FIELD->get('name')}selected{/if}  value="{$DATETIME_FIELD->get('name')}">{vtranslate($DATETIME_FIELD->get('label'),$MODULE_MODEL->get('name'))}</option>
					{/foreach}
				</select>
			</span>
		</div>
		...
		<div class="row-fluid padding-bottom1per">
			...
			<span class="span6">
				<select class="chzn-select" name="endDatefield">
					{foreach from=$DATETIME_FIELDS item=DATETIME_FIELD}
						<option {if $TASK_OBJECT->endDatefield eq $DATETIME_FIELD->get('name')}selected{/if}  value="{$DATETIME_FIELD->get('name')}">{vtranslate($DATETIME_FIELD->get('label'),$MODULE_MODEL->get('name'))}</option>
					{/foreach}
				</select>
			</span>
		</div>
