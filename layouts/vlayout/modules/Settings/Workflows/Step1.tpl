    ...
    <div class="workFlowContents" style="padding-left: 3%;padding-right: 3%">
        <form name="EditWorkflow" action="index.php" method="post" id="workflow_step1" class="form-horizontal">
            ...
            <div class="padding1per" style="border:1px solid #ccc;">
                ...
                <br>
                {if is_array($ACCESIBLE_AGENTS)}
                <div class="control-group">
                    <div class="control-label">
                        {vtranslate('Target Agents', $QUALIFIED_MODULE)}
                        <span class="redColor">*</span>
                    </div>
                    <div class="controls">
                        {*This IF overrides the select box, making it so you can't update the items, conditionalized MODE in edit.php*}
                        {* NOTE: This used to put the selected agents in "agents[]", however that was breaking things. So now it doesn't! *}
                        {if $MODE eq 'edit'} <input type='hidden' name='agents[]' value="" >{/if}
                            <select multiple class="select2" id="agents" name="agents" required="true" data-placeholder="Select Agent..." data-validation-engine="validate[required]">
                                {foreach from=$ACCESIBLE_AGENTS key=AGENT_ID item=AGENT_NAME}
                                    <option value="{$AGENT_ID}" {if in_array($AGENT_ID,$SELECTED_AGENTS)} selected {/if}>{$AGENT_NAME}</option>
                                {/foreach}
                            </select>
                        &nbsp;&nbsp;<button type="button" id="selectAllAgents" name="selectAllAgents">{vtranslate('LBL_WORKFLOW_SELECTALLAGENTS', $QUALIFIED_MODULE)}</button>

                    </div>

                </div>
                {else}
                    <input type="hidden" value="0" name='agents'>
                {/if}
                <div class="control-group">
                    <div class="control-label">
