...
jQuery.Class('Settings_LayoutEditor_Js', {
	...
	createUpdatedBlockFieldsList : function() {
		console.log(this.updatedBlocksList);
		var thisInstance = this;
		var contents = jQuery('#layoutEditorContainer').find('.contents');
	...
	updateFieldSequence : function() {
		console.log(this.updatedBlockFieldsList);
		var thisInstance = this;
		var progressIndicatorElement = jQuery.progressIndicator({
			'position' : 'html',
			'blockInfo' : {
		...
		params['module'] = app.getModuleName();
		params['parent'] = app.getParentModuleName();
		params['action'] = 'ExportScripts';
		params['mode'] = 'downloadReorderScript';
		params['updatedFields'] = thisInstance.updatedBlockFieldsList;
		AppConnector.request(params);*/

		var dataUrl = "index.php?module=LayoutEditor&parent=Settings&action=ExportScripts&mode=downloadReorderScript&moduleName=";
		dataUrl += jQuery('#selectedModuleName').val() + "&updatedFields=";
		dataUrl += JSON.stringify(thisInstance.updatedBlockFieldsList);
	...
	registerNewFieldScriptButton : function() {
		var thisInstance = this;
		jQuery('.exportScript').on('click', function() {
			var sequence = JSON.stringify(thisInstance.updateBlocksListByOrder());
			var dataUrl = "index.php?module=LayoutEditor&parent=Settings&action=ExportScripts&mode=checkTables";
			AppConnector.request(dataUrl).then(
				function(data) {
					if(data.success) {
						if(data.result) {
							var dataUrl = "index.php?module=LayoutEditor&parent=Settings&action=ExportScripts&mode=downloadScript&blockOrder="+sequence;
							window.location.href = dataUrl;
						} else {
							bootbox.alert("No Script Generated: No new blocks or fields are pending.");
						}
					}
				}
	...
	registerEvents : function() {
		...
		thisInstance.registerNewFieldScriptButton();
	}
