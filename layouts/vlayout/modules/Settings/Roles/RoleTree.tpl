...
{foreach from=$ROLE->getChildren() item=CHILD_ROLE}
	<style>
	.treeView li .toolbar-handle .depth{$CHILD_ROLE->getDepth()}:before{
		background: {$COLORS[$CHILD_ROLE->getDepth()]};
	}
	a.depth{$CHILD_ROLE->getDepth()}{
		background-color: {$COLORS[$CHILD_ROLE->getDepth()]};
	}
	li.depth{$CHILD_ROLE->getDepth()}{
		border-left: 4px solid {$COLORS[($CHILD_ROLE->getDepth())]};
	}
	</style>
	<li data-role="{$CHILD_ROLE->getParentRoleString()}" data-roleid="{$CHILD_ROLE->getId()}" class="depth{$CHILD_ROLE->getDepth()}">		
