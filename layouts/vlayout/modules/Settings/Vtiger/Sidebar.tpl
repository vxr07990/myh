			...
			{foreach item=MENUITEM from=$MENU->getMenuItems()}
				<div class="{if $MENUITEM->getId() eq $SELECTED_FIELDID} selectedMenuItem selectedListItem{/if}" style="padding:7px;border-top:0px;">
					<div class="row-fluid menuItem"  data-actionurl="{$MENUITEM->getPinUnpinActionUrl()}">
						<a href="{$MENUITEM->getUrl()}" data-id="{$MENUITEM->getId()}" class="span9 menuItemLabel" data-menu-item="true" >{if $MENUITEM->get('name') eq $MENUITEM->get('description')}{vtranslate($MENUITEM->get('name'), $MENUITEM->get('name'))}{else}{vtranslate($MENUITEM->get('name'), $QUALIFIED_MODULE)}{/if}</a>
