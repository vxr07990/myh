...
{if $MULTISAVE}
    <input type="hidden" id="multiSaveMessage" value="{$MULTISAVE}" />
{/if}
...
<div class="listViewEntriesDiv contents-bottomscroll">
	...
	<table class="table table-bordered listViewEntriesTable">
		...
		<input type='hidden' id='{$LISTVIEW_ENTRY->getId()}_fixed' value={$LISTVIEW_ENTRY->get('isFixed')} />
		<tr class="listViewEntries record{$LISTVIEW_ENTRY->getId()}" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}" {if isParticipantForRecord($LISTVIEW_ENTRY->getId())}style="background-color: #ffff99"{/if}>
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
                ...
                {else if $LISTVIEW_HEADER->get('uitype') eq 33 || $LISTVIEW_HEADER->get('uitype') eq 16}
                    {vtranslate($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME), $MODULE)}
                {else if $LISTVIEW_HEADER->get('uitype') eq 1989 || $LISTVIEW_HEADER->get('uitype') eq 1990}
                    {*{assign var=FIELD_MODEL value = $LISTVIEW_ENTRY->getField($LISTVIEW_HEADERNAME)}*}
