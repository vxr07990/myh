	...
	<span class="listViewLoadingImageBlock hide modal" id="loadingListViewModal">
		...
		<thead>
			<tr class="listViewHeaders">
				{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
					...
					{elseif $LISTVIEW_HEADER->getName() neq 'last_name' and $LISTVIEW_HEADER->getName() neq 'email1'}
						<th nowrap><a href="javascript:void(0);" class="listViewHeaderValues" data-nextsortorderval="{if $COLUMN_NAME eq $LISTVIEW_HEADER->get('column')}{$NEXT_SORT_ORDER}{else}ASC{/if}" data-columnname="{$LISTVIEW_HEADER->get('column')}">{vtranslate($LISTVIEW_HEADER->get('label'), $LISTVIEW_HEADER->getModuleName())}
                            &nbsp;&nbsp;{if $COLUMN_NAME eq $LISTVIEW_HEADER->get('column')}<img class="{$SORT_IMAGE} icon-white">{/if}</a>
						</th>
                    {/if}
            ...
            {if $MODULE_MODEL->isQuickSearchEnabled()}
                <tr>
                {foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
                    {if $LISTVIEW_HEADER->getName() eq 'email1' or $LISTVIEW_HEADER->getName() eq 'last_name'}
                        {continue}
                    {/if}
                    {if  $LISTVIEW_HEADER->getName() neq 'user_name'}
                        <td>
                        </td>
                        {continue}
                    {/if}
