...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=feloop}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{/if}
								{if $FIELD_NAME=='claimitemsdetails_datetaken'}
		...
		{/if}
                ...
                {if $MODULE eq 'ClaimItems' && ($QTY eq $BCOUNT)}
                     {if $ORIGINAL_CONDITION_SHOW eq true}
                        {include file=vtemplate_path('originalConditionsEdit.tpl', 'ClaimItems')}
                    {/if}
                    {if $DAILY_EXPENSES_SHOW eq true}
                        {include file=vtemplate_path('dailyExpensesEdit.tpl', 'ClaimItems')}
                    {/if}
                    {include file=vtemplate_path('settlementAmountEdit.tpl', 'ClaimItems')}
