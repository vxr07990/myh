...
-->*}
{strip}
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE name=feloop}
	...
	{/foreach}
        {if $MODULE_NAME eq 'ClaimItems'}
            {if $ORIGINAL_CONDITION_SHOW eq true}
                {include file=vtemplate_path('originalConditionsDetail.tpl', 'ClaimItems')}
            {/if}
            {if $DAILY_EXPENSES_SHOW eq true}
                {include file=vtemplate_path('dailyExpensesDetail.tpl', 'ClaimItems')}
            {/if}  
            {include file=vtemplate_path('settlementAmountDetail.tpl', 'ClaimItems')}
