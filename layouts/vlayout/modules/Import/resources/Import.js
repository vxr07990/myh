...
if (typeof(ImportJs) == 'undefined') {
    ...
    ImportJs = {
			...
			for(var i=0; i<fieldsList.length; ++i) {
				...
				if(selectedFieldName != '') {
					if(selectedFieldName in mappedFields) {
						errorMessage = app.vtranslate('JS_FIELD_MAPPED_MORE_THAN_ONCE') + selectedFieldElement.data('label') + app.vtranslate('JS_FIELD_MAPPED_MORE_THAN_ONCE_TAIL');
						params = {
							text: errorMessage,
							'type': 'error'
						};
		...
		loadDefaultValueWidget: function(rowIdentifierId) {
			 ...
			 *
			 * When form validation happens, it creates a popup above invalid fields. However, this won't work for
			 * fields with their display value set to 'none'. So instead we set the visibility to 'hidden' and position
			 * the element absolute to the container so that the form validator knows where to place the invalid field
			 * popup.
             */
			var select = selectedFieldDefValueContainer.children('select');
			if (select.length > 0) {
				select.css({
