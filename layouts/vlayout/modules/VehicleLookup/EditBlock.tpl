        ...
        <table class="table table-bordered blockContainer showInlineTable equalSplit{if is_array($HIDDEN_BLOCKS)}{if in_array($TABLE_NAME, $HIDDEN_BLOCKS)}hide{/if}{/if} block_{$GUEST_BLOCK_LABEL}" name="{$TABLE_NAME}">
            ...
            {if !$LOCK_RATING}
				...
				<tr class="fieldLabel">
					<td colspan="4">
						...
						<span style="width:55%;clear:right;float:right;">{if $MODULE_NAME eq 'Estimates'}<button type="button" style="float:left;width:20%" id="vehicleRatingButton" class="interstateRateBtns interstateRateDetail">Get Rate</button>{/if}
						<input type="hidden" id="num{$GUEST_MODULE}" name="num{$GUEST_MODULE}" value="{${$GUEST_LIST}|@count}">
						<button type="button" class="add{$GUEST_MODULE}" name="add{$GUEST_MODULE}2" style="clear:right;float:right">+</button></span>
					</td>
				</tr>
				</tbody>
            {/if}
            <tbody class="default{$GUEST_MODULE} {$GUEST_MODULE}Block hide">
				...
				<tr>
                    ...
                    {else}
                                </tr><tr><td class="fieldLabel {$WIDTHTYPE}"></td><td class="{$WIDTHTYPE}"></td>
                                <td class='fieldLabel {$WIDTHTYPE}'>
						</td>
                                <td class='fieldValue {$WIDTHTYPE}'>
							<button type='button' name='lookupVin{$VEHICLE_INDEX+1}' id='lookupVin{$VEHICLE_INDEX+1}'>{vtranslate('LBL_VEHICLE_LOOKUP', 'VehicleLookup')}</button>
