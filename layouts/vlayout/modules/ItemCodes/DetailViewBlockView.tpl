...
{strip}
	...
	{if not $BLOCK->get('hideblock')}<br>{/if}
		{if $ITEMCODESMAPPING_MODULE_MODEL && $ITEMCODESMAPPING_MODULE_MODEL->isActive() && $BLOCK_LABEL_KEY eq 'LBL_ITEMCODES_DETAILS'}
			{include file=vtemplate_path('ItemCodesMappingDetail.tpl', 'ItemCodesMapping')}
		{/if}
	{/foreach}
{/strip}