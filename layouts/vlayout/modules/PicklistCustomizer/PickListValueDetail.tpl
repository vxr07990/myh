...
<ul class="nav nav-tabs massEditTabs" style="margin-bottom: 0;border-bottom: 0">
	...
	<li class="marginRight5px"><a href="#dependencyValuesLayout" data-toggle="tab"><strong>{vtranslate('LBL_PICKLIST_DEPENDENCY',$QUALIFIED_MODULE)}</strong></a></li>
...
<div class="tab-content layoutContent padding20 themeTableColor overflowVisible">
	...
	<div class="tab-pane active" id="allValuesLayout">
			...
			<div class="span5 marginLeftZero textOverflowEllipsis">
				<table id="pickListValuesTable" class="table table-bordered" style="table-layout: fixed">
					...
					{foreach key=PICKLIST_KEY item=PICKLIST_VALUE from=$PICKLIST_VALUES}
						<tr class="pickListValue" data-key-id="{$PICKLIST_KEY}" data-key="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_VALUE)}" data-special="{Vtiger_Util_Helper::isPicklistValueSpecial($SELECTED_PICKLIST_FIELDMODEL->get('name'),$PICKLIST_VALUE)}">
        ...
        <div class="tab-pane" id="dependencyValuesLayout">
			<form id="pickListDependencyForm" target="index.php">
            <table>
            <tr>
                <td style="width: 80px"><span class="redColor">*</span>Target Field</td>
                <td>
                    <select id="target_field_select" name="target_field_select" class="chzn-select" style="width: 250px;">
                        {$TARGET_FIELD_OPTIONS}
                    </select>
                </td>
            </tr>
            </table>
            <div class="row-fluid hide errorMessage"><div class="alert alert-error"><strong>{vtranslate('LBL_ERR_CYCLIC_DEPENDENCY',$QUALIFIED_MODULE)}</strong></div></div>
