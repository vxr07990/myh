...
{strip}
	...
	<table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if} block_{$BLOCK_LABEL_KEY}">
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			{if $FIELD_NAME eq 'file_name' OR $FIELD_NAME eq 'thumb_file_name' OR $FIELD_NAME eq 'archiveid'}{continue}{/if}
		<!-- {$FIELD_NAME} -->
    ...
    <table class="table table-bordered equalSplit detailview-table">
        ...
        <tr>
            {assign var=COUNTER value=0}
            {foreach item=REL_RECORD key=RECORDNUM from=$RELATED_RECORDS}
            <!-- {$REL_RECORD|@print_r} -->
            {if $COUNTER eq 2}
        </tr><tr>
            {assign var=COUNTER value=1}
            ...
            <td class='fieldLabel'>
                <label class="muted pull-right marginRight10px">
                    {assign var=MODNAME value="SINGLE_"|cat:$REL_RECORD.module}
                    {vtranslate({$MODNAME},{$REL_RECORD.module})}
                </label>
            </td>
            <td class='fieldValue'>
					<span class='value' data-field-type='owner'>
						<a href='{$REL_RECORD.link}'>{$REL_RECORD.label}</a>
					</span>
            </td>
            {/foreach}
            {while $COUNTER neq 2}
    ...
    {if $RECORD->get('is_video') eq 0}
        <img style="display:block;margin:0 auto" src="{$S3_IMAGE_URL}" />
    {else}
        <video style="display:block;margin:0 auto" controls>
            <source type="video/mp4" src="{$S3_VIDEO_URL}">
        </video>
    {/if}
{/strip}
