...
{if {$REFERENCE_LIST_COUNT} gt 1}
	...
	{if empty($DISPLAYID)}
		{assign var="DISPLAYID" value=$SOURCE_MODULE}
	{/if}
	{assign var="REFERENCED_MODULE_STRUCT" value=$FIELD_MODEL->getUITypeModel()->getReferenceModule($SOURCE_RECORD)}
...
{if empty($VALUE)}
	{assign var=VALUE value=$SOURCE_RECORD}
{/if}
<input name="{$FIELD_MODEL->getFieldName()}" type="hidden" value="{$VALUE}" class="sourceField" data-displayvalue='{$FIELD_MODEL->getEditViewDisplayValue($VALUE)}' data-fieldinfo='{$FIELD_INFO}' />
{assign var="displayId" value=$VALUE}
