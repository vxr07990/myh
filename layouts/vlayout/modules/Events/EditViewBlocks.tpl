					...
					{if $isReferenceField neq "reference"}<label class="muted pull-right marginRight10px">{/if}
						...
						{if $isReferenceField eq "reference"}
							...
							{if $REFERENCE_LIST_COUNT > 1}
								...
								{/if}
								{assign var=SELECTED value=$SOURCE_MODULE}
								{assign var=FIELDVALUE value=$FIELD_MODEL->get('fieldvalue')}
								{if !empty($FIELDVALUE) && !empty($FIELD_MODEL->getUITypeModel()->getReferenceModule($FIELD_MODEL->get('fieldvalue')))}
									{assign var=SELECTED value=$FIELD_MODEL->getUITypeModel()->getReferenceModule($FIELD_MODEL->get('fieldvalue'))->get('name')}
								{/if}
								<span class="pull-right">
                                    ...
                                    <select id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->getName()}_dropDown" class="chzn-select referenceModulesList streched" style="width:160px;">
										<optgroup>
											{foreach key=index item=value from=$REFERENCE_LIST}
												<option value="{$value}" {if vtranslate($value, $MODULE) eq vtranslate($SELECTED, $MODULE)}selected{/if}>{vtranslate($value, $MODULE)}</option>
											{/foreach}
										</optgroup>
									</select>
								</span>
