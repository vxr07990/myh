...
{strip}
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
		{if $BLOCK_LABEL_KEY neq 'LBL_REVENUE_GROUPING_ITEMS'}
		...
		{else}
			{if $BLOCK_LABEL_KEY eq 'LBL_REVENUE_GROUPING_ITEMS' && $REVENUEGROUPINGITEM_MODULE_MODEL && $REVENUEGROUPINGITEM_MODULE_MODEL->isActive()}
				{include file=vtemplate_path('RevenueGroupingItemDetail.tpl', 'RevenueGroupingItem')}
			{/if}
		{/if}
	{/foreach}
	{if $SOURCE_MODULE eq "AgentManager"}
		<div style="text-align: center">
			<button class="btn btn-success" type="button" id="btnAddRevenueGrouping" ><strong>Edit</strong></button>
			{if $REVENUEGROUPINGITEM_LIST|@count gt 0}
				<button class="btn btn-danger" type="button" id="btnDeleteRevenueGrouping" data-record-id="{$RECORD_ID}"><strong>Delete</strong></button>
			{/if}
		</div>
	{/if}
