...
jQuery.Class("Calendar_CalendarView_Js",{
	...
	registerCalendar : function(customConfig) {
        ...
        var timeZone= jQuery('#timeZoneTmp').clone();
        timeZone.attr("id",'timeZone').attr('name','time_zone').addClass("select2").removeClass("hide").css('width','200px');
        var timeZoneSpan= jQuery('<span class="pull-left" style="margin-top:4px"></span>').append(timeZone);
        app.showSelect2ElementView(timeZoneSpan.find('select.select2'));
        timeZoneSpan.prependTo(calendarview.find('.fc-header .fc-header-right')).on('change', '#timeZone', function(e) {
            var calendarfeeds = jQuery('[data-calendar-feed]');
            calendarfeeds.each(function(index,element){
                var feedcheckbox = jQuery(element);
                var	disabledOnes = app.cacheGet('calendar.feeds.disabled',[]);
                ...
                // Remove all feeds
                thisInstance.getCalendarView().fullCalendar('removeEventSource', thisInstance.calendarfeedDS[feedcheckbox.data('calendar-feed')]);
                ...
                // feed new based on selected timezone
                thisInstance.fetchCalendarFeed(feedcheckbox);
            });
        });

		//@TODO: Hiding this button, until it can be made to work cross platform more reliably.
		//@NOTE: Reshowing it for SIRVA, because they want it.
		if($('[name="instance"]').val() == 'sirva') {
            this.calendarSettingsButton(thisInstance, calendarview, app, false);
        }else{
            this.calendarSettingsButton(thisInstance, calendarview, app, true);
        }
	},
    calendarSettingsButton : function(thisInstance, calendarview, app, hide) {
        jQuery('<span class="pull-right marginLeft5px '+(hide?'hide':'')+'"><button class="btn"><i id="calendarSettings" class="icon-cog"></i></button></span>')
		.prependTo(calendarview.find('.fc-header .fc-header-right')).on('click', 'button', function(e){
			var params = {
				module : 'Calendar',
				view : 'Calendar',
