        ...
        {foreach item=SIDEBARWIDGET key=index from=$QUICK_LINKS['SIDEBARWIDGET']}
            <div class="quickWidget">
                <div class="accordion-heading accordion-toggle quickWidgetHeader"
                     data-target="#{$MODULE}_sideBar_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($SIDEBARWIDGET->getLabel())}"
                     data-toggle="collapse" data-parent="#quickWidgets" data-label="{$SIDEBARWIDGET->getLabel()}"
                     data-widget-url="{$SIDEBARWIDGET->getUrl()}">
                    <span class="pull-left"><img class="imageElement"
                                                 data-rightimage="{vimage_path('rightArrowWhite.png')}"
                                                 data-downimage="{vimage_path('downArrowWhite.png')}"
                                                 src="{vimage_path('rightArrowWhite.png')}"/></span>
                    {if $SIDEBARWIDGET->getLabel() == 'LBL_ADDED_CALENDARS' || $SIDEBARWIDGET->getLabel() == 'LBL_ACTIVITY_TYPES'}
                        <span class="pull-right"><i class="icon-plus addCalendarView"
                                                    title="{vtranslate('LBL_ADD_CALENDAR_VIEW',$MODULE)}"></i></span>
                    {/if}
                    <h5 class="title widgetTextOverflowEllipsis"
                        title="{vtranslate($SIDEBARWIDGET->getLabel(), $MODULE)}">{vtranslate($SIDEBARWIDGET->getLabel(), $MODULE)}</h5>
                ...
                <div class="widgetContainer accordion-body collapse"
