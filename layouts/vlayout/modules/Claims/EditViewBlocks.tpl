...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
                                                            {if $FIELD_MODEL->get('name') eq "claims_date_received" || $FIELD_MODEL->get('name') eq "claims_date_closed" || $FIELD_MODEL->get('name') eq "claims_calendar_days_settle" || $FIELD_MODEL->get('name') eq "claims_business_days_settle"}                                
                                                                <input type="text" readonly name="{$FIELD_MODEL->get('name')}" value="{$FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue'))}">
                                                            {else}
		...
		{/if}
                ...
                {if $MODULE_NAME eq 'Claims' && $BLOCK_LABEL eq 'LBL_STATUS_INFORMATION'}
                        {*include file=vtemplate_path('statusClaimEdit.tpl', 'Claims')*}
                        {include file=vtemplate_path('statusClaimDetail.tpl', 'Claims')}
                {/if}
                {if $MODULE_NAME eq 'Claims' && $BLOCK_LABEL eq 'LBL_APPOINTMENT_INFORMATION'}
                        {include file=vtemplate_path('paymentsEdit.tpl', 'Claims')}
                        {include file=vtemplate_path('serviceProviderEdit.tpl', 'Claims')}
                        {include file=vtemplate_path('summaryTable.tpl', 'Claims')}
                {/if}      
</tbody>
			</table>
			<br>
        ...
        <div class="row">
            <a class="btn" readonly disabled style="text-decoration:none;margin-bottom: 2%;float: right;" href="javascript:void(0);">
                <strong>Please save to add new items</strong>
            </a>
        </div>
        
        {include file=vtemplate_path('claimItemsRelated.tpl', 'Claims')}
{/strip}