...
{strip}
	...
	<table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if}">
		...
		{/if}
         	{include file=vtemplate_path('GuestDetailBlocks.tpl', $MODULE_NAME)}
                {if $MODULE_NAME eq 'Claims' && $BLOCK_LABEL_KEY eq 'LBL_STATUS_INFORMATION'}
                    {include file=vtemplate_path('statusClaimDetail.tpl', 'Claims')}
                {/if}
                {if $MODULE_NAME eq 'Claims' && $BLOCK_LABEL_KEY eq 'LBL_APPOINTMENT_INFORMATION'}
                    {include file=vtemplate_path('paymentsDetail.tpl', 'Claims')}
                    {include file=vtemplate_path('serviceProviderDetail.tpl', 'Claims')}
	...
	{/foreach}
        ...
        <div class="row" style="margin-bottom: 2%;">
            <a class="btn pull-right" style="text-decoration:none;" href="index.php?module=ClaimItems&view=Edit&sourceModule=Claims&sourceRecord={$RECORD->getId()}&relationOperation=true&linked_claim={$RECORD->getId()}">
                <i class="icon-plus"></i>&nbsp;<strong>Add Record</strong>
            </a>
            <a class="btn btn-default pull-right" style="text-decoration:none;margin-right: 1%;" href="javascript:void(0);" id="multipleSPResp">
                <i class="icon-pencil"></i>&nbsp;<strong>S.P. Responsibility</strong>
            </a>   
        </div>
