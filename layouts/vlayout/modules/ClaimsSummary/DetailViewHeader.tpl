		...
		<div class="row-fluid detailViewTitle">
			<div class="{if $NO_PAGINATION} span12 {else} span10 {/if}">
				<div class="row-fluid">
					...
					<div class="span7">
						{*$DETAILVIEW_LINKS|@debug_print_var*}
						<div class="pull-right detailViewButtoncontainer">
							<div class="btn-toolbar">
							{foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
							{if $DETAIL_VIEW_BASIC_LINK->get('linklabel') eq 'LBL_EDIT'}{* && $CREATOR_PERMISSIONS eq 'true'*}
							...
							{/if}
							{/foreach}
							{foreach item=DETAIL_VIEW_CUSTOM_LINK from=$DETAILVIEW_LINKS['DETAILVIEWCUSTOM']}
								<span class="btn-group">
									<button class="btn" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_CUSTOM_LINK->getLabel())}"
											{if $DETAIL_VIEW_CUSTOM_LINK->isPageLoadLink()}
										onclick="window.location.href='{$DETAIL_VIEW_CUSTOM_LINK->getUrl()}'"
											{else}
									onclick={$DETAIL_VIEW_CUSTOM_LINK->getUrl()}
											{/if}>
										<strong>{vtranslate($DETAIL_VIEW_CUSTOM_LINK->getLabel(), $MODULE_NAME)}</strong>
									</button>
								</span>
							{/foreach}
							{if $DETAILVIEW_LINKS['DETAILVIEW']|@count gt 0}
								{*if $CREATOR_PERMISSIONS eq 'true'*}
									<span class="btn-group">
										...
										<ul class="dropdown-menu pull-right">
                                                                                    {foreach item=DETAIL_VIEW_LINK from=$DETAILVIEW_LINKS['DETAILVIEW']}
                                                                                        {if $DETAIL_VIEW_LINK->getLabel()|strpos:"Delete" === false}
                                                                                            {if $DETAIL_VIEW_LINK->getLabel() eq ""} 
												<li class="divider"></li>	
                                                                                            {else}
                                                                                                <li id="{$MODULE_NAME}_detailView_moreAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_LINK->getLabel())}" class="{if $DETAIL_VIEW_LINK->getLabel() eq 'Delete Order'}hide{/if}">
			...
			{if !{$NO_PAGINATION}}
				<div class="span2 detailViewPagingButton">
					<span class="btn-group pull-right"> 
						<button class="btn" id="detailViewPreviousRecordButton" {if empty($PREVIOUS_ENTRY_URL)} disabled="disabled" {else} onclick="window.location.href='{$PREVIOUS_ENTRY_URL}'" {/if}><i class="icon-chevron-left"></i></button>
						<button class="btn" id="detailViewNextRecordButton" {if empty($NEXT_ENTRY_URL)} disabled="disabled" {else} onclick="window.location.href='{$NEXT_ENTRY_URL}'" {/if}><i class="icon-chevron-right"></i></button>
					</span>
				</div>
			{/if}
		</div>
		<div class="detailViewInfo row-fluid">
			<div class="{if $NO_PAGINATION} span12 {else} span10 {/if} {if !empty($DETAILVIEW_LINKS['DETAILVIEWTAB']) || !empty($DETAILVIEW_LINKS['DETAILVIEWRELATED']) } details {/if}">
				<form id="detailView" data-name-fields='{ZEND_JSON::encode($MODULE_MODEL->getNameFields())}' method="POST">
                                    ...
                                    {/if} 
					<div class="contents"><!-- end of DetailViewHeader.tpl -->
{/strip}
