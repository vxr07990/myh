	...
	<table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if}">
		...
		{/if}
         	...
         	{include file=vtemplate_path('GuestDetailBlocks.tpl', $MODULE_NAME)}
                {if $MODULE_NAME eq 'ClaimsSummary' && $BLOCK_LABEL_KEY eq 'LBL_CLAIMSSUMMARY_INFORMATION'}
                    {include file=vtemplate_path('claimsDateDetail.tpl', 'ClaimsSummary')}
                    {include file=vtemplate_path('claimsPaymentDetail.tpl', 'ClaimsSummary')}
                    {include file=vtemplate_path('claimsTypeSummaryGrids.tpl', 'ClaimsSummary')}
                {/if}
                </tbody>
	</table>
                
                <table class="table table-bordered blockContainer showInlineTable equalSplit" style="margin-top: 2%;">
                    <thead>
                    <tr>
