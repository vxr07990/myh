...
<table class="table table-bordered blockContainer showInlineTable equalSplit">
    ...
    <tr>
        <th class="blockHeader" colspan="6">
            ...
            <span class="AddressListTitle">&nbsp;&nbsp;
                {if $ADDRESS_RECORD['address_type']}
                    {$ADDRESS_RECORD['address_type']}: {$ADDRESS_RECORD['address1']}, {$ADDRESS_RECORD['city']}, {$ADDRESS_RECORD['state']}, {$ADDRESS_RECORD['zip_code']}
                {else}
                    Item {$ROWNO}
                {/if}
            </span>
            <i class="icon-trash pull-right deleteAddressButton" title="Delete"></i>
            <i class="pull-right copyAddressButton" title="Copy"><img width="14" height="14" src="{vimage_path('copy-icon.png')}"></i>
            <i class="pull-right dragAddressButton" title="Drag" style="margin-right: 2px"><img src="{vimage_path('move.png')}"></i>&nbsp;
            <input type="hidden" name="addresslistid_{$ROWNO}" value="{$ADDRESSLIST_ID}">
    ...
    <tr>
        ...
        {if $FIELD_MODEL->get('uitype') neq "83"}
            <td {if !in_array($FIELD_MODEL->getName(),['state','zip_code','of_flights','of_elevators','city','long_carry'])}colspan="2"{/if} class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                <div class="row-fluid">
							<span class="span10">
