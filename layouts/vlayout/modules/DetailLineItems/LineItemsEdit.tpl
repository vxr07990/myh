...
{strip}
    ...
    {assign var="IS_ESTIMATES_ACTUALS" value=$MODULE_NAME eq "Estimates" || $MODULE_NAME eq "Actuals"}
    ...
    <table class="table table-bordered blockContainer lineItemTable" id="lineItemTab">
        <tr>
			...
			<th colspan="2" class="chznDropDown {if $IS_ESTIMATES_ACTUALS}hide{/if}">
                <div class="row-fluid">
        ...
        {foreach key=row_no item=data from=$RELATED_PRODUCTS}
            <tr id="row{$row_no}" class="lineItemRow" {if $data["entityType$row_no"] eq 'Products'}data-quantity-in-stock={$data["qtyInStock$row_no"]}{/if}>
                {include file="LineItemsContent.tpl"|@vtemplate_path:'Estimates' row_no=$row_no data=$data}
            </tr>
        {/foreach}
    </table>
    ...
    <table class="table table-bordered blockContainer lineItemTable" id="lineItemResult">
        <tr {if $IS_ESTIMATES_ACTUALS}class='hide'{/if}>
            <td  width="83%">
		...
		<!-- Group Tax - starts -->
        <tr id="group_tax_row" valign="top" class="{if $IS_INDIVIDUAL_TAX_TYPE}hide{/if} {if $IS_ESTIMATES_ACTUALS}hide{/if}">
            <td width="83%">
        ...
        <tr valign="top" {if $IS_ESTIMATES_ACTUALS}class='hide'{/if}>
            <td width="83%" >
		...
		<tr valign="top">
			<td colspan="2">
                <span class="pull-right">
                    &nbsp;
                    {if !$LOCK_RATING}
                        <button type='button' class="interstateRateBtns interstateRateDetail hide">Detailed Rate Estimate</button>
                        <button class="localMoveTables hide localRate" type="button">Get Rate Estimate</button>
                    {/if}
                    {if $RECORD_ID}
				        <button type='button' id='getReportSelectButton'>Get Report</button>
                    {/if}
                </span>
			</td>
		</tr>
...
{foreach key=GUEST_INDEX item=GUEST_MODULE from=$GUEST_MODULES}
    {if $GUEST_MODULE eq 'DetailLineItems'}
        <input type="hidden" value="{$GUEST_MODULE}" class="guestModule" id="guestModule_{$GUEST_INDEX}">
