					...
					<div class="span7">
						...
						<div class="pull-right detailViewButtoncontainer">
							...
							{if $DETAIL_VIEW_BASIC_LINK->get('linklabel') eq 'LBL_EDIT'}{* && $CREATOR_PERMISSIONS eq 'true'*}
                                {if $RECORD->get("ordersstatus") neq "Cancelled"}
								...
								{*if $CREATOR_PERMISSIONS eq 'true'*}
									<span class="btn-group">
										...
										<ul class="dropdown-menu pull-right">
											...
											{/foreach}
                                                                                        {if $RECORD->get("ordersstatus") neq "Cancelled"}
                                                                                            {if $RECORD->isPermittedUserRole("Cancel")}
                                                                                            <li id="showCancelModal">
												<a href="javascript:void(0);">{vtranslate("LBL_CANCEL_ORDER", $MODULE_NAME)}</a>
                                                                                            </li>
                                                                                            {/if}
                                                                                        {else}
                                                                                            {if $RECORD->isPermittedUserRole("Uncancel")}
                                                                                            <li id="showUnCancelModal">
												<a href="javascript:void(0);">{vtranslate("LBL_UNCANCEL_ORDER", $MODULE_NAME)}</a>
                                                                                            </li>
                                                                                            {/if}
                                                                                        {/if}
										</ul>
