...
Vtiger_Popup_Js("Orders_Popup_Js", {},{
	...
	getAccountId : function(){
		return jQuery('[name="orders_account"]').val();
	},
	...
	show : function(urlOrParams, cb, windowName, eventName, onLoadCb){
		...
		if(urlOrParams.src_field == 'account_contacts') {
			var relId = jQuery('input[name="orders_account"]').val();
			urlOrParams.related_parent_module = 'Accounts';
			urlOrParams.related_parent_id = relId;
		}
		...
		if(this.getPopupStatus()){
			...
			if (typeof urlOrParams == 'object') {
				...
				urlOrParams['accountId'] = jQuery('input[name="orders_account"]').val();
			} else {
				urlOrParams += '&triggerEventName=' + eventName;
				urlOrParams += '&assignedTo=' + jQuery('select[name="assigned_user_id"]').val();
				urlOrParams += '&accountId=' + jQuery('input[name="orders_account"]').val();
