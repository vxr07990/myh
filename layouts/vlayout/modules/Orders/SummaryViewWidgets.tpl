...
<div class="row-fluid">
	...
	<div class='span5' style="overflow: hidden">
						...
						<span class="span3">
							<span class=" pull-right">
								{if $MILESTONE_WIDGET_MODEL->get('action')}
									<button class="btn addButton" id="createOrdersMileStone" type="button" data-url="{$MILESTONE_WIDGET_MODEL->get('actionURL')}" data-parent-related-field="ordersid">
										<strong>{vtranslate('LBL_ADD',$MODULE_NAME)}</strong>
									</button>
								{/if}
							</span>
		...
		{*{if $TASKS_WIDGET_MODEL}
			<div class="summaryWidgetContainer">
				<div class="widgetContainer_2" data-url="{$TASKS_WIDGET_MODEL->getUrl()}" data-name="{$TASKS_WIDGET_MODEL->getLabel()}">
					<div class="widget_header row-fluid">
						<span class="span9">
							<div class="row-fluid">
								<span class="span4 margin0px"><h4>{vtranslate($TASKS_WIDGET_MODEL->getLabel(),$MODULE_NAME)}</h4></span>
								<span class="span7">
									{assign var=RELATED_MODULE_MODEL value=Vtiger_Module_Model::getInstance('OrderTask')}
									{assign var=FIELD_MODEL value=$RELATED_MODULE_MODEL->getField('orderstaskstatus')}
									{assign var="FIELD_INFO" value=Zend_Json::encode($FIELD_MODEL->getFieldInfo())}
									{assign var=PICKLIST_VALUES value=$FIELD_MODEL->getPicklistValues()}
									{assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
						...
						<span class="span3">
							{if $TASKS_WIDGET_MODEL->get('action')}
								<button class="btn pull-right addButton" id="createOrdersTask" type="button" data-url="{$TASKS_WIDGET_MODEL->get('actionURL')}" data-parent-related-field="ordersid">
									<strong>{vtranslate('LBL_ADD',$MODULE_NAME)}</strong>
								</button>
							{/if}
						</span>
					...
					<div class="widget_header row-fluid">
						<span class="span9">
							<div class="row-fluid">
								<span class="span4 margin0px"><h4>{vtranslate($TASKS_WIDGET_MODEL->getLabel(),$MODULE_NAME)}</h4></span>
								<span class="span7">
									{assign var=RELATED_MODULE_MODEL value=Vtiger_Module_Model::getInstance('OrdersTask')}
									{assign var=FIELD_MODEL value=$RELATED_MODULE_MODEL->getField('orderstaskprogress')}
									{assign var="FIELD_INFO" value=Zend_Json::encode($FIELD_MODEL->getFieldInfo())}
									{assign var=PICKLIST_VALUES value=$FIELD_MODEL->getPicklistValues()}
									{assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
									<!--<select style="width: 160px;" class="chzn-select" name="{$FIELD_MODEL->get('name')}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} >
    ...
    <div class="row-fluid calendar-wrapper">
        <div class="quickWidgetContainer accordion span2 user-column">
            <div class="quickWidget">
                <div class="accordion-heading accordion-toggle quickWidgetHeader" data-label="LBL_ADDED_CALENDARS">Available Users
                    <span class="pull-right"><i class="icon-plus addCalendarView" title="Add Calendar View"></i></span>
                </div>
                <div id="calendarview-feeds"></div>
...
<script>
    {literal}
        jQuery(document).ready(function () {
            var instance = Orders_CalendarView_Js.getInstanceByView();
            instance.registerEvents();
            instance.loadSharedCalendarUsers();

            Orders_CalendarView_Js.currentInstance = instance;
