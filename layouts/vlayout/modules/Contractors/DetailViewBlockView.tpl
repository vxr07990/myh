	...
	<table class="table table-bordered equalSplit detailview-table">
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			...
			{else}
				...
				{/if}
				{* I start here *}
				{if  $BLOCK_LABEL_KEY eq 'LBL_CONTRACTORS_AVAILABILITY'}
					 {if $COUNTER eq '4'}
					 </tr>
                            <tr>
					{assign var=COUNTER value=1}
				...
				{/if}
				 {* I end here *}
				 <td class="fieldLabel {$WIDTHTYPE}"{if $BLOCK_LABEL_KEY eq 'LBL_CONTRACTORS_AVAILABILITY'}style='width:12.5%'{/if} id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}"{if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
					 <label class="muted pull-right marginRight10px">
						 {vtranslate({$FIELD_MODEL->get('label')},{$MODULE_NAME})}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}"{if $BLOCK_LABEL_KEY eq 'LBL_CONTRACTORS_AVAILABILITY'}style='width:12.5%'{/if} id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
