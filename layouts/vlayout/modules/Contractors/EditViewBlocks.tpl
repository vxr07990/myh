		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{/if}
					...
					{* I start here *}
			{if  $BLOCK_LABEL eq 'LBL_CONTRACTORS_AVAILABILITY'}
					 {if $COUNTER eq '4'}
					 </tr>
                            <tr>
				...
				<td class="fieldLabel {$WIDTHTYPE}" {if $BLOCK_LABEL eq 'LBL_CONTRACTORS_AVAILABILITY'}style='width:12.5%'{/if}>
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $BLOCK_LABEL eq 'LBL_CONTRACTORS_AVAILABILITY'}style='width:12.5%'{/if} {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
