    ...
    <table class="table table-bordered blockContainer showInlineTable">
        ...
        <tr class="defaultBillingAddress hide" style="text-align:center">
            ...
            <td>
                <div class="row-fluid">
                    <span class="span12">
                        <select class="chosen-select" multiple name="commodity[][]" style="width:95%">
            ...
            {foreach item=billing_row key=ROW_NUM from=$BILLING_ADDRESSES}
                ...
                <td>
                    ...
                    <span class="span10">
                        ...
                        <select class="chosen-select chzn-select" multiple name="commodity[{$COUNT}][]" required style="max-width: 150px;">
                           {*<option value="">Select an Option</option>*}
                            {assign var="FIELD_VALUE_LIST" value=explode(' |##| ',$billing_row.commodity)}
                            {foreach item=business_line key=ROW_NUM from=$BUSINESS_LINES}
                                {*<option value="{$business_line}" {if $business_line == $billing_row.business_line}selected {/if}>{$business_line}</option>*}
                                <option value="{Vtiger_Util_Helper::toSafeHTML($business_line)}" {if in_array(Vtiger_Util_Helper::toSafeHTML($business_line), $FIELD_VALUE_LIST)} selected {/if}>{vtranslate($business_line, $MODULE)}</option>
                            {/foreach}
                        </select>
                    </span>
                    </div>
