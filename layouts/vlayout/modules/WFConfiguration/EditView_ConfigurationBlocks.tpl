...
{assign var=COUNT value=0}
  {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
      {if $COUNT == 5 OR $COUNT == 0}
          ...
          <tr>
          <td class='fieldLabel'>
            {vtranslate($BLOCK_FIELDS[$FIELD_NAME]->get('label'),$MODULE)}
          </td>
          {if $SHOW_LABEL}
            <td>
              {include file=vtemplate_path($BLOCK_FIELDS[$FIELD_NAME]->getUITypeModel()->getTemplateName(),$MODULE) FIELD_MODEL=$BLOCK_FIELDS[$FIELD_NAME] COUNTER=1 MODULE=$MODULE}
            </td>
          {/if}
      {else}
          <td style='width:50px;'>
