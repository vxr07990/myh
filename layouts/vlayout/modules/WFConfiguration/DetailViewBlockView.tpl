...
{strip}
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
        {* START MODIFICATION FROM BASE *}
        {if $BLOCK_LABEL_KEY neq 'LBL_WFCONFIGURATION_DETAILS' && $BLOCK_LABEL_KEY neq 'LBL_RECORD_UPDATE_INFORMATION'}
        {include file=vtemplate_path('DetailView_ConfigurationBlocks.tpl', $MODULE) MODULE=$MODULE_NAME BLOCK_FIELDS=$FIELD_MODEL_LIST BLOCK_LABEL=$BLOCK_LABEL_KEY}
        <br>
        {else}
            {* END MODIFICATION FROM BASE : + find the {/if} to that else *}
	...
	<table class="table table-bordered equalSplit detailview-table {if $BLOCK->get('hideblock') eq true}hide{/if} block_{$BLOCK_LABEL_KEY}">
		<thead>
		<tr>
	...
	{/if}
	{if $MODULE_NAME eq 'VanlineManager' && $ACCOUNTING_INTEGRATION_ACTIVE}
		{include file=vtemplate_path('AccountingIntegrationBlock.tpl', 'AgentManager')}
	{/if}
{/strip}
