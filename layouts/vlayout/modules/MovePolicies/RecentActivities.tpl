							...
							<li>
								<div>
									...
									<span class="pull-right"><p class="muted"><small title="{Vtiger_Util_Helper::formatDateTimeIntoDayString($RECENT_ACTIVITY->getActivityTime())}">{Vtiger_Util_Helper::formatDateDiffInStrings($RECENT_ACTIVITY->getActivityTime())}</small></p></span>
                                                                ...
                                                                {foreach item=DETAIL from=$RECENT_ACTIVITIES_DETAILS}
									{if $DETAIL.id eq $RECENT_ACTIVITY->getId()}
										<div class='font-x-small updateInfoContainer'>
											<i>{$DETAIL.label}</i> :&nbsp;
                                                                                        &nbsp;{vtranslate('LBL_FROM')} <b style="white-space:pre;">{if $DETAIL.prevalue eq NULL}NULL{else}{Vtiger_Util_Helper::toVtiger6SafeHTML($DETAIL.prevalue)}{/if}</b>
                                                                                        &nbsp;{vtranslate('LBL_CHANGED')}
                                                                                        &nbsp;{vtranslate('LBL_TO')}&nbsp;<b style="white-space:pre;">{if $DETAIL.postvalue eq NULL}NULL{else}{Vtiger_Util_Helper::toVtiger6SafeHTML($DETAIL.postvalue)}{/if}</b>
										</div>
									{/if}
								{/foreach}
