...
var Vtiger_CustomView_Js = {
	...
	loadFilterView : function(url) {
		...
		AppConnector.request(url).then(
			function(data){
				...
				var view = Vtiger_CustomView_Js.getUrlParameter("view");
	...
	getColumnSelectElement : function() {
		if(Vtiger_CustomView_Js.columnSelectElement == false) {
			...
			if (app.getModuleName() == 'Opportunities') {
				Vtiger_CustomView_Js.columnSelectElement.find('option[data-field-name="agmt_id"], option[data-field-name="subagmt_nbr"]').remove();
			}
		}
	...
	saveFilter : function() {
	    if(getQueryVariable('mode') == 'showRelatedList') {
	        jQuery('#CustomView').append("<input type='hidden' name='fromRelatedList' value='1' />");
        }
	...
	registerSelect2ElementForColumnsSelection : function() {
		...
		app.changeSelectElementView(selectElement, 'select2', {maximumSelectionSize: 30,dropdownCss : {'z-index' : 0}});
	},

	registerAssignToAgentToggle : function() {
		var checkbox = jQuery('#assignToAgent');
		checkbox.on('change', function() {
			if(jQuery(this).is(':checked')) {
				//Show picklist
				jQuery('#assignedAgentLabel').removeClass('hide');
			} else {
				//Hide picklist
				jQuery('#assignedAgentLabel').addClass('hide');
			}
		});
	},

    hideStatusByRole: function() {
        var public_flag = $('#status[type="checkbox"]');
        if($('#is_vanline_user').val()) {
            public_flag.parent().hide();
            public_flag.val(0);
        }
	},

	checkRequiredFields: function(){
	    if(jQuery("#module").val() == "OrdersTask" && jQuery("#view").val() == "NewLocalDispatch"){
		var cvFunctionsInstance = new OrdersTask_LocalDispatchCustomView_Js;
		cvFunctionsInstance.checkLocalDispatchMandatoryColumns();
		cvFunctionsInstance.selectMandatoryFields();
	    }
	},
	
    registerSirvaEvents: function() {
        this.hideStatusByRole();
    },
	...
	registerEvents: function() {
		...
		Vtiger_CustomView_Js.registerAssignToAgentToggle();
		...
		Vtiger_CustomView_Js.checkRequiredFields();
                //OT5300
                if($('#resourceWidthSlider').length){
                    $('#resourceWidthSlider').slider();
                    $('#resourceWidthSlider').on("slide", function(slideEvt) {
                        $("#sliderVal").text(slideEvt.value);
                        $("input#resourceWidthValue").val(slideEvt.value);
                    });
                }
                //OT5553
                if($('#resourcecollapsed_checkbox').length){
                    $('#resourcecollapsed_checkbox').on("change", function(e) {
                        var checked = $(this).prop('checked')?'yes':'no';
                        $('input[name="resourcecollapsed"]').val(checked);
                    });
                }
	}
};
