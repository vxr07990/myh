...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{if $RECORD_ID neq ''}
			...
			{assign var="BUSINESSLINE_VALUE_LIST" value=explode(' |##| ',$RECORD_MODEL->get('agentcompgr_businessline'))}
			{assign var="BILLINGTYPE_VALUE_LIST" value=explode(' |##| ',$RECORD_MODEL->get('agentcompgr_billingtype'))}
			{assign var="AUTHORITY_VALUE_LIST" value=explode(' |##| ',$RECORD_MODEL->get('agentcompgr_authority'))}
			...
			<h3 class="span8 textOverflowEllipsis" title="{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} - {$BUSINESSLINE_VALUE_LIST} / {$BILLINGTYPE_VALUE_LIST} / {$AUTHORITY_VALUE_LIST}">{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} - {$BUSINESSLINE_VALUE_LIST} / {$BILLINGTYPE_VALUE_LIST} / {$AUTHORITY_VALUE_LIST}</h3>
		{else}
				...
				<td class="fieldLabel {$WIDTHTYPE}">
					{if $isReferenceField neq "reference"}<label class="muted pull-right marginRight10px">{/if}
						...
						{if $isReferenceField eq "reference"}
							...
							{if $REFERENCE_LIST_COUNT > 1 && $FIELD_MODEL->getName() neq 'agentcompgr_tariffcontract'}
		...
		{/if}
			...
			<br>
		{if $BLOCK_LABEL eq 'LBL_AGENTCOMPENSATION_GROUP'}
			{*include file=vtemplate_path('AgentCompensationItemsEdit.tpl', 'AgentCompensationItems')*}
			{include file=vtemplate_path('EscrowsEdit.tpl', 'Escrows')}
		{/if}
