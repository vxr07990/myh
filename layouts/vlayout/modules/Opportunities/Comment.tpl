			...
			<div class="commentTitle" id="{$COMMENT->getId()}">
				...
				<div class="row-fluid">
					...
					<div class="span11 commentorInfo">
						...
						{assign var=PARENT_RECORD_MODEL value=Vtiger_Record_Model::getInstanceById($COMMENT->get('related_to'))}
						<div class="inner">
							<span class="commentorName pull-left"><strong>{$COMMENTOR->getName()}</strong></span>
							<span class="span1">&nbsp;</span>
							<span class="span3 hide"><strong>{if $COMMENT->get('cf_comment_type') eq '1'}Lead Notes{elseif $COMMENT->get('cf_comment_type') eq '3'}Customer Facing Note{elseif $COMMENT->get('cf_comment_type') eq '2'}Survey Notes{else}Other Note{/if}</strong></span>
							<span class="span2"><a href="{$PARENT_RECORD_MODEL->getDetailViewUrl()}" title="{$PARENT_RECORD_MODEL->getModuleName()}"><strong>{$PARENT_RECORD_MODEL->getName()}</strong></a></span>
							...
							{if $COMMENT->get('note_source') && $COMMENT->get('note_source') neq 'moveCRM'}<br><span><strong>Provider : </strong>{$COMMENT->get('note_source')}</span>{/if}
							{if $COMMENT->get('provider') && $COMMENT->get('provider')}<br><span><strong>Source : </strong>{$COMMENT->get('provider')}</span>{/if}
				...
				<div class="pull-right commentActions">
					...
					{if $COMMENTS_MODULE_MODEL->isPermitted('EditView')}
						<span>
							...
							{if $CURRENTUSER->getId() eq $COMMENT->get('userid') && !$COMMENT->get('note_source') && !$COMMENT->get('provider')}
