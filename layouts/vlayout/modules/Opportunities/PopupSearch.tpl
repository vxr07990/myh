        ...
        <div class="span6">
            <form class="form-horizontal popupSearchContainer" onsubmit="return false;" method="POST">
                <div class="control-group margin0px">
                    {if $POPUP_MODE eq 'LeadMatching'}
			<input class="span3" type="text" {if $CONTACT_PHONE neq ''} value="{$CONTACT_PHONE}" {/if} placeholder="{vtranslate('LBL_TYPE_SEARCH_PHONE', $MODULE)}" id="searchvalue"/>&nbsp;&nbsp;
                    ...
                    *}
                    ...
                    <span class="customFilterMainSpan row-fluid">
                        {if $CUSTOM_VIEWS|@count gt 0}
                            <select id="popupRecordFilter" class="span12"
                                    data-placeholder="{vtranslate('LBL_SELECT_TO_LOAD_LIST', $RELATED_MODULE)}">
                                    ...
                                    <optgroup label=' {if $GROUP_LABEL eq 'Mine'} &nbsp; {else if} {vtranslate($GROUP_LABEL)} {/if}'>
                                        {foreach item="CUSTOM_VIEW" from=$GROUP_CUSTOM_VIEWS}
                                            <option id="filterOptionId_{$CUSTOM_VIEW->get('cvid')}" value="{$CUSTOM_VIEW->get('cvid')}"
                                                    ...
                                                    data-id="{$CUSTOM_VIEW->get('cvid')}" {if $VIEWID neq '' && $VIEWID neq '0'  && $VIEWID == $CUSTOM_VIEW->getId()} selected="selected" {elseif ($VIEWID == '' or $VIEWID == '0')&& $CUSTOM_VIEW->isDefault() eq 'true'} selected="selected" data-selected="selected" {/if} >{if $CUSTOM_VIEW->get('viewname') eq 'All'}{vtranslate($CUSTOM_VIEW->get('viewname'), $RELATED_MODULE)} {vtranslate($RELATED_MODULE, $RELATED_MODULE)}{else}{vtranslate($CUSTOM_VIEW->get('viewname'), $RELATED_MODULE)}{/if}{if $GROUP_LABEL neq 'Mine'} [ {$CUSTOM_VIEW->getOwnerName()} ] {/if}</option>
                                        {/foreach}
                                    </optgroup>
                                {/foreach}
                            </select>
