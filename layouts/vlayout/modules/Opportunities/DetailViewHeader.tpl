					...
					<div class="span7">
						<div class="pull-right detailViewButtoncontainer">
							...
							{foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
								{*{$DETAIL_VIEW_BASIC_LINK->get('linklabel')}*}
								{if $DETAIL_VIEW_BASIC_LINK->get('linklabel') eq 'LBL_EDIT'}{* OLD SECURITIES && $CREATOR_PERMISSIONS eq 'true'*}
								...
								{elseif $DETAIL_VIEW_BASIC_LINK->get('linklabel') eq 'LBL_BOOK_ORDER'}{* OLD SECURITIES && $CREATOR_PERMISSIONS eq 'true'*}
									<span class="btn-group">
										<button {if $DETAIL_VIEW_BASIC_LINK->get('status')}disabled{/if} class="btn btn-success" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
                                            {if $DETAIL_VIEW_BASIC_LINK->isPageLoadLink()}
                                            onclick="window.location.href='{$DETAIL_VIEW_BASIC_LINK->getUrl()}'"
                                            {else}
                                        onclick={$DETAIL_VIEW_BASIC_LINK->getUrl()}
								...
								{elseif $DETAIL_VIEW_BASIC_LINK->get('linklabel') eq 'LBL_SEND_EMAIL'}{* OLD SECURITIES && $CREATOR_PERMISSIONS eq 'true'*}
									<span class="btn-group">
										<button {if $DETAIL_VIEW_BASIC_LINK->get('status')}disabled{/if} class="btn" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
												{if $DETAIL_VIEW_BASIC_LINK->isPageLoadLink()}
											onclick="window.location.href='{$DETAIL_VIEW_BASIC_LINK->getUrl()}'"
												{else}
										onclick={$DETAIL_VIEW_BASIC_LINK->getUrl()}
								...
								{elseif $DETAIL_VIEW_BASIC_LINK->get('linklabel') eq 'LBL_STS_REGISTRATION' && getenv('INSTANCE_NAME') eq 'sirva'}{* OLD SECURITIES && $CREATOR_PERMISSIONS eq 'true'*}
									<span class="btn-group">
										<button {if $DETAIL_VIEW_BASIC_LINK->get('status')}disabled{/if} class="btn" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
												{if $DETAIL_VIEW_BASIC_LINK->isPageLoadLink()}
											onclick="window.location.href='{$DETAIL_VIEW_BASIC_LINK->getUrl()}'"
								...
								{elseif $DETAIL_VIEW_BASIC_LINK->get('linklabel') eq 'LBL_INTL_ONE_CLICK' && getenv('INSTANCE_NAME') eq 'sirva'}{* OLD SECURITIES && $CREATOR_PERMISSIONS eq 'true' *}
									<span class="btn-group">
										<button  class="btn" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
											{if $DETAIL_VIEW_BASIC_LINK->isPageLoadLink()}
												onclick="window.location.href='{$DETAIL_VIEW_BASIC_LINK->getUrl()}'"
							...
							{if $DETAILVIEW_LINKS['DETAILVIEW']|@count gt 0}
								{* OLD SECURITIES {if $CREATOR_PERMISSIONS eq 'true'} *}
									<span class="btn-group">
