	...
	{foreach item=FIELD_MODEL key=FIELD_NAME from=$SUMMARY_RECORD_STRUCTURE['SUMMARY_FIELDS']}
		{if $FIELD_MODEL->get('name') neq 'modifiedtime' && $FIELD_MODEL->get('name') neq 'createdtime'}
			<tr class="summaryViewEntries">
				...
				<td class="fieldValue" style="width:65%" id="{$FIELD_MODEL->get('name')}_value_td">
                    <div class="row-fluid">
                        <span data-fieldname="{$FIELD_MODEL->get('name')}" data-fielvalue="{$FIELD_MODEL->get('fieldvalue')}" class="value {if $FIELD_MODEL->get('uitype') eq '10' && !empty($FIELD_MODEL->get('fieldvalue'))}customToolTip{/if}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $FIELD_MODEL->get('uitype') eq '21'}style="word-wrap: break-word;white-space:pre-wrap;"{/if}>
	...
	<div class="span8">
		<div class="pull-right">
			<div>
				<p>
					<small>
						{vtranslate('LBL_CREATED_ON',$MODULE_NAME)} {Vtiger_Util_Helper::formatDateTimeIntoDayString(DateTimeField::convertToUserTimeZone($RECORD->get('createdtime'))->format('Y-m-d H:i:s'))}
					</small>
				</p>
			</div>
			<div>
				<p>
					<small>
						{vtranslate('LBL_MODIFIED_ON',$MODULE_NAME)} {Vtiger_Util_Helper::formatDateTimeIntoDayString(DateTimeField::convertToUserTimeZone($RECORD->get('modifiedtime'))->format('Y-m-d H:i:s'))}
					</small>
				</p>
			</div>
		</div>
