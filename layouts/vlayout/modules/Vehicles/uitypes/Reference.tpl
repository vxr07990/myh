...
<span class="add-on clearReferenceSelection cursorPointer {$MODULE}_editView_fieldName_{$FIELD_NAME}_clear">
...
<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" class="{if (($VIEW_NAME eq 'Edit') or ($MODULE_NAME eq 'Webforms'))} span7 {else} span8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
 value="{if $FIELD_MODEL->getFieldName() eq 'vehicles_agent_no'}
 {$FIELD_MODEL->getUITypeModel()->getEditViewFullName($displayId)}
 {else}{$FIELD_MODEL->getEditViewDisplayValue($displayId)}{/if}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"
...
{$referenceModule = Vtiger_Module_Model::getInstance($REFERENCE_LIST[0])}
{if $referenceModule}
	{assign var=IS_QUICK_CREATE_SUPPORTED value=$referenceModule->isQuickCreateSupported()}
{/if}
<!-- Show the add button only if it is edit view  -->
{if !$MODULE_QUICKCREATE_RESTRICTIONS}
	{assign var=MODULE_QUICKCREATE_RESTRICTIONS value=[]}
{/if}
{if (($VIEW_NAME eq 'Edit') or ($MODULE_NAME eq 'Webforms')) && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES) && !in_array($REFERENCE_LIST[0], $MODULE_QUICKCREATE_RESTRICTIONS)}
