	...
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_NAME neq 'self_haul_agentmanagerid' && $isReferenceField eq 'reference' && count($FIELD_MODEL->getReferenceList()) < 1}{continue}{/if}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
								{if $FIELD_NAME eq 'self_haul' && !$USER_MODEL->isAgencyAdmin()}
                                ...
                                {elseif $FIELD_NAME eq 'default_deposit_amount'}
                                    {assign var="DEPOSIT_TYPE" value=$RECORD_MODEL->get('default_deposit_type')}
                                    <div class="{if $DEPOSIT_TYPE eq 'Flat Amount'}hide{/if} percent_container">
                                        {include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
                                    </div>
                                    <div class="{if $DEPOSIT_TYPE neq 'Flat Amount'}hide{/if} currency_container">
                                        {assign var="FIELD_MODEL" value=$FIELD_MODEL->set('uitype',71)}
                                        {assign var="FIELD_MODEL" value=$FIELD_MODEL->set('fieldDataType','currency')}
                                        {include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
                                    </div>
                                {else}
                                    {include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
