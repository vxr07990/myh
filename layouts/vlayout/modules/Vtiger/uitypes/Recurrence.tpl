...
<div class="row-fluid">
	<div class="span">
		<input type="checkbox" name="recurringcheck" value="" {if $RECURRING_INFORMATION['recurringcheck'] eq 'Yes'}checked{/if}{if $OCCURRENCE} disabled{/if}/>&nbsp;&nbsp;
	</div>
	<div class="{if $RECURRING_INFORMATION['recurringcheck'] eq 'Yes'}show{else}hide{/if} span" id="repeatUI" style="width:90%;">
		<div class="row-fluid">
			<span class="span" style="width:150px;">
				...
				<select class="select2 input-mini" name="repeat_frequency"{if $OCCURRENCE} disabled{/if}>
			...
			<span class="span">
				<select class="select2 input-medium" style="width:100px;" name="recurringtype" id="recurringType"{if $OCCURRENCE} disabled{/if}>
			...
			<span class="span">
				...
				<span class="input-append date pull-right">
					<input type="text" id="calendar_repeat_limit_date" class="dateField input-small" name="calendar_repeat_limit_date" data-date-format="{$USER_MODEL->get('date_format')}"
						   ...
						   data-validation-engine='validate[required,funcCall[Vtiger_Date_Validator_Js.invokeValidation]]' style="width:100px;"{if $OCCURRENCE} disabled{/if} />
					<span class="add-on"><i class="icon-calendar"></i></span>
				</span>
			</span>
			{if $OCCURRENCE}<span>&nbsp;&nbsp;(Occurrence of <a href="{$RECURRENCE_ROOT->getDetailViewUrl()}" style="text-decoration:underline">event</a>)</span>{/if}
		</div>
		<div class="row-fluid {if $RECURRING_INFORMATION['eventrecurringtype'] eq 'Weekly'}show{else}hide{/if}"  id="repeatWeekUI" style="margin-top:10px;">
			...
			<span class="span10">
				<label class="checkbox inline" style="margin-left: 8px;"><input name="sun_flag" value="sunday" {$RECURRING_INFORMATION['week0']} type="checkbox"{if $OCCURRENCE} disabled{/if}/>{vtranslate('LBL_SM_SUN', $MODULE)}</label>
				<label class="checkbox inline" style="margin-left: 8px;"><input name="mon_flag" value="monday" {$RECURRING_INFORMATION['week1']} type="checkbox"{if $OCCURRENCE} disabled{/if}>{vtranslate('LBL_SM_MON', $MODULE)}</label>
				<label class="checkbox inline" style="margin-left: 8px;"><input name="tue_flag" value="tuesday" {$RECURRING_INFORMATION['week2']} type="checkbox"{if $OCCURRENCE} disabled{/if}>{vtranslate('LBL_SM_TUE', $MODULE)}</label>
				<label class="checkbox inline" style="margin-left: 8px;"><input name="wed_flag" value="wednesday" {$RECURRING_INFORMATION['week3']} type="checkbox"{if $OCCURRENCE} disabled{/if}>{vtranslate('LBL_SM_WED', $MODULE)}</label>
				<label class="checkbox inline" style="margin-left: 8px;"><input name="thu_flag" value="thursday" {$RECURRING_INFORMATION['week4']} type="checkbox"{if $OCCURRENCE} disabled{/if}>{vtranslate('LBL_SM_THU', $MODULE)}</label>
				<label class="checkbox inline" style="margin-left: 8px;"><input name="fri_flag" value="friday" {$RECURRING_INFORMATION['week5']} type="checkbox"{if $OCCURRENCE} disabled{/if}>{vtranslate('LBL_SM_FRI', $MODULE)}</label>
				<label class="checkbox inline" style="margin-left: 8px;"><input name="sat_flag" value="saturday" {$RECURRING_INFORMATION['week6']} type="checkbox"{if $OCCURRENCE} disabled{/if}>{vtranslate('LBL_SM_SAT', $MODULE)}</label>
			</span>
			{if $OCCURRENCE}<span>&nbsp;&nbsp;(Occurrence of <a href="{$RECURRENCE_ROOT->getDetailViewUrl()}" style="text-decoration:underline">event</a>)</span>{/if}
		</div>
		<div class="{if $RECURRING_INFORMATION['eventrecurringtype'] eq 'Monthly'}show{else}hide{/if}" id="repeatMonthUI" style="margin-top:10px;"RCa>
			<div class="row-fluid">
				<span class="span4">
					<span class="pull-right">
						<input type="radio" id="repeatDate" name="repeatMonth" checked value="date" {if $RECURRING_INFORMATION['repeatMonth'] eq 'date'} checked {/if}{if $OCCURRENCE} disabled{/if}/>
						<span class="alignMiddle" style="margin-left: 0.8em;">{vtranslate('LBL_ON', $MODULE)}</span>
					</span>	
				</span>
				<span class="span8">
					<input type="text" id="repeatMonthDate" class="input-mini" name="repeatMonth_date" data-validation-engine='validate[funcCall[Calendar_RepeatMonthDate_Validator_Js.invokeValidation]]' value="{if $RECURRING_INFORMATION['repeatMonth_date'] eq ''}2{else}{$RECURRING_INFORMATION['repeatMonth_date']}{/if}"{if $OCCURRENCE} disabled{/if}/>
			...
			<div class="row-fluid" id="repeatMonthDayUI" style="margin-top: 10px;">
				<span class="span4">
					<span class="pull-right">
						<input type="radio" id="repeatDay" name="repeatMonth" value="day" {if $RECURRING_INFORMATION['repeatMonth'] eq 'day'} checked {/if}{if $OCCURRENCE} disabled{/if}/>
						<span class="alignMiddle" style="margin-left: 0.8em;">{vtranslate('LBL_ON', $MODULE)}</span>
					</span>	
				</span>
				<span class="span3">
					<select id="repeatMonthDayType" class="select2 input-small" name="repeatMonth_daytype"{if $OCCURRENCE} disabled{/if}>
				...
				<span class="span5 margin0px">
					<select id="repeatMonthDay" class="select2 input-medium" name="repeatMonth_day"{if $OCCURRENCE} disabled{/if}>
