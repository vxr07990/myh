{strip}
    ...
    <input {if $FIELD_MODEL->isReadOnly()} readonly {/if} id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" class="dateField" name="{$FIELD_MODEL->getFieldName()}" data-date-format="{$dateFormat}"
    ...
    {if $TIMEZONE_FIELD_MODEL}
        ...
        {assign var=DATEZONE_FIELD_MODEL value=$MODULE_MODEL->getField($ZONE_FIELD_NAME)}
        {if $RECORD_MODEL->get($ZONE_FIELD_NAME)}
            {assign var=DATEZONE_FIELD_MODEL value=$DATEZONE_FIELD_MODEL->set('fieldvalue', $RECORD_MODEL->get($ZONE_FIELD_NAME))}
        {else}
            {assign var=DATEZONE_FIELD_MODEL value=$DATEZONE_FIELD_MODEL->set('fieldvalue', $USER_MODEL->get('time_zone'))}
        {/if}
        <select class="chzn-select" name="{$ZONE_FIELD_NAME}" data-validation-engine="validate[funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" >
            ...
            {foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
                <option value="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_NAME)}" {if trim(decode_html($DATEZONE_FIELD_MODEL->get('fieldvalue'))) eq trim($PICKLIST_NAME)} selected {/if}>{$PICKLIST_VALUE}</option>
            {/foreach}
        </select>&nbsp;
    {/if}
