...
{if $MODULEMODEL and $MODULEMODEL->isActive() and $FIELD_VALUE}
    ...
    {if $PERMISSION}
        {assign var=PHONE_FIELD_VALUE value=$FIELD_MODEL->getPhoneNumber($FIELD_VALUE)}
        {assign var=PHONE_NUMBER value=$PHONE_FIELD_VALUE|regex_replace:"/[-()\s]/":""}
        <a class="phoneField" class="phone-field" data-value="{$PHONE_NUMBER}" record="{$RECORD->getId()}" onclick="Vtiger_PBXManager_Js.registerPBXOutboundCall('{$PHONE_NUMBER}',{$RECORD->getId()})">{$FIELD_MODEL->getPhoneNumber($FIELD_MODEL->get('fieldvalue'))}</a>
    {else}
        {$FIELD_MODEL->getPhoneNumber($FIELD_MODEL->get('fieldvalue'))}
    {/if}
{else}
    {$FIELD_MODEL->getPhoneNumber($FIELD_MODEL->get('fieldvalue'))}
{/if}
