			...
			<div class="commentTitle" id="{$COMMENT->getId()}">
				...
				<div class="row-fluid">
					...
					<div class="span11 commentorInfo">
						...
						{if $PARENT_RECORD_MODEL->getModuleName() eq 'Opportunities'}
							<div class="inner">
								<span class="commentorName pull-left"><strong>{$COMMENTOR->getName()}</strong></span>
								<span class="span3">&nbsp;</span>
								<span class="span3"><strong>{if $COMMENT->get('cf_comment_type') eq '1'}Lead Notes{elseif $COMMENT->get('cf_comment_type') eq '3'}Customer Facing Note{elseif $COMMENT->get('cf_comment_type') eq '2'}Survey Notes{else}Other Note{/if}</strong></span>
								<span class="span3"><a href="{$PARENT_RECORD_MODEL->getDetailViewUrl()}" title="{$PARENT_RECORD_MODEL->getModuleName()}"><strong>{$PARENT_RECORD_MODEL->getName()}</strong></a></span>
