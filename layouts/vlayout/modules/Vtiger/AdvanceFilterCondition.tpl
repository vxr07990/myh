			...
			{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE}
				...
				{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
					...
					{if $FIELD_MODEL->getFieldDataType() eq 'agentpicklist'}
						...
						{*AHHHHHHHHHH YOU messed it up! You forgot the getCurrentUserModel! OOHH THE HUMANITY!*}
						{assign var=CURRENT_USER_MODEL value = Users_Record_Model::getCurrentUserModel()}
						{assign var=ACCESSIBLE_OWNERS value=$CURRENT_USER_MODEL->getAccessibleOwnersForUser($MODULE_MODEL->getName())}
						...
						{foreach key=OWNER_ID item=CURRENT_OWNER from=$ACCESSIBLE_OWNERS}
							...
							{else}
								{if $AGENTS_IN_PROGRESS eq true}
                                    ...
                                    {assign var=AGENT_DISPLAY value="$CURRENT_OWNER"}

                                    {$FIELD_INFO['picklistvalues']['Agents'][$OWNER_ID] = $AGENT_DISPLAY} {*Need to store the text value not the ID*} {* Nope *}
								{else}
                                    {if $IS_CUSTOMVIEWS eq 'true'}
                                        {$FIELD_INFO['picklistvalues']['Vanlines'][$AGENT_DISPLAY] = $AGENT_DISPLAY}
                                    {else}
                                        {$FIELD_INFO['picklistvalues']['Vanlines'][$OWNER_ID] = $AGENT_DISPLAY}
                                    {/if}
								{/if}
							{/if}
                                                {/foreach}
...
'					data-fieldinfo='{Vtiger_Util_Helper::toSafeHTML(ZEND_JSON::encode($FIELD_INFO))}'
            ...
            {* Required to display Commission Plan Group fields also while adding conditions *}
			...
			{* Required to display Item Code Mapping fields also while adding conditions *}
