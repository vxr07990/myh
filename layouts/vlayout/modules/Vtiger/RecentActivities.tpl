...
<div class="recentActivitiesContainer" id="updates">
	...
	<div>
		{if !empty($RECENT_ACTIVITIES)}
                {assign var=SHOWMOREUPDATES value='false'}
			<ul class="unstyled">
				{foreach item=RECENT_ACTIVITY from=$RECENT_ACTIVITIES}
					{assign var=PROCEED value= TRUE}
                                        {assign var=UPDATEINFOCOUNT value=1}
					{if ($RECENT_ACTIVITY->isRelationLink()) or ($RECENT_ACTIVITY->isRelationUnLink())}
						...
						{if !($RELATION->getLinkedRecord()) AND $RELATION->get('targetmodule') neq 'ParticipatingAgentsNonGuest'}
						...
						{else if $RECENT_ACTIVITY->isUpdate()}
							<li>
								...
								{foreach item=FIELDMODEL from=$RECENT_ACTIVITY->getFieldInstances()}
									{if $FIELDMODEL && $FIELDMODEL->getFieldInstance() && $FIELDMODEL->getFieldInstance()->isViewable() && $FIELDMODEL->getFieldInstance()->getDisplayType() neq '5' && $UPDATEINFOCOUNT lt 6}
										<div class='font-x-small updateInfoContainer'>
											<i>{vtranslate($FIELDMODEL->getName(),$MODULE_NAME)}</i> :&nbsp;
												...
												{if $FIELDMODEL->get('postvalue') neq '' && !($FIELDMODEL->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELDMODEL->get('postvalue') eq '0')}
													&nbsp;{vtranslate('LBL_TO')}&nbsp;<b style="white-space:pre;">{Vtiger_Util_Helper::toVtiger6SafeHTML($FIELDMODEL->getDisplayValue(decode_html($FIELDMODEL->get('postvalue'))))}{if $FIELDMODEL->get('timezone')}{$FIELDMODEL->get('timezone')}{/if}</b>
												{/if}
										</div>
									{/if}
                                                                        {assign var=UPDATEINFOCOUNT value=$UPDATEINFOCOUNT+1}
                                                                        {if $UPDATEINFOCOUNT eq 6}{assign var=SHOWMOREUPDATES value='true'}{/if}
								{/foreach}
						...
						{else if ($RECENT_ACTIVITY->isRelationLink() || $RECENT_ACTIVITY->isRelationUnLink())}
							<li>
								<div class="row-fluid">
									...
									<span><strong>{$RECENT_ACTIVITY->getModifiedBy()->getName()}</strong></span> <span>
											{if $RECENT_ACTIVITY->isRelationLink()}
												{vtranslate('LBL_ADDED', $MODULE_NAME)}
											{else}
												{vtranslate('LBL_REMOVED', $MODULE_NAME)}
											{/if} </span><span>
                                        {if $RELATION->get('targetmodule') neq 'ParticipatingAgentsNonGuest'}
                                        ...
                                        {else}
                                            {assign var=AGENT_INFO value=$RELATION->getParticipatingAgentInfo()}
                                            <strong>{$AGENT_INFO['name']}</strong> {vtranslate('as', $MODULE_NAME)} <strong>{vtranslate($AGENT_INFO['type'], $MODULE_NAME)}</strong></span>
                                        {/if}
		...
		<div class="row-fluid" id="moreLink">
		    {if $PAGING_MODEL->isNextPageExists() or $SHOWMOREUPDATES eq 'true'}
