...
{strip}
			...
			<div class="menuBar row-fluid">
					...
					<ul class="nav modulesList" id="largeNav">
						...
						{foreach key=moduleName item=moduleModel from=$topMenus name=topmenu}
							...
							<li class="{$topmenuClassName}">
								<a id="menubar_item_{$moduleName}" href="{$moduleModel->getDefaultUrl()}" {if $MODULE eq 'Orders' && $LDDList eq 'true' && $moduleName eq 'LongDistanceDispatch'} class="selected" {elseif $LDDList neq 'true' && $MODULE eq $moduleName} class="selected" {/if} ><strong>{$translatedModuleLabel}</strong></a>
							</li>
                            {if $moduleName eq 'Calendar' && getenv('INSTANCE_NAME') == 'graebel'}
                                {assign var='toDoURLPre' value = getenv('SITE_URL')|cat:'/index.php?module=Calendar&parent=&page=1&view=List&viewname=19&orderby=&sortorder=&search_params=%5B%5B%5B%22activitytype%22%2C%22e%22%2C%22Task%22%5D%2C%5B%22assigned_user_id%22%2C%22c%22%2C%22'}
                                {assign var = 'searchUser' value = $USER_MODEL->get('first_name')}
                                {if empty($searchUser)}
                                    {assign var='searchUser' value=$USER_MODEL->get('last_name')}
                                    {else}
                                    {assign var='searchUser' value=$searchUser|cat:'+'|cat:$USER_MODEL->get('last_name')}
                                {/if}
                                {assign var='toDoURLPost' value = '%22%5D%5D%5D'}
                                {assign var = 'toDoURL' value = $toDoURLPre|cat:$searchUser|cat:$toDoURLPost}
                                <li class="{$topmenuClassName}">
                                        <a id="menubar_item_todo" href="{$toDoURL}"><strong>To Do</strong></a>
                                </li>
                            {/if}
						{/foreach}
					</ul>
							...
							<div class="dropdown-menu">
								<div class="shortDropdown">
									{foreach key=parent item=moduleList from=$moreMenus name=more}
										{if $moduleList}
											<strong>{vtranslate("$parent",$moduleName)}</strong><hr>
						...
						<li class="dropdown" id="moreMenu">
							...
							<div class="dropdown-menu moreMenus" {if ($NUMBER_OF_PARENT_TABS <= 2) && ($NUMBER_OF_PARENT_TABS != 0)}style="width: 30em;"{elseif $NUMBER_OF_PARENT_TABS == 0}style="width: 10em;"{/if}>
								{foreach key=parent item=moduleList from=$moreMenus name=more}
									...
									{/if}
								{/foreach}
								{if $MENU_CLEANER_TOGGLE}
									<div class="row-fluid">
										<a id="menubar_item_moduleShortcuts" style="color: #2b2b2b" href="index.php?module=MenuCleaner&view=List" class="pull-right">{vtranslate('Module Shortcuts',$MODULE)}</a>
									</div>
								{/if}
				...
				<div class="span3 marginLeftZero pull-right" id="headerLinks">
					<span id="headerLinksBig" class="pull-right headerLinksContainer">
						...
						<span class="dropdown span settingIcons" style='margin-top:-2px;'>
							{if getenv('VIEW_BUGLIST')}
								<a href="javascript:window.open('{getenv('BUGLIST_URL')}')">
									<i title="View Bugs List" class="icon-white icon-th-list alignMiddle"></i>
								</a>
							{/if}
						</span>
						<span class="dropdown span settingIcons">
							{if getenv('REPORT_BUGS')}
								{assign var=URL_VAR value='url='}
								{assign var=CURRENT_URL value=$smarty.server.HTTP_HOST|cat:$smarty.server.REQUEST_URI|replace:'&':'::'|urlencode}
								{assign var=SEND_URL value=$URL_VAR|cat:$CURRENT_URL}
								{assign var=URL value=getenv('BUG_REPORT_URL')|cat:'?'|cat:$SEND_URL};
								<a href="javascript:window.open('{$URL}')">
									<img src='layouts/vlayout/skins/images/bug.png' />
								</a>
							{/if}
						</span>
						{foreach key=index item=obj from=$HEADER_LINKS}
							...
							{if $title eq 'LBL_FEEDBACK'}{continue}{/if}
	...
	<div class="announcement noprint" id="announcement">
		<span>{if !empty($announcement)}{$announcement}{else}{vtranslate('LBL_NO_ANNOUNCEMENTS',$MODULE)}{/if}</span>
	</div>
