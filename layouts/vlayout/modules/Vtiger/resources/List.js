...
jQuery.Class("Vtiger_List_Js",{
	...
	disableMassAction: function(element, onclick) {
		if(typeof onclick == "undefined"){
			onclick = '';
		}
		element.attr('style',"opacity:0.6;");
		element.children().first().attr('onclick',onclick).attr('style','cursor: default');
	},

	// @var jQuery Object element
	// @var String onclick
	enableMassAction: function(element,onclick) {
		element.attr('style','');
		element.children().first().attr('onclick',onclick).attr('style','');
	},
   ...
   getInstance: function(){
	...
	triggerMassAction : function(massActionUrl,callBackFunction,beforeShowCb, css, redirect) {
        // So apparently IE doesn't support default parameters in function declarations, so here we are.
		...
		if(massActionUrl.indexOf('selectedIds') == -1)  {
			var validationResult = listInstance.checkListRecordSelected();
			var selectedIds = listInstance.readSelectedIds();
		} else {
			var validationResult = true;
			var selectedIds = massActionUrl.split(/\[(.*?)\]/);
			selectedIds = [selectedIds[1]];
		}

		//IF it's duplicateRecords then only select one record.
		if((massActionUrl.indexOf('duplicateRecords')> -1) && typeof selectedIds == 'object' && selectedIds.length != 1){
            return alert(app.vtranslate('JS_PLEASE_SELECT_ONLY_ONE_RECORD'));
		}

		if(jQuery('[name="movehq"]').val()) {
            if (massActionUrl.indexOf('duplicateRecords') > -1 && massActionUrl.indexOf('module=Employees') > -1) {
                var duplicateURL = 'index.php?module=Employees&view=Edit&record='+selectedIds[0]+'&isDuplicate=true';
                window.location.href = duplicateURL;
                return true;
			}
			if (massActionUrl.indexOf('duplicateRecords') > -1 && massActionUrl.indexOf('module=MenuCreator') > -1) {
                var duplicateURL = 'index.php?module=MenuCreator&view=Edit&record='+selectedIds[0]+'&isDuplicate=true';
                window.location.href = duplicateURL;
                return true;
            }
        }

		if(validationResult != true || massActionUrl.indexOf('printRecords') > -1){
			var progressIndicatorElement = jQuery.progressIndicator();
			// Compute selected ids, excluded ids values, along with cvid value and pass as url parameters
			// var selectedIds;
			var excludedIds;
			...
			if(massActionUrl.indexOf('selectedIds') == -1) {
				if(validationResult && massActionUrl.indexOf('printRecords') > -1){
					selectedIds = listInstance.getAllViewedIds(true);
			...
			AppConnector.request(actionParams).then(
				function(data) {
					...
					if(data) {
						...
						//if it's duplicateRecords then reload the page because we're done.
						if(massActionUrl.indexOf('duplicateRecords') > -1){
							document.location.reload(true);
							return;
						}
						//otherwise let it post up the modal window.
						app.showModalWindow(data,function(data){
						...
						jQuery('.print-records').on('click', function(){
							jQuery.getScript('libraries/PrintArea/jquery.PrintArea.js', function(){
						        ...
						        if (msie > 0 || trident > 0 || edge > 0){
									jQuery('#massPrintContents').printArea({
											mode       : "popup",
											standard   : "html5",
											popClose   : true,
											extraHead  : '',
	...
	triggerMassEdit : function(massEditUrl) {
		...
		if(selectedCount == 1){
            ...
            var url = jQuery('.record'+selectedIdObj).find('.icon-pencil').parent().attr('href');
            window.location.href = url;
            return;
        }else if(selectedCount > 500) {
            var params = {
                title : app.vtranslate('JS_MESSAGE'),
    ...
    getListviewAdditionParams : function (massActionUrl) {
        var listInstance = Vtiger_List_Js.getInstance();
    ...
    checkAndTriggerMassEdit : function (massEditUrl) {
        var search_params = Vtiger_List_Js.getListviewAdditionParams(massEditUrl);
        Vtiger_Helper_Js.checkEditableAndDeletable({
            module: app.getModuleName(),
            actionword: 'Edit'
        }, search_params).then(function (data) {
            if (data) {
                if (data.length > 0) {
                    var message = data.join(", ");
                    bootbox.alert(message+app.vtranslate('JS_UNEDITABLE_N'));
                }else {
                    Vtiger_List_Js.triggerMassEdit(massEditUrl);
                }
            }else{
                Vtiger_List_Js.triggerMassEdit(massEditUrl);
            }
    ...
    checkAndTriggerMassDelete : function (massEditUrl) {
        var search_params = Vtiger_List_Js.getListviewAdditionParams(massEditUrl);
        Vtiger_Helper_Js.checkEditableAndDeletable({
            module: app.getModuleName(),
            actionword: 'Delete'
            ...
            if (data) {
                if (data.length > 0) {
                    var message = data.join(", ");
                    bootbox.alert(message+app.vtranslate('JS_UNDELETABLE_N'));
                }else {
                    Vtiger_List_Js.massDeleteRecords(massEditUrl);
                }
            }else{
                Vtiger_List_Js.massDeleteRecords(massEditUrl);
            }
	...
	getAllViewedIds : function(decode){
		var selectedIds = new Array();
	...
	*/
    ...
    registerDeleteRecordClickEvent: function(){
        ...
        listViewContentDiv.on('click','.deleteRecordButton',function(e){
            var elem = jQuery(e.currentTarget);
            var isPrecheck = elem.closest('tr').data('is-precheck');
    ...
    registerEditRecordClickEvent: function(){
        ...
        listViewContentDiv.on('click','.editRecordButton',function(e){
            var elem = jQuery(e.currentTarget);
            var isPrecheck = elem.closest('tr').data('is-precheck');
            var editUrl = elem.data('edit-url');
            var recordId = elem.closest('tr').data('id');
            if(isPrecheck) {
                Vtiger_Helper_Js.checkEditableAndDeletable({
                    module: app.getModuleName(),
	...
	formatPhoneFieldOnChange : function() {
		jQuery('td').each(function() {
			if(jQuery(this).data('field-type') == 'phone') {
				var input = jQuery(this).html().replace(/\D/g, '');
...
function triggerDeleteFilter() {
	var selectedFilterElement = jQuery('#customFilter').find(':selected');
	var cvid = selectedFilterElement.val();
	var lockedViews = JSON.parse(jQuery('input[name="lockedViews"]').val());
	if(lockedViews.indexOf(cvid) != -1) {
		bootbox.alert(app.vtranslate('JS_LBL_NO_DELETE_FILTER_PERMISSIONS'));
		return;
	}
	var message = app.vtranslate('JS_LBL_ARE_YOU_SURE_YOU_WANT_TO_DELETE');
	Vtiger_Helper_Js.showConfirmationBox({'message' : message}).then(
		function(e) {
			var deleteUrl = selectedFilterElement.data('deleteurl');
			var newEle = '<form action='+deleteUrl+' method="POST">'+
				'<input type = "hidden" name ="'+csrfMagicName+'"  value=\''+csrfMagicToken+'\'>'+
				'</form>';
			var formElement = jQuery(newEle);
...
function triggerCreateFilter() {
	Vtiger_CustomView_Js.loadFilterView(jQuery('#createFilter').data('createurl'));
}
