...
jQuery.Class("Vtiger_Header_Js", {
    ...
    getQuickCreateForm: function(url, moduleName, params) {
		// console.dir('in getQuickCreateForm');
		// console.dir('url');
		// console.dir(url);
    ...
    quickCreateSave: function(form) {
        ...
        var timeFields = jQuery('[name="time_fields[]"]',form);
        if(timeFields.length > 0){
            var timeFieldVal = new Array();
            timeFields.each(function () {
                timeFieldVal.push(jQuery(this).val());
            });
            quickCreateSaveUrl["time_fields[]"] =timeFieldVal;
        }
...
//        }
    ...
    handleQuickCreateData: function(data, params) {
        ...
        app.showModalWindow(data, function(data) {
            ...
            editViewInstance.registerBasicEvents(quickCreateForm, params.data, true);
	...
	adjustContentHeight: function(){
		if(app.getViewName() === 'Detail' || app.getViewName() === 'Calendar' || app.getViewName() === 'ExtensionImport'){
			...
			bodyHeight = jQuery('div.editViewContainer').innerHeight();
		}
    ...
    registerEvents: function() {
		...
		if(jQuery.browser.msie && jQuery.browser.version < 9.0) {
			if(app.getCookie('oldbrowser') != 'true') {
				...
				window.location.href = 'Browser_compatibility.html';
			}
		}
        ...
        jQuery('#moreMenu').click(function(e) {
			var leftOffset = jQuery('#moreMenu').offset().left; // OT Defect 11558
            ...
            } else if(leftOffset < 270) { // OT Defect 11558
				moreMenu.css('left', (-1)*leftOffset);
				if(jQuery('#moreMenuStyle').length == 0) {
					jQuery('head').append('<style type="text/css" id="moreMenuStyle">.moreMenus:before,.moreMenus:after{left:'+(leftOffset+22)+'px !important}</style>');
				}
			}
        });
