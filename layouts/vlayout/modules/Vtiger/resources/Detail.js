...
jQuery.Class("Vtiger_Detail_Js",{
	...
	getInstance: function(){
        if( Vtiger_Detail_Js.detailInstance == false ){
			...
			{
				var instance = app.currentPageController;
			}
			else if(typeof window[moduleClassName] != 'undefined'){
	...
	I: function(){
		return Vtiger_Detail_Js.getInstance();
	},
	 ...
	 */
	triggerSendEmail : function(extraParams, module){
    ...
    triggerSendRegistration : function(params, module){
        Vtiger_Detail_Js.triggerDetailViewAction(params, Vtiger_Detail_Js.RegistrationCallback);
    },

    RegistrationCallback : function(data) {
        if (!data) {
            return;
        }
        if (!data.length) {
        ...
        try {
            ...
            //couldn't parse the data as a json
            //since I haven't account for the non-json return case, you'll have to deal with this
            return;
        }

        var message = '';
        if (data.success) {
            message = app.vtranslate('LBL_DEFAULT_SUCCESSFUL_REGISTRATION');
            var result = data.result;
            if (result) {
                if (result.message) {
                    message = result.message;
                }
                if (result.registrationNumberField && result.registrationNumber) {
                    var registrationField = jQuery("[id$='Value_" + result.registrationNumberField + "']");
                    if (typeof registrationField != "undefined") {
                        registrationField.find('span').html(result.registrationNumber);
                    }
                }
            }
        } else {
            ...
            if (!message) {
                message = app.vtranslate('LBL_DEFAULT_FAILED_REGISTRATION');
            }
        }
        if (message) {
            app.showModalWindow(message, {'text-align': 'left'});
        }
    },
    ...
    checkAndDeleteRecord : function (deleteRecordActionUrl) {
        Vtiger_Helper_Js.checkEditableAndDeletable({
            module: app.getModuleName(),
            actionword: 'Delete',
            ids: app.getRecordId()
        }).then(function (data) {
            if (data) {
                if (data.length > 0) {
                    bootbox.alert(app.vtranslate('JS_UNDELETABLE_1'));
                }else {
                    Vtiger_Detail_Js.deleteRecord(deleteRecordActionUrl);
                }
            }else{
                Vtiger_Detail_Js.deleteRecord(deleteRecordActionUrl);
            }
	...
	initializeAddressAutofill : function(moduleName) {
		var thisInstance = this;
		thisInstance.autofillModuleName = moduleName;
		thisInstance.addressAutofill = true;
		...
		if(jQuery('#'+moduleName+'_editView_fieldName_bill_code').length) {
			...
			google.maps.event.addListener(autocompleteBillingZip, 'place_changed', function() {
				thisInstance.fillInAddress('Billing', autocompleteDestinationZip);
		...
		if(jQuery('#'+moduleName+'_editView_fieldName_vanline_address1').length) {
			...
			google.maps.event.addListener(autocompleteAgentAdd, 'place_changed', function() {
				thisInstance.fillInAddress('Vanline', autocompleteAgentAdd);
			});
		}
		...
		if(jQuery('#'+moduleName+'_editView_fieldName_vanline_zip').length) {
			...
			google.maps.event.addListener(autocompleteAgentZip, 'place_changed', function() {
				thisInstance.fillInAddress('Vanline', autocompleteAgentZip);
			});
		}
	...
	fillInAddress : function(formType, autocomplete) {
		...
		for (var i=0; i<place.address_components.length; i++) {
			...
			if(addressType == 'street_number') {
				...
				if(jQuery('#'+form[addressType]).length) {
					jQuery('#'+form[addressType]).val(val);
					thisInstance.saveItem(jQuery('#'+form[addressType]));
				}
		...
		if(jQuery('#'+form['street_address']).val() != 'Will Advise'){
			jQuery('#'+form['street_address']).val(street_address);
		}

		thisInstance.saveItem(jQuery('#'+form['street_address']));
	},

	saveItem : function(input_field) {
		var thisInstance = this;
		var currentTdElement = input_field.closest('td');
		...
		if(ajaxEditNewValue == previousValue) {
		...
		thisInstance.saveFieldValues(fieldNameValueMap).then(function(response) {
			...
			fieldElement.trigger(thisInstance.fieldUpdatedEvent,{'old':previousValue,'new':ajaxEditNewValue});
	...
	registerRelatedRowClickEvent: function(){
		...
		detailContentsHolder.on('click','.listViewEntries',function(e){
            ...
            if(targetElement.closest('td').find('select').hasClass('taskstatus')) return;
	...
	registerBlockAnimationEvent : function(){
		...
		detailContentsHolder.on('click','.blockToggle',function(e){
			...
			var bodyContents = closestBlock.children('tbody');
			...
			if(data.mode == 'show'){
				hideHandler();
				currentTarget.hide();
				closestBlock.find("[data-mode='hide']").first().show();
			}else{
				showHandler();
				currentTarget.hide();
				closestBlock.find("[data-mode='show']").first().show();
	...
	registerBlockStatusCheckOnLoad : function(){
		...
		blocks.each(function(index,block){
            ...
            var extraBlock=currentBlock.data('extra-block');
			var headerAnimationElement = currentBlock.find('.blockToggle').not('.hide');
			var bodyContents = currentBlock.children('tbody');
			...
			if(value != null){
				if(value == 1){
					...
					currentBlock.find("[data-mode='show']").first().show();
					bodyContents.show();
				} else {
					headerAnimationElement.hide();
					currentBlock.find("[data-mode='hide']").first().show();
					bodyContents.hide();
				}
			}
		});
  ...
  registerEventsForActionButtons : function(){
    var thisInstance = this;
    var recordId = jQuery('[name="recordId"]').val();
    var relatedModule = jQuery('[name="relatedModuleName"]').val();

    jQuery('[name="previewPrint"]').on('click',function(e){
      Vtiger_List_Js.triggerMassAction('index.php?module=Leads&view=MassActionAjax&mode=printRecords&selectedIds=[' + recordId + ']&relatedModule=' + relatedModule);
    });


    jQuery('[name="previewForward"]').on('click',function(e){
      var module = "Emails";
      var progressIndicatorElement = jQuery.progressIndicator();
      Vtiger_Helper_Js.checkServerConfig(module).then(function(data){
        if(data == true){
          var url = 'index.php?module=Emails&view=ComposeEmail&record=' + recordId + '&mode=emailForward&relatedLoad=true';
    ...
    var thisInstance = this;
		...
		detailContentsHolder.on('click','[name="emailsRelatedRecord"]',function(e){
			...
			params['relatedLoad'] = true;
			popupInstance.show(params).then(function(){
	...
	registerEventForRelatedTabClick : function(){
		...
		jQuery('.related', detailContainer).on('click', 'li', function(e, urlAttributes){
			...
			var element = jQuery('<div></div>');
			if(tabElement.hasClass('linkToMovePolicies')){
				return thisInstance.showMovePoliciesPopup();
			}
			element.progressIndicator({
				'position':'html',
				'blockInfo' : {
	...
	showMovePoliciesPopup: function () {
		var thisInstance = this;
		...
		AppConnector.request(params).then(
			function (data) {
				...
				thisInstance.detailViewContentHolder = jQuery('.modelContainer');
				thisInstance.registerBlockAnimationEvent();
				thisInstance.detailViewContentHolder = false;
			}
		);
	},
    /**
	...
	changeLabelsBasedOnCountry : function() {
		var originCountry = $("[id$='fieldValue_origin_country']").children().html();
		var destinationCountry = $("[id$='fieldValue_destination_country']").children().html();

		if(originCountry) {
			console.log('Before: '+originCountry);
			originCountry = originCountry.trim();
			console.log('After: '+originCountry);
			if(originCountry == 'Canada') {
				$("[id$='fieldLabel_origin_state']").children().html('Origin Province');
				$("[id$='fieldLabel_origin_zip']").children().html('Origin Postal Code');
			}
		}
		...
		if(destinationCountry) {
			...
			if(destinationCountry == 'Canada') {
				$("[id$='fieldLabel_destination_state']").children().html('Destination Province');
				$("[id$='fieldLabel_destination_zip']").children().html('Destination Postal Code');
			}
		}
	},


	registerEventHiddenSomeFields : function () {
		var hiddenFields  = [
			'business_line',
			'isconvertedfromlead',
			'converted_from',
        ...
        if(!jQuery('input[name="movehq"]').val()){
            hiddenFields.push('created_user_id');
        }
		...
		if (modulename !== 'OrdersTask'){
			var fields = jQuery(container).find('td.narrowWidthType');
			$.each( fields, function( key , itemElement ) {
				var attrIdElement = jQuery(itemElement).attr('id');
				if (attrIdElement!==undefined){
					$.each( hiddenFields, function( key , hiddenField ) {
						var matchKey = attrIdElement.match(hiddenField+'$');
						if (matchKey){
							if(hiddenField == 'business_line' &&
								jQuery('[id$="_detailView_fieldLabel_business_line2"]').length == 0)
							{
								return;
							}
							Vtiger_Edit_Js.hideCell(jQuery('span',itemElement));
						}
					});
				}
			});
	...
	registerEventForMovePolicies: function(){
		var thisInstance = this;
		var detailContentsHolder = thisInstance.getContentHolder();
		var detailContainer = detailContentsHolder.closest('div.detailViewInfo');
		jQuery('.related', detailContainer).on('click', '.linkToMovePolicies', function(e, urlAttributes){
			 thisInstance.showMovePoliciesPopup();
		});
	},
	registerEvents : function(){
		...
		this.changeLabelsBasedOnCountry();
		Vtiger_Helper_Js.showHorizontalTopScrollBar();
    ...
    try {
      ...
      console.log(e);
		...
		this.registerEventHiddenSomeFields();
		this.registerEventForMovePolicies();
		...
		detailContentsHolder.on('click','.detailViewThread',function(e){
			...
			// };
      window.location.href = recentCommentsTab.data('url') + '#' + commentId;
		...
		detailContentsHolder.on('click','.moreRecentComments', function(){
			var recentCommentsTab = thisInstance.getTabByLabel(thisInstance.detailViewRecentCommentsTabLabel);
			var lnk = recentCommentsTab.children().attr('href');
			if(lnk)
			{
		...
		detailContentsHolder.on('click', '.moreRecentUpdates', function (e) {
			var isSummaryView = jQuery('li.active[data-link-key="LBL_RECORD_SUMMARY"]').length > 0;
			if(isSummaryView) {
				e.stopImmediatePropagation();
				jQuery('li[data-label-key="LBL_UPDATES"] a')[0].click();
			}
		});
		...
		detailContentsHolder.on('click','.moreRecentDocuments', function(){
			...
			recentDocumentsTab.find('a')[0].click();
		});
		...
		this.registerEventForDetailviewEditBtn();

		// this.registerLocalStorageEvents();
	},
    ...
    registerEventForDetailviewEditBtn: function () {
        var thisInstance = this;
        var detailViewContainer = jQuery('div.detailViewContainer');

        detailViewContainer.on("click",'.btnDetailViewEdit', function (event) {
            Vtiger_Helper_Js.checkEditableAndDeletable({
                module: app.getModuleName(),
                ids: app.getRecordId(),
                actionword: 'Edit',
                ...
                if (data) {
                    if (data.length > 0) {
                        bootbox.alert(app.vtranslate('JS_UNEDITABLE_1'));
                        event.preventDefault();
                    }else {
                        var idRecord = app.getRecordId();
                        window.location.href = 'index.php?module='+app.getModuleName()+'&view=Edit&record=' + idRecord;
                    }
                }else{
                    var idRecord = app.getRecordId();
                    window.location.href = 'index.php?module='+app.getModuleName()+'&view=Edit&record=' + idRecord;
