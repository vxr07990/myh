...
var Vtiger_Index_Js = {
	...
	broadcastMessage : function(message) {
		localStorage.setItem('moveCRMmessage',JSON.stringify(message));
		localStorage.removeItem('moveCRMmessage');
	},
				...
				AppConnector.request(params).then(
					function(data) {
						...
						if(data) {
							...
							if (length>1) {
								emailFields[0].checked = true;
							} else {
								emailFields.attr('checked','checked');
							}

								var params = form.serializeFormData();
								if(saveParam['documentIds'] && Array.isArray(saveParam['documentIds'])){
									params['documentIds'] = JSON.stringify(saveParam['documentIds']);
								}
								if(saveParam['module']){
									params['module'] = saveParam['module'];
								}
								var timer = emailEditInstance.showComposeEmailForm(params);
...
//										emailEditInstance.registerEvents();
	...
	registerActivityReminder : function() {
		...
		activityReminder = activityReminder * 100;
		if(activityReminder != '') {
		...
		var url = 'index.php?module=Vtiger&action=GetNotifications&mode=resetReminders';
	...
	requestNotification : function() {
		if(window.notificationTracker==undefined){
			window.notificationTracker = [];
		}
		var url = 'index.php?module=Vtiger&action=GetNotifications&mode=getReminders';
		AppConnector.request(url).then(function(data){
			if(data.success && data.result) {
				for(i=0; i< data.result.length; i++) {
					var record  = data.result[i];
					if($.inArray(data.result[i], window.notificationTracker)==-1){
						window.notificationTracker.push(data.result[i]);
						Vtiger_Index_Js.showNotificationPopup(record);
					}
					else{
	 ...
	 */
	//<a class="btn btn-success" href="'+record.onClickTarget+'">Go To</a><a class="btn btn btn-danger mar" goTo="'+record.onCloseCall+'" href="#">Dont Show Again</a>
	showNotificationPopup : function(record) {
		var params = {
			title: record.subject+'<br><br>',
			text: '<span class="right"><h4>'+record.message.substring(0, 500)+((record.message.length>500)?'...':'')+'</h4></span></div>',
			addclass:'vtReminder',
		...
		jQuery.each(record.buttons, function(i, val) {
			console.log(i);
			if(i!=''){
				params.confirm.buttons.push(
					{
						text: i,
						type: val.type,
						click: function(notice){
							AppConnector.request(val.link).then(function(data){
								if(!data.success && notice.get().find('.error').length == 0) {
									notice.update({ title: '<span style="position: relative; top: 8px;"><a target="_blank" href="'+record.onClickTarget+'">There was an error marking <b class="error">'+record.subject+'</b> as read!'+'</a></span>'});
								}
								else
									notice.remove();
							});
		...
		params.confirm.buttons.push({text: 'Hide', addClass:'dismiss'});



		var notify = Vtiger_Helper_Js.showPnotify(params);
		notify.get().click(function(e){
			if($(e.target).is('.dismiss')){
				var url = 'index.php?module=Vtiger&action=GetNotifications&mode=blockReminders&targetModule='+record.module+'&id='+record.id;
				AppConnector.request(url).then(function(data){notify.remove();});
			}
			else if($(e.target).not('button')){
				if(record.onClickTarget!=''){
					window.open(record.onClickTarget,'_blank');
					var url = 'index.php?module=Vtiger&action=GetNotifications&mode=blockReminders&targetModule='+record.module+'&id='+record.id;
					AppConnector.request(url).then(function(data){notify.remove();});
				}
				else
