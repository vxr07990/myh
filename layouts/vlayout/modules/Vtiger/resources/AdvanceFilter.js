...
jQuery.Class("Vtiger_AdvanceFilter_Js",{
	...
	getInstance: function(container){
		...
		if(typeof window[moduleClassName] != 'undefined'){
			instance = new window[moduleClassName](container);
		}else{
	...
	allConditionValidationNeededFieldList : ['double', 'integer', 'currency','referencemultipicklist','referencemultipicklist'],
	...
	loadFieldSpecificUi : function(fieldSelect) {
		...
		if(typeof fieldInfo != 'undefined') {
                    if(fieldInfo.type == 'custompicklist'){
                        fieldInfo.type = 'picklist';
                    }
			fieldType = fieldInfo.type;
			fieldInfo.modulename = selectedOption.data('fieldmodulename');
		}
		...
		if(fieldSpecificUi.find('.add-on').length > 0){
            ...
            fieldSpecificUi.find('input[type="text"]').css('width','70%');
		} else {
		...
		if(fieldSpecificUi.is('input.select2')){
            if(fieldModel.getType() == 'referencemultipicklist' || fieldModel.getType() == 'referencemultipicklistall'  || fieldModel.getType() == 'personnelpicklist'){
                fieldSpecificUi.data('fieldinfo',JSON.stringify(fieldInfo));
                app.showReferenceMultiSelectView(fieldSpecificUi);
            }else{
...
Vtiger_Multiagent_Field_Js('AdvanceFilter_Multiagent_Field_Js',{},{
...
Vtiger_Date_Field_Js('AdvanceFilter_Date_Field_Js',{},{
	...
	getUi : function() {
        ...
        var readonly = (module == 'Reports' ? '' : 'ReadOnly="true"');
		if(comparatorSelectedOptionVal == 'bw' || comparatorSelectedOptionVal == 'custom'){
                        var html = '<div class="date"><input class="dateField" data-calendar-type="range" name="'+ this.getName() +'" data-date-format="'+ this.getDateFormat() +'" type="text" ' + readonly + ' value="'+  this.getValue() + '"></div>';			var element = jQuery(html);
