...
jQuery.Class('Vtiger_Widget_Js',{
	...
	postLoadWidget : function() {
		...
		this.restrictContentDrag();
                ...
                //move text label so it dont cuts off below
		jQuery('.jqplot-axis.jqplot-xaxis').css('padding','1%');
                //move hole graphic to the right so the first text label dont cuts off if its long
                var widgetName = this.container["0"].dataset.name;
                if(widgetName === 'LeadsBySource' || widgetName === 'LeadsByStatus'){
                    var widgetId = this.container["0"].id;
                    jQuery('li#'+widgetId).find('.dashboardWidgetContent').css('padding-left','10%');
                }
	},
