...
{strip}
    {foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
        {if $MODULE_NAME eq "Quotes" and ($BLOCK_LABEL_KEY eq "LBL_QUOTES_VALUATIONDETAILS"
        or $BLOCK_LABEL_KEY eq "LBL_QUOTES_SITDETAILS"
        or $BLOCK_LABEL_KEY eq "LBL_QUOTES_ACCESSORIALDETAILS")}
            {continue}
        {/if}
        {if $BLOCK_LABEL_KEY == 'LBL_DRIVER_INFORMATION' && $HIDE_DRIVER_INFO}
            {continue}
        {/if}
        ...
        <table class="table table-bordered equalSplit detailview-table block_{$BLOCK_LABEL_KEY}">
            ...
            <tr>
                <th class="blockHeader" colspan="4">
                    ...
                    {if $BLOCK_LABEL_KEY eq 'Microsoft Exchange'}
                        &nbsp;&nbsp;<span><button type="button" class="btn" id="exchangeCredentialsButton">{vtranslate('LBL_EXCHANGE_SET_CREDENTIALS', 'Exchange')}</button></span>
                    {/if}
                </th>
            </tr>
            </thead>
            ...
            {/if}
                ...
                {/if}
                {* I start here *}
                {if  $BLOCK_LABEL_KEY eq 'LBL_EMPLOYEES_AVAILABILITY'}
                {if $COUNTER eq '4'}
                </tr>
                <tr>
                {assign var=COUNTER value=1}
            ...
            <tr>
                ...
                {* I end here *}
                <td class="fieldLabel {$WIDTHTYPE}"
                    {if $BLOCK_LABEL_KEY eq 'LBL_EMPLOYEES_AVAILABILITY'}style='width:12.5%'{/if}
                    id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}"{if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
                    <label class="muted pull-right marginRight10px">
                        {vtranslate({$FIELD_MODEL->get('label')},{$MODULE_NAME})}
                ...
                <td class="fieldValue {$WIDTHTYPE}"
                    {if $BLOCK_LABEL_KEY eq 'LBL_EMPLOYEES_AVAILABILITY'}style='width:12.5%'{/if}
                    id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					 <span class="value"
                    ...
                    {if $IS_AJAX_ENABLED && $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $FIELD_MODEL->isAjaxEditable() eq 'true'}
                        <span class="hide edit">
                            ...
                            {if $FIELD_MODEL->getFieldDataType() eq 'multipicklist'}
                                ...
                                <input type="hidden" class="fieldname" value='{$FIELD_MODEL->get('name')}'
                                       data-prev-value='{Vtiger_Util_Helper::toSafeHTML($FIELD_MODEL->getDisplayValue($FIELD_MODEL->get('fieldvalue')))}'/>
                            {/if}
						 </span>
                    {/if}
                </td>
            ...
            {if $MODULE_NAME eq "Quotes" and $BLOCK_LABEL_KEY eq "LBL_QUOTES_INTERSTATEMOVEDETAILS"}
                ...
                <tr>
                    ...
                    <td class='fieldLabel'></td>
                    <td class='fieldValue'>
                        <button type='button' id='interstateRateDetail'>Detailed Rate Estimate</button>
                    </td>
                </tr>
            {/if}
            </tbody>
