...
<input type="hidden" id="popup_type" name="popup_type" value="{$POPUPTYPE}"/>
...
<div class="popupEntriesDiv relatedContents contents-bottomscroll" style="overflow-y:scroll;">
	...
	<table class="table table-bordered listViewEntriesTable">
		<thead>
			<tr class="listViewHeaders">
				...
				<th class="{$WIDTHTYPE}" {if $LISTVIEW_HEADER@last} colspan="2" {/if}>
		...
		<tr class="listViewEntries" data-dismiss="modal" data-id="{$LISTVIEW_ENTRY->getId()}" data-name='{$LISTVIEW_ENTRY->getName()}' data-info='{ZEND_JSON::encode($LISTVIEW_ENTRY->getRawData())}'
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" {if $LISTVIEW_HEADER@last} colspan="2" {/if}>
				{if $LISTVIEW_HEADER->isNameField() eq true or $LISTVIEW_HEADER->get('uitype') eq '4'}
                                    <a {if $LISTVIEW_ENTRY->get('on_notice') eq 1 }class="redColor" title="This driver is On Notice"{/if}>{$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}</a>
                ...
                {elseif $LISTVIEW_HEADER->get('uitype') eq '1989'}
                    {assign var=FIELD_MODEL value=$RELATED_MODULE_MODEL->getField($LISTVIEW_HEADERNAME)}
                    {$FIELD_MODEL->getDisplayValue($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME))}
				{elseif $LISTVIEW_HEADER->get('uitype') eq '10'}
					{assign var=FIELD_MODEL value=$RELATED_MODULE_MODEL->getField($LISTVIEW_HEADERNAME)}
                    {$FIELD_MODEL->getDisplayValue($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME))}
				{else}
