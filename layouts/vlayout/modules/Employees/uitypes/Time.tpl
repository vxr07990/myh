...
{assign var="TIME_FORMAT" value=$USER_MODEL->get('hour_format')}
	...
	{if $TIMEZONE_FIELD_MODEL}
        ...
        <input data-wrong-range="{if $FIELD_MODEL->getDisplayValue($FIELD_VALUE, $RECORD_ID, $RECORD) neq ''}false{else}true{/if}" {if $FIELD_MODEL->isReadOnly()} readonly {/if} id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->get('name')}" type="text" data-format="{$TIME_FORMAT}" class="timepicker-default input-small" value="{$FIELD_MODEL->getDisplayValue($FIELD_VALUE, $RECORD_ID, $RECORD)}" name="{$FIELD_MODEL->getFieldName()}" style="width: 100px"
