...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			<table name='{$BLOCK_LABEL}' class="table table-bordered blockContainer showInlineTable block_{$BLOCK_LABEL} {if $BLOCK_LABEL eq 'LBL_TERMINATION_INFO'}hide{/if}">
                <thead>
			<tr>
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{/if}
					...
					{* I start here *}
			{if  $BLOCK_LABEL eq 'LBL_EMPLOYEES_AVAILABILITY'}
					 {if $COUNTER eq '4'}
					 </tr>
                            <tr>
				...
				<td class="fieldLabel {$WIDTHTYPE}" {if $BLOCK_LABEL eq 'LBL_EMPLOYEES_AVAILABILITY'}style='width:12.5%'{/if}>
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $BLOCK_LABEL eq 'LBL_EMPLOYEES_AVAILABILITY'}style='width:12.5%'{/if} {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
								{assign var=USER_FIELD value=$USER_MODULE_MODEL->getField($FIELD_MODEL->get('name'))}
								{if $USER_FIELD}
									{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),'Users') BLOCK_FIELDS=$BLOCK_FIELDS}
								{else}
		...
		{/foreach}
		{* tag cloud starts
		<table class="table table-bordered marginLeftZero" name="Tag Cloud Display">
			<tr class="listViewActionsDiv">
				<th colspan="4">{vtranslate('Tag Cloud Display', $MODULE)}</th>
			</tr>
			<tr>
