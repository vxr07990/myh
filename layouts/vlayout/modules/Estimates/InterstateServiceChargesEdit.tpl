            ...
            <tbody id="originServiceCharges">
                ...
                {foreach item=SERVICE_CHARGE key=KEY from=$INTERSTATE_SERVICECHARGES[0]}
                    {if getenv('INSTANCE_NAME') == 'graebel'}
                        {assign var=WEIGHT_INPUT_TYPE value='text'}
                        {assign var=WEIGHT_INPUT_TITLE value='Leave blank or as 0 to use full shipment weight'}
                    {else}
                        {if $SERVICE_CHARGE['minimum'] eq 0}
                            {assign var=WEIGHT_INPUT_TYPE value='hidden'}
                        {else}
                            {assign var=WEIGHT_INPUT_TYPE value='text'}
                        {/if}
                    {/if}
                    ...
                    <td style="text-align:center">
                        <input type="checkbox" name="applied" {if $SERVICE_CHARGE['applied']}checked{/if} {if $SERVICE_CHARGE['always_used']}readonly{/if} />
                    </td>
                    {* <td style="text-align:center">
                        <input type="checkbox" name="always_used" readonly {if $SERVICE_CHARGE['always_used']}checked{/if} />
                    </td> *}
                    <td style="text-align:center">
                        <input type="{$WEIGHT_INPUT_TYPE}" {if $WEIGHT_INPUT_TITLE}title="{$WEIGHT_INPUT_TITLE}"{/if} name="service_weight" style="width:85%" value="{$SERVICE_CHARGE['service_weight']}" />
                    </td>
                    {if getenv('INSTANCE_NAME') == 'graebel'}
                        <td style="text-align:center">
                    ...
                    {else}
                        <td style="text-align:center">
                            <input type="text" name="service_description" style="width:85%" readonly value="{$SERVICE_CHARGE['service_description']}" />
                        </td>
                    {/if}
