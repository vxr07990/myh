...
{if $HAS_CONTENT}
    ...
    <thead>
        <tr>
            <th class="blockHeader" colspan="35">{vtranslate('Estimate Details', $MODULE)}</th>
        </tr>
    </thead>
    <tbody>
    <tr>
            <td class="narrowWidthType" style="width: 10px">
            </td>
            ...
            <td class="{$hide_or_not}"><b>{vtranslate('Item Name', $MODULE)}</b></td>
            <td class="" ><b>{vtranslate('Item', $MODULE)}</b></td>
            ...
            <td class="" style="width: 15%"><b class="pull-right">{vtranslate('Gross Amount', $MODULE)}</b></td>
            <td class="{$hide_or_not}" ><b>{vtranslate('Invoice Discount', $MODULE)}</b></td>
            <td style="width: 15%"><b class="pull-right">{vtranslate('Net Amount', $MODULE)}</b></td>
        ...
        {foreach key=SECTION_NAME item=LINEITEM_SECTION from=$LINEITEMS}
            {assign var=SECTION_GROSS_TOTAL value=0}
            {assign var=SECTION_NET_TOTAL value=0}
            {foreach key=row_no item=LINEITEM from=$LINEITEM_SECTION}
                {assign var=SECTION_GROSS_TOTAL value=$SECTION_GROSS_TOTAL+CurrencyField::convertToDBFormat($LINEITEM.Gross)}
                {assign var=SECTION_NET_TOTAL value=$SECTION_NET_TOTAL+CurrencyField::convertToDBFormat($LINEITEM.InvoiceCostNet)}
            {/foreach}
            <tr class="sectionRollup" data-rollup="{str_replace(' ','_',$SECTION_NAME)}">
                <td class="narrowWidthType" style="width: 10px">
					<img class="alignMiddle lineItemSectionToggleShow"  src="{vimage_path('arrowRight.png')}" style="margin-right:5px;">
					<img class="alignMiddle lineItemSectionToggleHide hide"  src="{vimage_path('arrowDown.png')}" style="margin-right:5px;">
                </td>
                <td class="narrowWidthType">
                    <div class="row-fluid"><b>{$SECTION_NAME}</b></div>
                </td>
               <td class="narrowWidthType">
                    <div class="row-fluid"><span class="add-on" style="margin-left: 8px"><b>$</b></span><b class="pull-right">{number_format($SECTION_GROSS_TOTAL, 2)}</b></div>
                </td>
               <td class="narrowWidthType">
                    <div class="row-fluid"><span class="add-on" style="margin-left: 8px"><b>$</b></span><b class="pull-right">{number_format($SECTION_NET_TOTAL, 2)}</b></div>
                </td>
            </tr>
            {foreach key=row_no item=LINEITEM from=$LINEITEM_SECTION}
                {assign var=row_no value=$COUNT}
                {assign var=COUNT value=$COUNT+1}
                <tr class="section_{str_replace(' ','_',$SECTION_NAME)} hide">
                <td class="narrowWidthType" style="width: 10px">
                </td>
                ...
                <td class="narrowWidthType">
                    <div class="row-fluid" name="description{$row_no}"><span style="margin-left: 10px">{$LINEITEM.Description}</span></div>
               ...
               <td class="narrowWidthType">
                    <div class="row-fluid" name="gross{$row_no}"><span class="add-on" style="margin-left: 8px">$</span><a class="pull-right">{number_format(str_replace(',','',$LINEITEM.Gross),2)}</a></div>
               ...
               <td class="narrowWidthType">
                    <div class="row-fluid" name="invoicecostnet{$row_no}"><span class="add-on" style="margin-left: 8px">$</span><a class="pull-right">{number_format(str_replace(',','',$LINEITEM.InvoiceCostNet), 2)}</a></div>
            ...
            {/foreach}
        {/foreach}
    <tr><td colspan="30" class="item-detail-division">&nbsp;</td></tr>
    <tr>
        <td class="narrowWidthType" style="width:10px">
            &nbsp;
        </td>
        ...
        <td class="narrowWidthType">
            <div class="row-fluid">
                <span class="add-on" style="margin-left:8px">
                    <b>$</b>
                </span>
                <b class="pull-right">{number_format($GROSS_TOTALS, 2)}</b>
            </div>
        </td>
        <td class="narrowWidthType">
            <div class="row-fluid">
                ...
                <b class="pull-right">{number_format($INVOICE_AMOUNT_TOTALS, 2)}</b>
            </div>
        </td>
    </tr>
...
<input type="hidden" name="dist_net_total" value="{$DISTRIBUTABLE_AMOUNT_TOTALS}" />
    ...
    <input type="hidden" name="subtotal" id="subtotal" value="{$INVOICE_AMOUNT_TOTALS}" />
    <input type="hidden" name="total" id="total" value="{$INVOICE_AMOUNT_TOTALS}" />
        {*
    <tr>
{if $hide_or_not}
<td colspan="2" class="narrowWidthType">
