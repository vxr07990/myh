	...
	<table class="table table-bordered listViewEntriesTable">
		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
				{*OLD SECURITIES{if $EXTRA_PERMISSIONS[$INDEX][1] != 'no_rates' || $LISTVIEW_HEADER->get('column') != 'total'}*}
					...
					{else}
						...
						{elseif $LISTVIEW_HEADER->getName() eq 'effective_tariff'}
					...
					<div class="actions pull-right">
						<span class="actionImages">
							...
							{if $IS_MODULE_EDITABLE}{*OLD SECURITIES && $CURRENT_USER_MODEL->getExtraPermission($LISTVIEW_KEY)*}
								<a href='{$LISTVIEW_ENTRY->getEditViewUrl()}'><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="icon-pencil alignMiddle"></i></a>&nbsp;
							{/if}
							{if $IS_MODULE_DELETABLE && $LISTVIEW_ENTRY->get('quotestage') neq 'Accepted'}{*OLD SECURITIES && $CURRENT_USER_MODEL->getExtraPermission($LISTVIEW_KEY)*}
