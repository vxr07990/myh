...
jQuery.Class("Emails_MassEdit_Js",{},{
	...
	registerEmailFieldSelectionEvent : function(){
		...
		selectEmailForm.on('submit',function(e){
			...
			*/
			...
			var params = form.serializeFormData();
			//var timer = thisInstance.showComposeEmailForm(params,"","composeEmail");
			var timer = thisInstance.showComposeEmailForm(params);
	...
	registerAjaxEvent : function(){
		jQuery(document).ajaxStart(function(event, xhr, settings){
			//console.dir('ajaxStart!');
			if(settings){
				//console.dir(settings.url);
			}
			//location.reload();
		});
	},
	...
	*/
	registerSendEmailEvent : function(){
		var currentForm = this.getMassEmailForm();
		currentForm.on('submit',function(e){
			jQuery('#description').val(jQuery('.cke_wysiwyg_frame').contents().find("html").html());
			//console.dir(jQuery('.cke_wysiwyg_frame').contents().find("html").html());
			//e.preventDefault();
			...
			//console.dir(jQuery('.cke_wysiwyg_frame').contents().find("html").html());
			//console.dir($(currentForm).attr('action'));
		...
		jQuery('#selectEmailTemplate').on('click',function(e){
			...
			popupInstance.show(url,function(data){
				...
				if(jQuery('#isMoveHQ').val() == '1'){
					...
					versionNum = jQuery('#versionNum').val();
					supportTeam = 'moveHQ Support Team';
					supportEmail = 'messagedelivery@igcsoftware.com';
				} else{
				...
				selectedName = selectedName.replace(/\$software_name\$/g, softwareName);
				selectedTemplateBody = selectedTemplateBody.replace(/\$website\$/g, developerSite);
				selectedTemplateBody = selectedTemplateBody.replace(/\$version_num\$/g, versionNum);
				selectedTemplateBody = selectedTemplateBody.replace(/\$software_name\$/g, softwareName);
				selectedTemplateBody = selectedTemplateBody.replace(/\$year\$/g, new Date().getFullYear());
				selectedTemplateBody = selectedTemplateBody.replace(/\$logo\$/g, logo);
				selectedTemplateBody = selectedTemplateBody.replace(/\$developer_name\$/g, developerName);
				selectedTemplateBody = selectedTemplateBody.replace(/\$developer_site\$/g, developerSite);
				selectedTemplateBody = selectedTemplateBody.replace(/\$support_team\$/g, supportTeam);
				selectedTemplateBody = selectedTemplateBody.replace(/\$support_email\$/g, supportEmail);
	...
	registerEvents : function(){
		...
		if(composeEmailForm.length > 0){
            if(sourceModule == 'Documents'){
    			if (!jQuery.isFunction(jQuery.fn.MultiFile)) { //If MultiFile plugin is not loaded, load it first. This also makes sure Multifile only loads once, as loading it multiple times will break
    				jQuery.getScript("libraries/jquery/multiplefileupload/jquery_MultiFile.js?v=1.0").then(function(){
    					jQuery("#multiFile").MultiFile({
    						list: '#attachments',
    						'afterFileSelect' : function(element, value, master_element){
    							//console.dir(element);
    							//console.dir(value);
    							//console.dir(master_element);
...
/*
//On massEmailForm load
jQuery(document).ajaxComplete(function(event, xhr, settings) {
	if(xhr.responseText.indexOf('emailPageID') >= 0){
