...
CkEditorImageBrowser.initEventHandlers = function () {
	...
	var isCustomBrowser = getURLParameter('is_custom_browser');

	if (isCustomBrowser == 'true') {
		$(document).on('click', '.js-image-link', function () {
			 ...
			 */
			window.opener.CkEditorImageBrowser.callBack($(this).data('url'));
			window.close();
		});
	} else {
		$(document).on('click', '.js-image-link', function () {
...
/**
 * @link http://stackoverflow.com/questions/11582512/how-to-get-url-parameters-with-javascript/11582513#11582513
 *
 * @param name
 * @returns {string|null}
 */
