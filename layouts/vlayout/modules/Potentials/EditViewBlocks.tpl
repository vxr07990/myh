...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			<table class="table table-bordered blockContainer showInlineTable equalSplit" {if is_array($HIDDEN_BLOCKS)}{if in_array($BLOCK_LABEL, $HIDDEN_BLOCKS)} style="display:none;"{/if}{/if}>
                <thead>
			<tr>
				<th class="blockHeader" colspan="{if $BLOCK_LABEL eq 'LBL_POTENTIALS_DATES'}6{else}4{/if}">{vtranslate($BLOCK_LABEL, $MODULE)}</th>
			</tr>
                </thead>
                <tbody>
			<tr>
			...
			{/if}
				...
				<td class="fieldLabel {$WIDTHTYPE}" {if $BLOCK_LABEL eq 'LBL_POTENTIALS_DATES'}style='min-width:14%'{/if}>
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $BLOCK_LABEL eq 'LBL_POTENTIALS_DATES'}style='min-width:16.6%'{/if} {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
				...
				{/if}
			{/foreach}
		{if $BLOCK_LABEL eq 'LBL_POTENTIALS_DATES' and $COUNTER eq 1}
