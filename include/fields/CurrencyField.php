...
class CurrencyField
{
     ...
     * @param Users $user
     */
    public function initialize($user=null, $decimalPlacesOverride=false)
    {
        ...
        $this->numberOfDecimal = $decimalPlacesOverride === false ? getCurrencyDecimalPlaces() : $decimalPlacesOverride;
    }
     ...
     */
    public static function convertToUserFormat($value, $user=null, $skipConversion=false, $skipFormatting=false, $decimalPlacesOverride=false)
    {
        ...
        $self = new self($value);
        $value = $self->getDisplayValue($user, $skipConversion, $skipFormatting, $decimalPlacesOverride);
     ...
     */
    public function getDisplayValue($user=null, $skipConversion=false, $skipFormatting=false, $decimalPlacesOverride=false)
    {
        global $current_user;
        if (empty($user)) {
            $user = $current_user;
        }
        $this->initialize($user, $decimalPlacesOverride);
    ...
    private function _formatCurrencyValue($value)
    {
        ...
        // ARgh, hacking
        $diff = strlen($value) - strlen(rtrim($value,  '0'));
        if($currencyDecimalPlaces > 2 && $diff > 0)
        {
            $diff = min($diff, $this->numberOfDecimal - 2);
            $currencyDecimalPlaces = $this->numberOfDecimal - $diff;
            $value = number_format($value, $currencyDecimalPlaces, '.', '');
        }
     ...
     * @return Number
     */
    public function getDBInsertedValue($user=null, $skipConversion=false, $decimalPlacesOverride=false)
    {
        global $current_user;
        if (empty($user)) {
            $user = $current_user;
     ...
     * @return Number
     */
    public static function convertToDBFormat($value, $user=null, $skipConversion=false, $decimalPlacesOverride=false)
    {
        $self = new self($value);
        return $self->getDBInsertedValue($user, $skipConversion, $decimalPlacesOverride);
    }
