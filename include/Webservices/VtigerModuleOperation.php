...
class VtigerModuleOperation extends WebserviceEntityOperation
{
    ...
    public function query($q)
    {
        ...
        // Add Owner Condition to query
        if ($this->user->is_admin == 'off' && $this->webserviceObject->getEntityName() != 'Users' && $this->webserviceObject->getEntityName() != 'ModComments') {
            $userModel=Users_Record_Model::getInstanceById($this->user->id, 'Users');
            $position = stripos($mysql_query, ' LIMIT ');
            if ($position) {
                $split = spliti(' LIMIT ', $mysql_query);
            }

            if ($this->user->agent_ids != '' && !$userModel->isVanLineUser()) {
                $accessible = explode(' |##| ', $this->user->agent_ids);
            } elseif ($userModel->isVanLineUser()) {
                $accessibleAgents = $userModel->getVanlineUserAccessibleAgents();
                $accessibleVanlines = $userModel->getAccessibleVanlinesForUser();
                $accessible = $userModel->AssocArrayMerge($accessibleAgents, $accessibleVanlines);
                $accessible         = array_keys($accessible);
            }

            if ($this->webserviceObject->getEntityName() == 'Accounts' || $this->webserviceObject->getEntityName() == 'Contracts') {
                $accessible = array_merge($accessible, array_keys($userModel->getAccessibleVanlinesForUser()));
            }

            //agentid and smownerid conditions
            $accessibleGroups = $userModel->getUserGroups($this->user->id);
            //Adding crmentity ownerid allows users to see records in list view that they don't have access to edit.
            $ownerCondition   = '(vtiger_crmentity.agentid IN ('.implode(',', $accessible).') OR vtiger_crmentity.smownerid IN ('.$this->user->id;
            if ($accessibleGroups) {
                $ownerCondition .= ','.implode(',', $accessibleGroups);
            }
            //close the IN for smownerid conditions
            $ownerCondition .= '))';
            $mysql_query = $split[0]. " AND ".$ownerCondition." LIMIT ".$split[1];
        }
