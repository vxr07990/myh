 ...
 *************************************************************************************/
    ...
    class DataTransform
    {
        ...
        {
            $newRow = DataTransform::filterAllColumns($row, $meta);
            if($meta->getEntityName() == 'Surveys' && $row['survey_type'] == 'LiveSurvey') {
                $newRow['cubesheetid'] = vtws_getWebserviceEntityId('Cubesheets', $row['cubesheetid']);
            }
            $row = DataTransform::sanitizeData($newRow, $meta);
            return $row;
        }
        ...
        public function sanitizeFields($row, $meta)
        {
            ...
            /*foreach($moduleFields as $fieldName=>$fieldModel) {
                $uitype = $fieldModel->getUIType();
                if($uitype == 15 || $uitype == 16) {
                    $row[$fieldName] = vtranslate($row[$fieldName], $meta->getTabName());
                }
            }*/
        ...
        public function sanitizeDateFieldsForInsert($row, $meta)
        {
            ...
            foreach ($moduleFields as $fieldName=>$fieldObj) {
                if ($fieldObj->getFieldDataType()=="date") {
                    if (!empty($row[$fieldName]) && !preg_match('/^\d\d\d\d-/', $row[$fieldName])) {
