...
function vtws_addWebserviceOperation($name, $handlerFilePath, $handlerMethodName, $requestType, $preLogin = 0)
{
    ...
    if ($operationId = vtws_checkWebserviceOperation($name)) {
        //@NOTE: \OperationManager::fillOperationDetails takes just the first result by name.
        //@TODO: so consider instead throwing an error that a webservice operation with this name exists.
        $createOperationQuery = "UPDATE vtiger_ws_operation SET 
                                    `name` = ?,
                                    `handler_path` = ?,
                                    `handler_method` = ?,
                                    `type` = ?,
...
/**
 * @param $name name of the webservice to find
 *
 * @return null|int the operationid of the existing webservice operation
 */
function vtws_checkWebserviceOperation($name)
{
    global $adb;
...
function vtws_addWebserviceOperationParam($operationId, $paramName, $paramType, $sequence)
{
    ...
    //@NOTE: Is this really required, what if they want it to be really first?
    if ($sequence <=1) {
        $sequence = 1;
    }

    if (!vtws_checkWebserviceOperationParam($operationId, $paramName, $paramType, $sequence)) {
 ...
 * @param $sequence sequence of the parameter
 *
 * @return Boolean true if this data already exists in the database.
 */
function vtws_checkWebserviceOperationParam($operationId, $paramName, $paramType, $sequence)
{
    global $adb;
    ...
    $sql = "SELECT * FROM `vtiger_ws_operation_parameters` WHERE 
                `operationid` = ?
                AND `name` = ?
                AND `type` = ?
                AND `sequence` = ?
                LIMIT 1";
    $result = $adb->pquery($sql, [$operationId, $paramName, $paramType, $sequence]);
    if ($result) {
        $rowCount = $adb->num_rows($result);
        if ($rowCount > 0) {
            return true;
