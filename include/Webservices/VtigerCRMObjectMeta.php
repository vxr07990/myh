...
class VtigerCRMObjectMeta extends EntityMeta
{
    ...
    private function computeAccess()
    {
        ...
        // @Todo: Fix this
        if (!file_exists('user_privileges/user_privileges_'.$this->user->id.'.php')) {
            return;
        }
    ...
    public function hasPermission($operation, $webserviceId)
    {
        ...
        // @Todo: maybe add a user override to this isPermitted function so we can pass a user?
        // That way we wouldn't need to set the global $current_user
    ...
    private function retrieveMetaForBlock($block)
    {
        ...
        require('user_privileges/user_privileges_'.$this->user->id.'.php');
        //if it's vtiger_users get all the fields, per RP.
        if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0 || $tabid==29) {
            $sql = "select *, '0' as readonly
                    from vtiger_field
                    where tabid =?
                    and block in (".generateQuestionMarks($block).")
                    and displaytype in (1,2,3,4,5)
                    and vtiger_field.presence in (0,2)";
            //@NOTE: added the presence check here, because you can't save a field with presence 1
            $params = array($tabid, $block);
        } else {
            $profileList = getCurrentUserProfileList();
            
            if (count($profileList) > 0) {
                //@NOTE: If the field you want isn't being returned from:
                //jesus I don't know where I came from to get here ... getModuleFields maybe or
                //was it $moduleFields = $this->meta->getModuleFields(); in QueryGenerator.php?
                //Whatever if the field isn't being returned somewhere somehow for a non-admin listview then chances
                //are the field doesn't exist in vtiger_def_org_field.  unsure on what that means though.
                // select * from vtiger_def_org_field where fieldid=482;
                // select *From vtiger_field where fieldid=482;
                $sql = "SELECT vtiger_field.*, vtiger_profile2field.readonly
						...
						and vtiger_field.block in (".generateQuestionMarks($block).")
						and vtiger_field.displaytype in (1,2,3,4,5)
						and vtiger_field.presence in (0,2)
						group by columnname";
        ...
        for ($i=0; $i<$noofrows; $i++) {
            ...
            //See NOTE above about missing moduleFields.
