...
class EmailTemplate
{
    ...
    public function process($params)
    {
        ...
        $resultsImg = preg_match_all('/src="(?:[^"]*)"/', $this->processedDescription, $matches);
        if ($resultsImg != 0) {
            $toReplace = [];
            $updatedURL = [];
            for ($i = 0; $i < count($matches); $i++) {
                $pieces = explode('"', $matches[$i][0]);
                if(strpos($pieces[1], 'test/upload') !== FALSE){  //Conver images to base64. Other option would be use the full URL but that will disclosure hq url in the emails
                    $image = $pieces[1];
            ...
            $this->processedDescription = str_replace($toReplace, $updatedURL, $this->processedDescription);
        }
