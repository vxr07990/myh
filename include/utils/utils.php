...
require_once('libraries/MoveCrm/InputUtils.php');
require_once('libraries/MoveCrm/LogUtils.php');
// this might need to be higher up?
require_once('libraries/MoveCrm/AccountingIntegration/IAccountingIntegrationBridge.php');
require_once('libraries/MoveCrm/AccountingIntegration/QBOBridge.php');
require_once('libraries/MoveCrm/AccountingIntegration/SelfBridge.php');
require_once('libraries/MoveCrm/AccountingIntegration.php');
require_once ('libraries/MoveCrm/WorkflowHandler/IWorkflowAPIBridge.php');
require_once ('libraries/MoveCrm/WorkflowHandler/WorkflowFTPBridge.php');
require_once ('libraries/MoveCrm/WorkflowHandler/RepublicWorkflowFTP.php');
...
function get_group_array($add_blank = true, $status = "Active", $assigned_user = "", $private = "", $module = false) {
    ...
    if ($group_array == NULL) {
        ...
        if ($private == 'private') {
            ...
            if (count($current_user_groups) != 0) {
                $query .= " union select vtiger_groups.groupid as groupid,vtiger_groups.groupname as groupname from vtiger_groups inner join vtiger_group2rs on vtiger_groups.groupid=vtiger_group2rs.groupid where vtiger_group2rs.roleandsubid in (".
                          generateQuestionMarks($parent_roles).
                          ")";
...
function to_html($string, $encode = true) {
    ...
    $ajax_action = NULL;
    $doconvert   = false;
    $parameters  = ['action', 'file', 'module', 'search'];
    ...
    if (is_string($string)) {
        if ($request->action != 'CustomView'
            && $request->action != 'Export'
            && $request->action != $ajax_action
            && $request->action != 'LeadConvertToEntities'
            && $request->action != 'CreatePDF'
            && $request->action != 'ConvertAsFAQ'
            && $request->module != 'Dashboard'
            && $request->action != 'CreateSOPDF'
            && $request->action != 'SendPDFMail'
...
function getRecordOwnerId($record) {
    ...
    if ($ownerId) {
        ...
        if ($count === false) {
            $sql_result = $adb->pquery("select count(*) as count from vtiger_users where id = ?", [$ownerId]);
...
function getEmailsRelatedAccounts($record_id) {
	...
	global $adb;
    $query     =
        "select vtiger_seactivityrel.crmid from vtiger_seactivityrel inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_seactivityrel.crmid where vtiger_crmentity.setype='Accounts' and activityid=?";
    $result    = $adb->pquery($query, [$record_id]);
...
function getEmailsRelatedLeads($record_id) {
	...
	global $adb;
    $query  =
        "select vtiger_seactivityrel.crmid from vtiger_seactivityrel inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_seactivityrel.crmid where vtiger_crmentity.setype='Leads' and activityid=?";
    $result = $adb->pquery($query, [$record_id]);
...
function getHelpDeskRelatedAccounts($record_id) {
	...
	global $adb;
    $query     =
        "select parent_id from vtiger_troubletickets inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_troubletickets.parent_id where ticketid=? and vtiger_crmentity.setype='Accounts'";
    $result    = $adb->pquery($query, [$record_id]);
...
function getAccessPickListValues($module) {
    ...
    for ($i = 0; $i < $adb->num_rows($result); $i++) {
        ...
        if (count($roleids) > 1) {
            $mulsel =
                "select distinct $fieldname from vtiger_$fieldname inner join vtiger_role2picklist on vtiger_role2picklist.picklistvalueid = vtiger_$fieldname.picklist_valueid where roleid in (\"".
                implode($roleids, "\",\"").
                "\") and picklistid in (select picklistid from vtiger_$fieldname) order by sortid asc";
        } else {
            $mulsel =
                "select distinct $fieldname from vtiger_$fieldname inner join vtiger_role2picklist on vtiger_role2picklist.picklistvalueid = vtiger_$fieldname.picklist_valueid where roleid ='".
                $roleid.
                "' and picklistid in (select picklistid from vtiger_$fieldname) order by sortid asc";
	}
...
function getReferenceCombinations($array, $tempString = '') {
    for ($i = 0; $i < count($array); $i++) {
        $splicedArray = $array;
        $element      = array_splice($splicedArray, $i, 1);// removes and returns the i'th element
        if (count($splicedArray) > 0) {
            ...
            $result = array_merge($result, getReferenceCombinations($splicedArray, $tempString.','.$element[0]));
        } else {
            return [$tempString.','.$element[0]];
        }
    }
...
function getFieldTimeZoneValue($fieldid, $crmid) {
	global $adb;
	if (!$crmid) {
	    return '';
    }
    $query = "SELECT * FROM vtiger_fieldtimezonerel WHERE crmid=? AND fieldid=?";
    $rs    = $adb->pquery($query, [$crmid, $fieldid]);
    if ($adb->num_rows($rs) > 0) {
        return $adb->query_result($rs, 0, 'timezone');
	}
	global $current_user;
...
/*The function to check if Windfall is Activated
 * Need to add : WINDFALL_ACTIVE = 1
 * To .env file
 * */
function checkIsWindfallActive() {
    if(getenv('WINDFALL_ACTIVE') == 1) {
        return true;
    } else {
        return false;
    }
...
function multicreate($createArray) {
    $isNew = false;
    $newFilter = false;
foreach($createArray as $module=>$data) {
  $moduleInstance = Vtiger_Module::getInstance($module);
  if ($moduleInstance) {
    echo "<h2>$module already exists </h2><br>".PHP_EOL;
  } else {
    ...
    if ($blockInstance) {
      echo "<h3>The $blockLabel block already exists</h3><br>".PHP_EOL;
    } else {
    ...
    if(empty($fields)) {
      ...
      if (!checkInvalidFieldNames($fieldAttributes['name'])) {
          //@NOTE: I am unbelievably frustrated that I was not warned a fieldname was invalid.
          //@NOTE: So now everybody will be prewarned.
          throw new Exception('INVALID FIELD NAME: '.$fieldAttributes['name']);
      }

      $field = Vtiger_Field::getInstance($fieldAttributes['name'], $moduleInstance);
      if ($field) {
        echo "<br> $fieldLabel already exists <br>".PHP_EOL;
      } else {
        ...
        $field->readonly = isset($fieldAttributes['readonly']) ? $fieldAttributes['readonly'] : 1;
        $field->presence = isset($fieldAttributes['presence']) ? $fieldAttributes['presence'] : 0;
        $field->sequence = $fieldAttributes['sequence'];
        if (strtolower($fieldAttributes['sequence']) == 'auto') {
            $field->sequence = $autoSequence++;
        }
        $field->defaultvalue = $fieldAttributes['defaultvalue'];
        $field->summaryfield = isset($fieldAttributes['summaryfield']) ? 1 : 0;
        $blockInstance->addField($field);
        setArrayFieldAttributes($field, $fieldAttributes, 'setRelatedModules');
        setArrayFieldAttributes($field, $fieldAttributes, 'setPicklistValues');
        if(isset($fieldAttributes['entityIdentifier']) && $fieldAttributes['entityIdentifier']) {
          $moduleInstance->setEntityIdentifier($field);
        }
      }
      if($field && isset($fieldAttributes['resetFilter'])){
          $newFilter = true;
      ...
      if (($isNew || $newFilter) && $field && isset($fieldAttributes['filterSequence'])) {
        $filter = Vtiger_Filter::getInstance('All', $moduleInstance);
        if(!$filter) {
    ...
    //@TODO - Figure out a good way to get this in the menu
    //Menu
 ...
 * @param              $attributeName
 */
function setArrayFieldAttributes(Vtiger_Field $field, array $fieldAttributes, $attributeName) {
    $fieldValue = $fieldAttributes[$attributeName];
    if (isset($fieldValue)) {
        if (!is_array($fieldValue)) {
            $fieldValue = [$fieldValue];
        }
        $field->$attributeName($fieldValue);
    }
}

/**
 * compares an input string against a set list...
 ...
 * @return bool
 */
function checkInvalidFieldNames($fieldName) {
    if (!$fieldName) {
        return false;
    }
    ...
    $compareValue = strtolower($fieldName);

    if (!in_array($compareValue, $invalidFieldNames)) {
        return true;
    }
...
function generateDefaultRecords($moduleName, $agentid, $current_user){
    ...
    while($row = $result->fetchrow()){
        ...
        if($moduleName == 'WFConditions'){
            $params['wfconditions_account'] = vtws_getWebserviceEntityId('WFAccounts', 0);
        }
        vtws_create($moduleName, $params, $current_user, true);

    }
}

/// Updates the Type Of Data variable sent in, and updates it accordingly.
/// Params:
///     string &$tod: The Type Of Data string. (mutable)
///     string $search: The TOD item to be replaced.
///     string $new: The new item value.
/// Returns: true or false as to whether or not the search value was found/replaced.
function updateTypeOfData(&$tod, $search, $new) {
    $tod      = explode('~', $tod);
    $optional = array_search($search, $tod);
...
function reorderFields($moduleName, $fieldArray, $includesBlocks = false){
    ...
    $seq = 0;
    if($includesBlocks){
        foreach($fieldArray as $blockLabel=>$fields) {
            $blockInstance = Vtiger_Block::getInstance($blockLabel, $moduleInstance);
            if ($blockInstance) {
 ...
 * @param string|Vtiger_Module_Model $module
 *
 * @return bool|Vtiger_Field
 * @throws Exception
 */
function getFieldModel($fieldName, $module) {
    if (!$fieldName) {
        return false;
        throw new Exception('Field name not passed.');
    }
    if (!$module) {
        return false;
        throw new Exception('Module not passed.');
    }
    if (!is_object($module)) {
        $moduleInstance = Vtiger_Module::getInstance($module);
    } else {
    ...
    if (!is_object($moduleInstance)) {
        return false;
        throw new Exception('Module instance must be an object.');
    }
...
/**
 * function that updates the $dbFieldName field in the vtiger_field table with the $newValue
 *
 ...
 * @throws Exception
 */
function updateFieldValue($fieldModel, $dbFieldName, $newValue) {
    if (!is_object($fieldModel)) {
        return false;
        throw new Exception('Field model must be an object.');
    }

    if (!$dbFieldName) {
        return false;
        throw new Exception('Database field name not passed.');
    }

    if (!$newValue) {
        return false;
        throw new Exception('New Value not passed.');
    }

    $db = &PearDatabase::getInstance();
    if ($newValue && $fieldModel->$dbFieldName != $newValue) {
        print 'Updating '.$fieldModel->name.' to '.$dbFieldName.'='.$newValue.' for '.$fieldModel->getModuleName().'<br />'.PHP_EOL;
        $stmt = 'UPDATE `vtiger_field` SET `'.$dbFieldName.'` = ? WHERE `fieldid` = ?';
        $db->pquery($stmt, [$newValue, $fieldModel->id]);
    }

    return true;
}

function updateFieldUIType($fieldName, $module, $newValue) {
    $field = getFieldModel($fieldName, $module);
    return updateFieldValue($field, 'uitype', $newValue);
}

function updateFieldTypeOfData($fieldName, $module, $newValue) {
    $field = getFieldModel($fieldName, $module);
    return updateFieldValue($field, 'typeofdata', $newValue);
}

function updateFieldPresence($fieldName, $module, $newValue) {
    $field = getFieldModel($fieldName, $module);
    return updateFieldValue($field, 'presence', $newValue);
}

/**
 * Function to update the column type of a field in it's database table.
 *
 * @param string $fieldName
 * @param string|Vtiger_Module_Model $module
 * @param string $newValue
 ...
 * @throws Exception
 */
function updateFieldColumnType($fieldName, $module, $newValue) {
    if (!$newValue) {
        return false;
        throw new Exception('New Value not passed.');
    }
    ...
    if ($res = $db->pquery($stmt)) {
        while ($value = $res->fetchRow()) {
            if ($value['Field'] == $field->name) {
                if (strtolower($value['Type']) != strtolower($newValue)) {
                    echo 'Updating '.$field->name.' to be a '.$newValue.' type.<br />'.PHP_EOL;
                    $stmt = 'ALTER TABLE `'.$field->table.'` MODIFY COLUMN `'.$field->column.'` '.$newValue;
                    $returnObject = $db->pquery($stmt);
                    if (!$returnObject) {
                        //@TODO: This may not always be correct.
                        throw new Exception('Failed to alter table, check mysql error log. ('.$stmt.')');
                    }
                }
                //we're only affecting the one $field->column so if we find it we are done with that
                //update the vtiger_fields table now.
                //@NOTE: This is not stored in the table. sort of amazing really.
                //return updateFieldValue($field, 'columntype', $newValue);
                return true;
            }
        }
    }
    echo 'NO '.$field->column.' column in the actual table ('.$field->table.')?<br />'.PHP_EOL;
    return false;
}

function setDefaultCoordinator($recordModel, &$viewer) {
    $currentUserModel = Users_Record_Model::getCurrentUserModel();
    $agentManagerId = $recordModel->get('agentid') ?: $currentUserModel->getPrimaryOwnerForUser();
    ...
    $default = $currentUserModel->getDefaultCoordinator($isAVL);

    $viewer->assign('COORDINATORS_LIST', $coordinators);
    if($record) {
        $viewer->assign('DEFAULT_COORDINATOR', $recordModel->get('assigned_user_id'));

        $agents = ParticipatingAgents_Module_Model::getParticipants($record);
        $agent = array_filter($agents, function($item) {
            return $item['agent_type'] == 'Origin Agent';
        });
        $viewer->assign('SALESPERSON_OWNER', array_values($agent)[0]['agentmanager_id']);
    }else{
        $viewer->assign('DEFAULT_COORDINATOR', $default);
        $viewer->assign('SALESPERSON_OWNER', false);
    }
}
