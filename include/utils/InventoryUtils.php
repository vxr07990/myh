 ...
 *	@return void
 */
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='', $tempTables=false)
{
    global $log, $adb;
    $id=$focus->id;
    ...
    for ($i=1; $i<=$tot_no_prod; $i++) {
        ...
        $query ="insert into `".$tablePrefix."vtiger_inventoryproductrel`(id, productid, sequence_no, quantity, listprice, comment, description) values(?,?,?,?,?,?,?)";
        ...
        if (!empty($sub_prod_str)) {
            ...
            for ($j=0;$j<count($sub_prod);$j++) {
                $query ="insert into `".$tablePrefix."vtiger_inventorysubproductrel`(id, sequence_no, productid) values(?,?,?)";
        ...
        $updatequery = "update `".$tablePrefix."vtiger_inventoryproductrel` set ";
        $updateparams = array();
    ...
    $updatequery  = " update `".$tablePrefix.$focus->table_name."` set ";
    $updateparams = array();
    ...
    $sh_query = "insert into `".$tablePrefix."vtiger_inventoryshippingrel`($sh_query_fields) values($sh_query_values)";
...
function getInventoryCurrencyInfo($module, $id)
{
    ...
    $inv_table_array = array('PurchaseOrder'=>'vtiger_purchaseorder','SalesOrder'=>'vtiger_salesorder','Quotes'=>'vtiger_quotes','Invoice'=>'vtiger_invoice', 'Estimates'=>'vtiger_quotes', 'Actuals'=>'vtiger_quotes');
    $inv_id_array = array('PurchaseOrder'=>'purchaseorderid','SalesOrder'=>'salesorderid','Quotes'=>'quoteid','Invoice'=>'invoiceid', 'Estimates'=>'quoteid', 'Actuals'=>'quoteid');
