...
function getAssociatedProducts($module, $focus, $seid='')
{
	...
	//DP: Nope, we're ordering by sequence now. not sequence_no because that didn't do the right things

    if ($module == 'Quotes' || $module == 'PurchaseOrder' || $module == 'SalesOrder' || $module == 'Invoice' || $module == 'Estimates' || $module == 'Actuals') {
		if ($focus->id) {
            //I made id table specific since it's just confusing to see 'WHERE id = ?'....
            $params = [$focus->id];
            if(getenv('INSTANCE_NAME') == 'sirva') {
                $query ="SELECT 'NA' AS qtyinstock, 'Services' AS entitytype,
                               vtiger_crmentity.deleted,
                               vtiger_rating_line_items.estimate_id,
                               vtiger_rating_line_items.billing_item,
                               vtiger_rating_line_items.amount,
                               vtiger_inventoryproductrel.quantity AS quantity,
                               vtiger_inventoryproductrel.productid,
                               vtiger_inventoryproductrel.listprice
                        FROM vtiger_rating_line_items
                            LEFT JOIN vtiger_inventoryproductrel ON vtiger_inventoryproductrel.id=vtiger_rating_line_items.estimate_id
                            LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_inventoryproductrel.productid
                        WHERE vtiger_rating_line_items.estimate_id=?
                        GROUP BY vtiger_rating_line_items.billing_item
                        ORDER BY vtiger_rating_line_items.line_item_id";

                $services_query = "SELECT vtiger_service.serviceid as productid,
                                          vtiger_service.service_no as service_no,
                                          vtiger_crmentity.deleted as deleted,
                                          vtiger_service.service_no as productcode,
                                          vtiger_service.servicename as productname,
                                          vtiger_rating_line_items.line_item_id
                                    FROM vtiger_service
                                        JOIN vtiger_crmentity ON vtiger_service.serviceid = vtiger_crmentity.crmid
	                                    LEFT JOIN vtiger_rating_line_items ON vtiger_service.servicename = vtiger_rating_line_items.billing_item
                                    WHERE vtiger_crmentity.deleted = 0
                                    GROUP BY vtiger_service.servicename
                                    ORDER BY -vtiger_rating_line_items.line_item_id DESC";
                $ServResult=$adb->query($services_query);
           }else {
			$query = "SELECT
    ...
    if(getenv('INSTANCE_NAME') == 'sirva') {
        ...
        for($j = 0; $j < $adb->num_rows($ServResult); $j++) {
            $detailsInfo['productname'] = $adb->query_result($ServResult,$j,'productname');
            $detailsInfo['hdnProductId'] = $adb->query_result($ServResult,$j,'productid');
            $detailsInfo['hdnService_no'] = $adb->query_result($ServResult,$j,'service_no');
            $detailsInfo['hdnProductcode'] = $adb->query_result($ServResult,$j,'productcode');
            $detailsInfo['deleted'] = $adb->query_result($ServResult,$j,'deleted');
            for($i = 1; $i <= $adb->num_rows($result); $i++) {

                $detailsInfo['qtyinstock']=$adb->query_result($result,$i-1,'qtyinstock');
                $detailsInfo['qty']=$adb->query_result($result,$i-1,'quantity');
                $detailsInfo['unitprice']=$adb->query_result($result,$i-1,'unit_price');
                $detailsInfo['entitytype']=$adb->query_result($result,$i-1,'entitytype');
                $detailsInfo['estimateId'] = $adb->query_result($result,$i-1,'estimate_id');
                //@NOTE: _ is an any character.
                $detailsInfo['items'] = $adb->pquery('SELECT * FROM vtiger_rating_line_item_details WHERE estimate_id = ? AND ratingitem LIKE ?', array($detailsInfo['estimateId'], preg_replace('/ /','', $detailsInfo['productname']) . '\\_%'));

                $name=$adb->query_result($result,$i-1,'billing_item');
                if($name == $detailsInfo['productname']) {
                    $detailsInfo['listprice'] =$adb->query_result($result,$i-1,'amount');
                    break;
                }elseif($detailsInfo['hdnProductId'] == $adb->query_result($result, $i-1, 'productid')){
                    $detailsInfo['listprice'] = $adb->query_result($result, $i-1, 'listprice');
                }else {
                    $detailsInfo['listprice'] = 0;
		}
		}
            $product_Detail[$j] = generateProductDetailItem($adb, empty($result) ? $ServResult : $result, $j, $detailsInfo);
        }
        if(!$focus->id) {
            $result = $ServResult;
		}
        ...
        for($i = 1; $i < $adb->num_rows($result); $i++) {
            $detailsInfo['hdnService_no'] = $adb->query_result($result,$i-1,'service_no');
            $detailsInfo['listprice']=$adb->query_result($result,$i-1,'listprice');
            $detailsInfo['hdnProductId'] = $adb->query_result($result,$i-1,'productid');
            $detailsInfo['hdnProductcode'] = $adb->query_result($result,$i-1,'productcode');
            $detailsInfo['productname']=$adb->query_result($result,$i-1,'productname');
            $detailsInfo['estimateId'] = $adb->query_result($result,$i-1,'id');
            $detailsInfo['productdescription']=$adb->query_result($result,$i-1,'product_description');
            $detailsInfo['comment']=$adb->query_result($result,$i-1,'comment');
            $detailsInfo['deleted'] = $adb->query_result($result,$i-1,'deleted');
            $detailsInfo['qtyinstock']=$adb->query_result($result,$i-1,'qtyinstock');
            $detailsInfo['qty']=$adb->query_result($result,$i-1,'quantity');
            $detailsInfo['unitprice']=$adb->query_result($result,$i-1,'unit_price');
            $detailsInfo['entitytype']=$adb->query_result($result,$i-1,'entitytype');
            $detailsInfo['estimateId'] = $adb->query_result($result,$i-1,'id');
            //$detailsInfo['items'] = $adb->pquery('SELECT * FROM vtiger_rating_line_item_details WHERE estimate_id = ? AND ratingitem LIKE ?', array($detailsInfo['estimateId'], preg_replace('/ /',

            $product_Detail[$i] = generateProductDetailItem($adb, $result, $i, $detailsInfo);
				}
	}
...
function generateProductDetailItem($adb, $result, $i, $data) {
    extract($data);

    $no_of_decimal_places = getCurrencyDecimalPlaces();
    $product_Detail['detailed_items'] = array();

    //@TODO: This may need verified.
    if(getenv('INSTANCE_NAME') == 'sirva') {
        if ($data['items']) {
            if ($adb->num_rows($data['items']) > 0) {
                for ($k = 0; $k < $adb->num_rows($data['items']); $k++) {
                    $subProduct = [];
                    foreach (['amount', 'quantity', 'description', 'rate', 'weight'] as $detailKey) {
                        $subProduct[$detailKey] = $adb->query_result($data['items'], $k, $detailKey);
                    }
                    $product_Detail['detailed_items'][] = $subProduct;
                }
            }
        }
    }
    ...
    if(($deleted) || (!isset($deleted))){
        $product_Detail['productDeleted'.$i] = true;
    }elseif(!$deleted){
        $product_Detail['productDeleted'.$i] = false;
    }

    if (!empty($entitytype)) {
        $product_Detail['entityType'.$i]=$entitytype;
    }
    ...
    $product_Detail['subProductArray'.$i] = $subProductArray;
    $product_Detail['hdnProductId'.$i] = $hdnProductId;
    $product_Detail['productName'.$i]= from_html($productname);
    ...
    if($_REQUEST['action'] == 'CreateSOPDF' || $_REQUEST['action'] == 'CreatePDF' || $_REQUEST['action'] == 'SendPDFMail')
        $product_Detail['productName'.$i]= htmlspecialchars($product_Detail['productName'.$i]);
    $product_Detail['hdnProductcode'.$i] = $hdnProductcode;
    $product_Detail['hdnService_no'.$i] = $hdnService_no;
    $product_Detail['productDescription'.$i]= from_html($productdescription);
    if($module == 'Potentials' || $module == 'Products' || $module == 'Services') {
        $product_Detail['comment'.$i]= $productdescription;
    }else {
        $product_Detail['comment'.$i]= $comment;
    }

    if($module != 'PurchaseOrder' && $focus->object_name != 'Order') {
        $product_Detail['qtyInStock'.$i]=decimalFormat($qtyinstock);
    }
    $listprice = number_format($listprice, $no_of_decimal_places,'.','');
    $product_Detail['qty'.$i]=decimalFormat($qty);
    $product_Detail['listPrice'.$i]=$listprice;
    $product_Detail['unitPrice'.$i]=number_format($unitprice, $no_of_decimal_places,'.','');
    $product_Detail['productTotal'.$i]=$productTotal;
    $product_Detail['subproduct_ids'.$i]=$subprodid_str;
    $product_Detail['subprod_names'.$i]=$subprodname_str;
    ...
    $product_Detail['discount_percent'.$i] = 0;
    $product_Detail['discount_amount'.$i] = 0;

    if(!empty($discount_percent)) {
        $product_Detail['discount_type'.$i] = "percentage";
        $product_Detail['discount_percent'.$i] = $discount_percent;
        $product_Detail['checked_discount_percent'.$i] = ' checked';
        $product_Detail['style_discount_percent'.$i] = ' style="visibility:visible"';
        $product_Detail['style_discount_amount'.$i] = ' style="visibility:hidden"';
        $discountTotal = $productTotal*$discount_percent/100;
    } elseif(!empty($discount_amount)) {
        $product_Detail['discount_type'.$i] = "amount";
        $product_Detail['discount_amount'.$i] = $discount_amount;
        $product_Detail['checked_discount_amount'.$i] = ' checked';
        $product_Detail['style_discount_amount'.$i] = ' style="visibility:visible"';
        $product_Detail['style_discount_percent'.$i] = ' style="visibility:hidden"';
        $discountTotal = $discount_amount;
    }
    else
    {
        $product_Detail['checked_discount_zero'.$i] = ' checked';
    }
    ...
    $product_Detail['discountTotal'.$i] = $discountTotal;
    $product_Detail['totalAfterDiscount'.$i] = $totalAfterDiscount;

    $taxTotal = '0.00';
    $taxTotal = number_format($taxTotal, $no_of_decimal_places,'.','');
    $product_Detail['taxTotal'.$i] = $taxTotal;
    ...
    $product_Detail['netPrice'.$i] = $netPrice;
