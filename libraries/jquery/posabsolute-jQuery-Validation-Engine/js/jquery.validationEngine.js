		...
		*/
		_validateField: function(field, options, skipAjaxValidation) {
           ...
           if (!options.validateNonVisibleFields && (field.is(":hidden") && !options.prettySelect || field.parent().is(":hidden")))
			...
			/* removed because it breaks things and seems shitty and pointless && field.val().length < 1*/
			var fieldValueCheck = field.val();
			fieldValueCheck = $.trim(fieldValueCheck);
			try{
				if(!required && !(fieldValueCheck || fieldValueCheck != '')){
					options.isError = false;
				}
			}
			catch(err){
