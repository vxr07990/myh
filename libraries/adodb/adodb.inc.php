...
*/
    ...
    */
    ...
    {
        if(getenv('MONITOR_BUSINESS_LINE_PICKLIST')) {
            if (preg_match('/(update|insert|delete).*\s+[`]*vtiger_business_line/is', $sql)) {
                file_put_contents('logs/picklistChange.log', "\n(adodb.inc.php:".__LINE__.") sql : ".print_r($sql, true), FILE_APPEND);
                file_put_contents('logs/picklistChange.log', "\n(adodb.inc.php:".__LINE__."): DEBUG TRACE\n".print_r(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS), true), FILE_APPEND);
                file_put_contents('logs/picklistChange.log', date('Y-m-d H:i:s')." - ".print_r($_SESSION, true)."\n", FILE_APPEND);
            }
        }
        if ($this->fnExecute) {
            $fn = $this->fnExecute;
        ...
        if ($this->_queryID === false) { // error handling if query fails
            ...
            $false = false;
            //@TODO: consider using ADODB_ERROR_HANDLER ex: libraries/adodb/adodb-errorhandler.inc.php
            //include libraries/adodb/adodb-errorhandler.inc.php prior to including database stuffs.
            //ryan says not to env flag it let because if it's erroring we want to know even when it's not "turned on"
            file_put_contents('logs/mysqlFail.log', "\n ".date(DATE_RFC2822).": MySQL FAIL (adodb.inc.php:".__LINE__."): DEBUG TRACE\n".print_r(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS), true), FILE_APPEND);
            file_put_contents('logs/mysqlFail.log', "\n ".date(DATE_RFC2822).": MySQL FAIL (adodb.inc.php:".__LINE__."):  Sql for ^^^ (".$sql.")", FILE_APPEND);
            file_put_contents('logs/mysqlFail.log', "\n ".date(DATE_RFC2822).": MySQL FAIL (adodb.inc.php:".__LINE__."):  ( ERROR (errno=".$this->ErrorNo()." m=".$this->ErrorMsg().") ", FILE_APPEND);
            return $false;
        }
