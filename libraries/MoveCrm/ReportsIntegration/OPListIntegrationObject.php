...
/**
 * Created by PhpStorm.
 * OPlist Reports integration object.
 */
...
class OPListIntegrationObject implements IReportsIntegrationObject {
    ...
    private function setRecordModel() {
        ...
        try {
            $recordModel = \Opportunities_Record_Model::getInstanceById($this->recordId);
            if (!$recordModel) {
                return false;
            }
    ...
    protected function getReportArray($recordId = 0) {
        ...
        $this->reportBuiltArray['survey_upload']['operational_list'] = MoveCrm\arrayBuilder::initializeOppList($recordId);
        $this->reportBuiltArray['survey_upload']['van_line_id'] = $this->getReportVanlineId($this->recordModel);
