...
class Vtiger_ModuleBasic
{
    ...
    public function setEntityIdentifier($fieldInstance)
    {
        ...
        //if you just call setEntityIdentifier you would expect it to work, but it doesn't
        //unless you did initTables with the same object first or an entity ident is already set.
        //But as you're setting one it seems fairly reasonable that one may not have been set.
        //so just set the basetableid like initTables does so this will just work.
        if (!$this->basetableid) {
            $lcasemodname = strtolower($this->name);
            if (!$this->basetable) {
                $this->basetable = "vtiger_$lcasemodname";
        ...
        if ($this->entityidfield && $this->entityidcolumn) {
            ...
            if ($adb->num_rows($result) == 0) {
                $adb->pquery("INSERT INTO vtiger_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
                             [$this->id, $this->name, $fieldInstance->table, $fieldInstance->name, $this->entityidfield, $this->entityidcolumn]);
                ...
                $adb->pquery("UPDATE vtiger_entityname SET fieldname=?,entityidfield=?,entityidcolumn=? WHERE tablename=? AND tabid=?",
                             [$fieldInstance->name, $this->entityidfield, $this->entityidcolumn, $fieldInstance->table, $this->id]);
