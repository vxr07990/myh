...
class Vtiger_Filter
{
    ...
    public function __create($moduleInstance)
    {
        ...
        $sql = 'INSERT INTO vtiger_customview(cvid,viewname,setdefault,setmetrics,entitytype';
        $params = array($this->id, $this->name, $this->isdefault, $this->inmetrics, $this->module->name);

        if(Vtiger_Utils::CheckColumnExists('vtiger_customview', 'view')){
            $this->view = $this->view;
            $sql .= ',view';
            array_push($params, $this->view);
        }
     ...
     * @return    Vtiger_FIlter $this
     */
    public function removeField($fieldInstance) {
      global $adb;

      $checkVal = $this->__getColumnValue($fieldInstance);

      $cvid = $adb->pquery("SELECT cvid FROM vtiger_cvcolumnlist WHERE columnname = ?",[$checkVal]);
      if($adb->num_rows($cvid) == 0) {
        $this->log("$fieldInstance->name does not exist in $this->name filter");
        return $this;
      }
      $cvid = $adb->query_result($cvid, 0, 'cvid');

      $adb->pquery("DELETE FROM vtiger_cvcolumnlist WHERE cvid = ? AND columnname = ?",[$cvid,$checkVal]);

      $this->log("Removing $fieldInstance->name from $this->name filter ... DONE");
      return $this;
    }
    ...
    public function addRule($fieldInstance, $comparator, $comparevalue, $index=0, $group=1, $condition='and')
    {
        ...
        $adb->pquery("INSERT INTO `vtiger_cvadvfilter_grouping` (`groupid`, `cvid`, `group_condition`, `condition_expression`) VALUES (?,?,?,?)",[1,$this->id,'and',0]);
     ...
     */
    public static function getInstance($value, $moduleInstance=false, $adminFilter = false)
    {
        global $adb;
        $instance = false;
        ...
        if (Vtiger_Utils::isNumber($value)) {
            ...
            if ($adminFilter){
                $query = "SELECT * FROM vtiger_customview WHERE viewname=? AND entitytype=? AND userid = 1";
            } else {
