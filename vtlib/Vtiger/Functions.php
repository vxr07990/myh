...
class Vtiger_Functions
{
    ...
    public static function updateCRMRecordLabel($module, $id, $label)
    {
        global $adb;
            ...
            if ($ids) {
                ...
                while ($row = $adb->fetch_array($result)) {
                    ...
                    foreach ($columns as $columnName) {
                        if (
                            getenv('IGC_MOVEHQ') &&
                            $columnName == 'orders_contacts' &&
                            $module == 'Orders'
                        ) {
                            //@TODO HERE This is a temporary fix AND should be done better.
                            $labelValues[]      = self::getCRMRecordLabels('Contacts', $row[$columnName])[$row[$columnName]];
                        } else if ($columnName == 'agentid' && $module == 'RevenueGrouping') {
                            $revenueModuleModel = Vtiger_Module_Model::getInstance($module);
                            $revenueFieldModel  = Vtiger_Field_Model::getInstance('agentid', $revenueModuleModel);
                            $labelValues[]      = $revenueFieldModel->getDisplayValue($row[$columnName]);
                        } else {
    ...
    public static function getModuleFieldInfos($mixed)
    {
        ...
        $fields_count = $adb->query_result($no_of_fields, 0, '0');
    ...
    public static function getMergedDescription($description, $id, $parent_type)
    {
        global $current_user;
